...
**Authorized sources (operator list):** [work-jiang-sources.md](../../docs/archive/skill-work-legacy/work-jiang/work-jiang-sources.md) — convention: [work-modules-sources-principle.md](../../docs/archive/skill-work-legacy/work-modules-sources-principle.md).

**Cursor skill — forward lecture chain:** [skill-jiang.md](../../docs/archive/skill-work-legacy/work-jiang/skill-jiang.md) (predict next episode from prefix only; Volume IV bake-off log under `prediction-tracking/`).
...
4. **Compressions** — After a large artifact or handoff, run `python3 scripts/jiang-compress.py -u grace-mar` to emit a labeled JSON pack under `research/external/work-jiang/compressions/` (operator checklist + optional gate stub; not Record). See [COMPRESSION-ENGINE](../../docs/archive/skill-work-legacy/work-jiang/COMPRESSION-ENGINE.md).
...
**Repo CI:** PRs that only touch work-jiang paths should use label **`lane/work-jiang`**. When pasting into `recursion-gate.md`, use the canonical **`### CANDIDATE-*`** block shape (see [work-jiang lane + gate CI](../../docs/archive/skill-work-legacy/work-jiang/LANE-CI.md)).
