...
Voice/bot is deprecated for strategy-codex operator work. See archive/grace-mar-instance/bot/DEPRECATED.md.
...
Usage (local polling):
    ...
    2. pip install -r archive/grace-mar-instance/bot/requirements.txt
    3. python -m bot.bot   # from repo root

For production: use webhook mode via platform/apps/miniapp_server.py (see docs/telegram-webhook-setup.md).
"""
# Allow running as script: python archive/grace-mar-instance/bot/bot.py (simulate package for relative imports)
if __package__ is None:
    import sys
    from pathlib import Path
    sys.path.insert(0, str(Path(__file__).resolve().parent.parent))
    __package__ = "archive/grace-mar-instance/bot"
...
def _load_swarm_orchestrator():
    path = Path(__file__).resolve().parent.parent / "research/auto-research" / "swarm" / "orchestrator.py"
...
def _profile_path(filename: str) -> Path:
    return Path(__file__).resolve().parent.parent / "platform/users" / USER_ID / filename
...
def _checkpoint_to_github() -> tuple[bool, str]:
    ...
    Returns (ok, message). Operator should run this from the archive/grace-mar-instance/bot's repo root.
    """
    repo_root = Path(__file__).resolve().parent.parent
    files = [
    ...
    if archive_path.exists():
        ...
        for line in text.splitlines():
            ...
            if query_lower in line.lower():
                results.append({"section": current_id or "archive/placeholders/evidence", "line": line.strip(), "source": "self-archive.md"})
...
def create_application(webhook_mode: bool = False) -> Application:
    ...
    app.add_handler(CommandHandler("platform/profile", profile_cmd))
...
def main() -> None:
    ...
    assert_canonical_record_layout(uid, context="Telegram archive/grace-mar-instance/bot")
