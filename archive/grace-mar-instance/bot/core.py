...
PROFILE_DIR = Path(__file__).resolve().parent.parent / "platform/users" / USER_ID
...
operator_prompt: "choose keep_rule | revise_rule | scope_rule | gather_more_archive/placeholders/evidence"
```
...
def resolve_intent_debate_packet(
    ...
    safe_resolution = resolution.strip() or "gather_more_archive/placeholders/evidence"
...
_GATE_RECLASSIFY_SURFACE_TO_PROPOSAL_CLASS: dict[str, str] = {
    ...
    "archive/placeholders/evidence": "SELF_KNOWLEDGE_ADD",
...
def reclassify_gate_candidate(
    ...
    *,
    ...
    kwargs["boundary_classification_rel_path"] = (
        f"{fork_slug}/archive/queues/review-queue/boundary-classifications/{candidate_id}.json"
    )
...
def run_lookup(question: str, channel_key: str = "platform/miniapp") -> str:
...
def run_grounded_response(
    question: str,
    channel_key: str = "platform/miniapp",
    ...
    try:
        ...
        emit_pipeline_event("dyad:grounded_query", None, channel_key=channel_key, source="platform/miniapp", replay_mode="dyad")
        return reply
    except Exception:
