# America Authority Carrier Objects

WORK only; not Record.

Use this bench for American vice presidents, cabinet figures, congressional leaders, military commanders, party figures, diplomats, and institutional offices that carry state intent across raw-inputs. These are not biographies. They are compact statecraft objects for tracking whether American policy has a credible carrier beyond presidential impulse, donor pressure, crisis theater, or factional drift.

## Objects

- [J.D. Vance](vance.md) - vice-presidential successor-legitimacy and restraint / realism carrier for Ukraine fatigue, Iran exit pressure, Europe burden language, domestic-populist credibility, and post-Trump continuity.
- [Elon Musk](musk.md) - technology-industrial complex carrier for private compute, satellite communications, launch capacity, AI, platforms, federal contracting, and sovereignty-under-acceleration risk.

## Use

Open an object when an America-lane draft depends on institutional credibility or successor-stable authority rather than a general speaker interpretation. The object should answer:

1. What American state interest is being carried?
2. Is the signal executive policy, factional pressure, campaign positioning, or successor-authority preparation?
3. Which institution can implement it after the headline moment passes?
4. What later U.S. statement, vote, order, leak, or action would falsify this reading?
5. Which speaker-state input helps interpret it without replacing the authority signal?
