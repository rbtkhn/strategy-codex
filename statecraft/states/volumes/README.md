...
- China opens first through the earliest durable dynastic-state chain in the set.
- Persia follows through the Cyrus-founded sovereignty chain, with Zoroaster treated as deeper sacred grammar beneath it.
...
- China, Persia, Rome, and Russia are the stronger core cases.
- America remains in the set because a strained case helps define the category as much as a clean one.

## Opener Rule

Every volume should be opened through the same three questions:

- **Deep grammar** - what sacred, mythic, literary, or civilizational substrate sits beneath the chain
- **Sovereign opening** - what first figure or formation opens the political continuity chain
- **Current carrier** - what present institution or state form now bears the chain
...
Use the opener block operationally:

- **Deep grammar** routes toward sacred grammar, literature, and legitimacy surfaces
...
**Andante note (`legacy-cut`, v0.1.3):** Export manifest [`config/civilizational_statecraft_public_export.yaml`](../../../platform/config/civilizational_statecraft_public_export.yaml) declares `legacy_archive.rel_path: archive/helix-lane-v1/legacy-cut` with `sanitize: true`, but the export script **does not copy** legacy-cut from `statecraft/states/` ? it only **sanitizes** that path when already present in the output tree. v0.1.3 PR #2 used a full staging sync; staging never contained `legacy-cut`, so the public drop is **absence from export source**, not a sanitize pass deleting workshop files. Confirmed on tag v0.1.3: `archive/helix-lane-v1/README.md` only.
...
- [Rome sovereign continuity](civ-state-rome/sovereign-continuity.md)
- [Russia sovereign continuity](civ-state-russia/sovereign-continuity.md)
- [America sovereign continuity](civ-state-america/sovereign-continuity.md)
...
- If the operator is asking `which civilization do I open?`, start here.
...
- China, Persia, Russia, and America are lane-backed volumes with live repo-root statecraft translations.
- Rome is currently a source-memory volume with retrieval anchors and index companions, not yet a full repo-root lane peer.

## Boundary Reminder

The volume layer answers:

- `what is this civilization-state's continuity chain?`
- `where does that chain open?`
- `what present carrier bears it now?`

It does **not** answer by itself:

- lane ownership of a live crisis object
