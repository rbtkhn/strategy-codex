...
Rank the strongest episodes or raw-input anchors.

1. [{best-anchor-title}]({best-anchor-path})
   Why it anchors the arc:

2. [{second-anchor-title}]({second-anchor-path})
   Why it matters:
...
Open **{best-date-or-title}** first when the operator needs:
...
Best paired read: [{paired-arc-or-object}]({paired-path})

Why:

Second-best paired read: [{optional-second-pair}]({optional-second-pair-path})
