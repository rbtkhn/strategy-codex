# Hormuz Recognition Transit Transaction

WORK only; not Record.

## Transaction Name

Hormuz recognition-for-transit restraint clause.

## State Lane

`iran`

## Source Inputs

- Speaker-state inputs: [Pape](../../voices/pape) for coercive-center leverage and shortage clocks; [Parsi](../../voices/parsi) for settlement architecture and sanctions-relief sequencing; [Marandi](../../voices/marandi) for Iranian bargaining logic; [Crooke](../../../README.md) for Western misreading; [Ritter](../../voices/ritter) for regime-change and military-risk warnings.
- Scoreboards used: [Pape coercive-center index](../../sheets/pape-coercive-center-index.md); [Mearsheimer structural-realist scoreboard](../../sheets/mearsheimer-structural-realist-scoreboard.md).
- Raw-input / provenance anchors: [Pape, 2026-04-16](../../../source-archive/statecraft/2026-04-16/source-pape-the-iran-war-is-about-to-hit-a-supply-2026-04-16.md); [Pape, 2026-05-03](../../../source-archive/statecraft/2026-05-03/source-pape-what-the-first-60-days-actually-changed-and-why-the-next-30-will-be-worse-2026-05-03.md); [Pape, 2026-05-18](../../../source-archive/statecraft/2026-05-18/source-pape-june-could-be-the-most-brutal-month-2026-05-18.md); [Pape Day 100, 2026-06-08](../../../source-archive/statecraft/2026-06-08/source-pape-day-100-why-irans-strategy-is-expanding-2026-06-08.md); [Marandi, 2026-06-08](../../../source-archive/statecraft/2026-06-08/source-glenn-diesen-seyed-m-marandi-iran-israel-strikes-trump-humiliated-yemen-red-sea-2026-06-08.md); [Parsi, 2026-05-01](../../../source-archive/statecraft/2026-05-01/source-parsi-trumps-blockade-snatches-defeat-from-the-jaws-of-victory-2026-05-01.md); [Parsi, 2026-05-03](../../../source-archive/statecraft/2026-05-03/source-parsi-a-few-observations-on-irans-latest-proposal-to-trump-2026-05-03.md).
- Paired gate instrument: [Lebanon third-party recognition gate transaction](lebanon-third-party-recognition-gate-transaction.md) — **SSOT for coupled-braid ledger rows** (`D-RED-*`, `D-HORM-01`, `D-LINK-01`) and gate-green precondition for transit notice here.
- Statecraft notes: [Pape Day 100 indicator watch sheet](../../notes/2026-06-08-pape-day-100-indicator-watch-sheet.md); [Marandi deal floor × gate clauses](../../notes/2026-06-08-persia-marandi-deal-floor-vs-lebanon-gate-clauses.md) — [§Hormuz Activation × `D-HORM-01`](../../notes/2026-06-08-persia-marandi-deal-floor-vs-lebanon-gate-clauses.md#hormuz-activation-rule--marandi-deal-floor-line-by-line).

## Historical Continuity

- Current government: Islamic Republic governing institutions, including executive, foreign ministry, military, and security institutions.
- Current state: Iranian state sovereignty over territory, ports, waters, and national security organs.
- Predecessor institutions: Persian / Iranian state continuity, imperial and republican maritime-adjacent state memory, and recurring outside-power intervention pressure.
- Geography: Strait of Hormuz, Gulf littoral, Iranian plateau, energy export routes, missile and drone depth, and resource base.
- Civilizational memory: resistance to humiliation, defense of sovereignty, and insistence that Iran is a necessary regional actor rather than an object of outside management.
- Possible successor authority: any successor Iranian state would still inherit geography, energy interests, sovereignty claims, and the need for non-humiliating security recognition.
- What survives regime or constitutional change: territorial integrity, Hormuz salience, export autonomy, sanctions-relief interest, and demand for security recognition.

## Pattern / Narrative Conversion

- Civilizational pattern or narrative: Outside powers misread Iran as coercible because they treat Hormuz as a shipping-management problem rather than a state and civilizational leverage problem carried by durable geography, sovereignty, and anti-humiliation memory.
- Authority: Named Iranian and reciprocal foreign authorities must authorize one coupled package of transit guarantees, restraint boundaries, security recognition, sanctions-relief triggers, and complaint channels.
- Restraint: Iran restrains transit disruption and third-party targeting only inside a bargain that simultaneously recognizes Iranian security interests, limits humiliation, and sequences reciprocal economic relief.
- Settlement: A phased transit-security compact that converts coupled leverage into recognized restraint, predictable commerce, and a governed bargaining channel rather than one-off de-escalation theater.

## Braid Structure

- Arcs: commercial passage; coercive signaling; negotiation-room leverage.
- Lens: announced-vs-material.
- Coupling claim: Passage language, coercive posture, and bargaining leverage belong to one draft object because each line changes the credibility and meaning of the others. A commercial-open formula without a coercive theory reads as rhetoric. Coercive signaling without a bargaining channel reads as permanent spoiler behavior. Negotiation leverage without a transit object lacks a governable mechanism.
- Authority carriers: Iranian foreign ministry, Supreme National Security Council, maritime and port authorities, mediator channel, and sanctions-relief mechanisms must keep the bundle aligned so the announced line, operating posture, and bargaining sequence do not split apart.
- Transaction use: This braid is being spent as a recognition-for-transit restraint clause.
- Tensions / falsifiers: The braid weakens if material transit conditions do not match the announced line, if outside partners react as though Iran is seeking durable veto rather than governed leverage, or if bargaining posture no longer converts restraint into an actual sanctions-relief and security-recognition sequence.

## Worked Example Placement

This file is the concrete endpoint of the workshop's new three-step example:

1. [State braid model](../../sheets/state-braid-model.md) keeps the Hormuz arcs together as one draftable object.
2. [Transaction router](../../sheets/transaction-router.md) distinguishes the Iran-local recognition route from the wider cross-lane transit / sanctions route.
3. This transaction spends the routed braid as one clause that couples transit restraint, recognition, relief sequencing, and review.

The practical question answered here is not "can Hormuz be reopened?" It is "what would let Iran present reopening as recognized restraint rather than surrender or permanent spoiler leverage?"

### Worked Example Snapshot

- Braided arcs: commercial passage + coercive signaling + negotiation-room leverage
- Lens: `announced-vs-material`
- Router choice: Iran-local recognition through transit restraint
- Authority carriers: Iranian foreign ministry, Supreme National Security Council, maritime / port authorities, mediator channel, sanctions-relief mechanisms
- Spendable output: a renewable transit guarantee tied to reciprocal security assurances, phased sanctions relief, complaint review, and an observable compliance hinge

If one of those lines drops out, the transaction degrades:

- transit without recognition reads as capitulation pressure
- coercion without review reads as spoiler leverage
- sanctions relief without transit restraint reads as payment for instability
- recognition without authority carrier reads as rhetoric

- Prior statecraft drafts: [Hormuz Strait Transit Agreement v0](../../../../hormuz-strait-transit-agreement-v0.md); [Hormuz Reciprocal Transit De-escalation Framework v2.1](../../../../hormuz-reciprocal-transit-deescalation-framework-v2-1.md).

## Coupled Gate Activation (Lebanon gate braid SSOT)

This transaction **does not duplicate** Bab el-Mandeb / Red Sea observability. Braid row definitions live in [Lebanon gate transaction — Clause D](lebanon-third-party-recognition-gate-transaction.md#clause-d--coupled-chokepoint-braid).

**Activation rule for Hormuz guarantee issuance:**

| Prerequisite | Source row / status |
| --- | --- |
| Lebanon/Gaza gate benchmark **green** | Gate transaction Observability Hinge |
| Braid status not **`failed`** | `D-RED-04` cluster or unlogged dual-corridor contest |
| Trigger log cross-reference when braid moves | `D-LINK-01` within 24h of any `D-*` status change |
| Hormuz material line matches announced line | `D-HORM-01` within pre-war daily band (Marandi reference: ~25–30 transits/day) for **72h** before guarantee window opens |

**Braid status rollup (derived — do not maintain a second ledger):**

| Rollup | Condition |
| --- | --- |
| `BRAID-OK` | All `D-*` rows pass; gate-green |
| `BRAID-STRESSED` | Any `D-*` → stressed **or** `PAUSE-CAVEAT` active |
| `BRAID-FAILED` | `D-RED-04` cluster **or** Hormuz-only relief offered while `D-LINK-01` fires |

Short rule: **Hormuz reopening is downstream of gate + braid; braid rows are owned by the gate transaction; this file spends the transit guarantee once rollup permits.**

## Observability Hinge

The credibility test for this transaction is whether outside actors can observe that restraint is governed rather than merely declared.

Minimum observable package:

- **Gate-green + braid rollup not `failed`** per [Lebanon gate — Coupled Gate Activation](lebanon-third-party-recognition-gate-transaction.md) and [Pape Day 100 watch sheet](../../notes/2026-06-08-pape-day-100-indicator-watch-sheet.md).
- Iran publishes a 30-day transit guarantee through named state carriers rather than through informal signaling alone.
- A neutral channel circulates the active transit-notice and complaint-contact mechanism to commercial shippers, insurers, and key partner states.
- During the guarantee window, Iran refrains from new transit interference against non-belligerent commercial vessels except under the suspension terms named in the clause.
- A mediator or secretariat records whether reciprocal assurances, relief steps, and complaints were performed on schedule.
- **`D-HORM-01` logged** in gate ledger at guarantee open and at each 30-day renewal — transit count band is the material proof of the announced commercial-passage line.

If those signals are absent, the transaction should be read as a tactical pause rather than recognized restraint.

**Hormuz-only reopening falsifier:** If transit notice issues while Lebanon/Gaza gate fails, `D-LINK-01` is active, or braid rollup is `failed`, read as **spoiler leverage** or ** capitulation theater** — not recognized restraint per [gate transaction falsifiers](lebanon-third-party-recognition-gate-transaction.md#falsifiers).

## Civilization / Empire Helix Input

- Civilization input: dignity, sovereignty, and refusal to be managed as a disposable target.
- Empire input: Hormuz leverage, denial capacity, sanctions endurance, and partner channels.
- Statecraft regulation: monetize leverage into recognition, relief, and restrained transit rather than permanent disruption.
- Relevant lane helix: [Iran helix](../helix.md).

## State Interest

Iran's durable interest is to convert the coupled Hormuz bundle of passage, coercive leverage, and negotiation-room advantage into recognized sovereignty, sanctions relief, deterrence, and regional architecture without becoming a permanent spoiler or losing escalation control.

## Leverage / Constraint

- Leverage being spent: Geography, Hormuz denial capacity, resource self-sufficiency, sanctions endurance, nuclear latency, partner channels, and the ability to impose system costs through a coupled transit-and-recognition problem rather than through brute disruption alone.
- Constraint that limits the leverage: Prolonged disruption can alienate China, India, Gulf states, and non-aligned states; it can also make sanctions relief politically impossible for adversaries and split the announced commercial-passage line from material reality.
- Volatility note if Pape and Mearsheimer scoreboards diverge: Pape may show high immediate coercive leverage while Mearsheimer still treats Iran as structurally weaker than comprehensive powers; the transaction must monetize the coupled bundle before counter-balancing erodes it or before the passage line loses credibility.

## Richelieu Validation

- Institutional carrier: Iranian foreign ministry, Supreme National Security Council, port and maritime authorities, central bank / sanctions-relief channels, and a third-party mediator or treaty secretariat must be able to carry one aligned transit, restraint, and relief package rather than three drifting lines.
- Administrative / fiscal / legal / military capacity: Maritime notification system, transit registry, sanctions-relief escrow, defensive-force command discipline, and complaint-resolution procedure are needed to keep the braid coordinated in practice.
- Observability requirement: The same carriers must be able to make restraint visible through published notice, complaint handling, and recorded compliance milestones rather than through rhetoric alone.
- Continuity risk: Factional actors may prefer maximal resistance language over a limited settlement; external actors may try to split Iranian institutions or detach transit from recognition and relief.
- Ideology or faction pressure: Anti-humiliation rhetoric can harden into terms no adversary can publicly accept.
- Long-pressure burden: Iran must sustain deterrence while proving it can regulate a coupled leverage bundle rather than merely threaten disruption.

## Bismarck Validation

- Limited aim: Secure recognition, sanctions-relief sequence, and transit-restraint bargain; do not demand full regional settlement in one clause.
- Actor to isolate: Regime-change advocates and actors seeking uncontrolled escalation.
- Actor to reassure, restrain, or keep neutral: China, India, Gulf states, Oman, Russia, and European energy importers.
- Alliance geometry: Preserve Russia and China support while leaving Gulf and Oman channels open; do not force partners to choose between Iran and energy stability.
- Restraint after advantage: Reopen transit predictably after recognition and relief benchmarks begin, rather than extending disruption to prove power or treating passage, coercion, and bargaining as separately expandable lines.
- Off-ramp: Reciprocal transit-security reviews every 30 days with automatic de-escalation steps if violations decline and relief milestones are met.

## Instrument Text

Draft clause:

> **Activation precondition:** No guarantee under this instrument shall issue unless the [Lebanon third-party recognition gate](lebanon-third-party-recognition-gate-transaction.md) mediator ledger records gate benchmark **green**, coupled-braid rollup not **`failed`**, and **`D-HORM-01`** within the pre-war daily transit band for 72 consecutive hours, with any braid status change cross-referenced to **`D-LINK-01`** and the gate trigger log.
>
> Upon written reciprocal assurances against regime-change operations, attacks on Iranian sovereign facilities, and interdiction of lawful Iranian energy commerce, and upon commencement within 72 hours of an initial verified and renewable sanctions-relief step through a neutral channel, Iran shall issue a renewable 30-day guarantee of safe commercial transit through the Strait of Hormuz for non-belligerent vessels. This guarantee shall be treated as a limited de-escalation and transit-security mechanism that converts reciprocal compliance into governed restraint, without prejudice to the wider positions of any party on regional security, alliance structure, nuclear status, or final sanctions architecture.
>
> Within 24 hours of issuance, the designated Iranian authorities and the mediator shall publish the guarantee window, complaint contact, suspension standard, and insurer-facing transit notice to commercial carriers, insurers, Gulf littoral states, and named partner governments. A parallel Gulf incident channel shall provide rapid reporting and deconfliction for commercial-shipping disputes during the guarantee period, but shall not exercise supervisory authority over Iranian sovereign decision-making.
>
> The mediator shall maintain a neutral compliance ledger recording transit conditions, reciprocal relief performance, security-assurance performance, and any filed complaints. Iran shall refrain from interference with non-belligerent commercial vessels during the guarantee window except in cases of direct armed attack or documented material breach of the reciprocal package. Suspension shall require documented violation, 72-hour notice to the mediator and notified participants, and failure of urgent consultation, except in cases of direct armed attack.
>
> Relief measures initiated under this instrument may be paused or reversed only through the same mediated compliance process that records performance under the transit guarantee. All parties shall treat this mechanism as a reciprocal and renewable bargain of restraint, recognition, and observable implementation, not as a unilateral concession, a standing veto over navigation, or a final settlement of wider regional disputes.

## Comparative Placement

This file remains the canonical Iran-lane transaction for the Hormuz recognition line. When the live object includes **Lebanon third-party triggers** and **Washington command-capture constraints**, pair with [Lebanon third-party recognition gate transaction](lebanon-third-party-recognition-gate-transaction.md) — gate benchmarks and **`D-*` braid rows (SSOT)** precede or coincide with transit/relief activation here. Weekly operational review: [Pape Day 100 indicator watch sheet](../../notes/2026-06-08-pape-day-100-indicator-watch-sheet.md). Cross-lane redlines, external objections, and comparison notes belong under [Hormuz Transit / Sanctions Relief Compact](../../transactions/hormuz-transit-sanctions-relief-compact/README.md) so the Iran clause is refined without competing with a second lane-local authority.

## Mirror Test

What is the shortest plausible objection from each of the other national lanes?

- America: This rewards coercive disruption and may prove Iran has a veto over global commerce.
- Russia: This may spend Iran's leverage too cheaply and reduce pressure on U.S. primacy before broader concessions are secured.
- China: This is useful only if transit predictability is real; prolonged ambiguity damages energy and supply-chain stability.
- Iran: This risks freezing Iran's leverage while sanctions relief is delayed or reversed.

## Falsifiers / Revisit Triggers

- Falsifier: China, India, Gulf states, or Russia publicly distance from Iran's transit position in a way that deprives Tehran of bargaining depth.
- Falsifier: Hormuz guarantee issued while gate benchmark red or braid rollup **`failed`** — treat as tactical pause / spoiler leverage (see gate transaction + watch sheet).
- Revisit trigger: U.S. or allied interdiction expands, Iran changes transit conditions, or a mediator proposes a sanctions-relief / transit-security formula.
- Scoreboard movement that would require review: Pape coercive-center score falls because Hormuz loses salience, or Mearsheimer score worsens because balancing pressure sharply rises.
- Actor defection or compliance signal: Gulf cooperation with a U.S.-led coercive framework, Chinese insurance or payment workaround, Iranian controlled reopening, or verified sanctions-relief step.

## Validity Status

`volatile` — Day 100 ceasefire breakdown; dual-corridor braid stressed; gate-green not met for guarantee issuance; executive channel non-credible for reciprocal assurances.

## Return Paths

- [Lebanon third-party recognition gate transaction](lebanon-third-party-recognition-gate-transaction.md) — braid row SSOT
- [Marandi deal floor × Hormuz activation](../../notes/2026-06-08-persia-marandi-deal-floor-vs-lebanon-gate-clauses.md#hormuz-activation-rule--marandi-deal-floor-line-by-line)
- [Pape Day 100 indicator watch sheet](../../notes/2026-06-08-pape-day-100-indicator-watch-sheet.md)
- [Persia transactions README](README.md)
- [Persia helix](../helix.md)

## Recursive Update Candidates

- Candidate needed: no
- Target:
- Candidate summary:
- Suggested action:
