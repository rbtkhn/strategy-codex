# Arc-Conditioned CIV-EMP Retrieval

WORK only; not Record.

Purpose: connect speaker-arc interpretation to `civ-emp` retrieval without collapsing speaker evidence into generic historical commentary or collapsing `civ-emp` into transcript gloss.
...
3. **CIV-EMP retrieval**
   - open the narrowest relevant `civ-emp` object first
4. **Lane translation**
...
- [Marandi CIV-EMP retrieval adapter](marandi-civ-emp-retrieval-adapter.md)
- [Parsi CIV-EMP retrieval adapter](parsi-civ-emp-retrieval-adapter.md)
...
Open these notes when the statecraft problem is not merely "what did the speaker say?" but "how should that kind of speaker claim change what I retrieve, test, and draft from `civ-emp`?"
