# Mercouris February 2025 Opening Watchlist

work only; not Record.

## Purpose

Open a bounded February 2025 Mercouris watchlist before any stronger repair or closure claims are made.

This note preserves the local archive truth currently on disk, distinguishes the solo lane from the co-hosted/panel lane, and marks February as an active month for further Mercouris URL recovery.

## Current February Mercouris Truth

On current local evidence, February 2025 is one of the weakest Mercouris months in the 2025 archive.

The month rollup at [2025-02.md](/C:/dev/strategy-codex/source-archive/statecraft/2025-02.md) shows:

- `27` source files total
- `7` Mercouris-thread captures total
- `2` current captures are `native solo Mercouris`
- `1` current capture is a `Glenn Diesen / Mearsheimer / Mercouris` panel object
- `4` current captures are `Duran / Mercouris co-hosted`

Against the current working floor of `8+` Mercouris-thread captures per month, February is still far below target.

## Native Solo Mercouris Present

The currently landed `native solo Mercouris` February captures are:

- `2025-02-10` - [trump-confirms-putin-call-putin-firm-4-regions-russian-ukraine-kursk-disaster-eu-gas-prices-surge](/C:/dev/strategy-codex/source-archive/statecraft/2025-02-10/source-alexander-mercouris-trump-confirms-putin-call-putin-firm-4-regions-russian-ukraine-kursk-disaster-eu-gas-prices-surge-2025-02-10.md)
- `2025-02-20` - [disastrous-zelensky-presser-angers-trump-zelensky-dictator-us-aid-gravy-train-hints-disengagement](/C:/dev/strategy-codex/source-archive/statecraft/2025-02-20/source-alexander-mercouris-disastrous-zelensky-presser-angers-trump-zelensky-dictator-us-aid-gravy-train-hints-disengagement-2025-02-20.md)

So February currently preserves only a minimal native solo Mercouris lane.

## Co-Hosted / Panel Presence

The currently landed February Mercouris panel object is:

- `2025-02-15` - [trump-to-force-ukraine-peace-on-europe](/C:/dev/strategy-codex/source-archive/statecraft/2025-02-15/source-diesen-mearsheimer-mercouris-trump-to-force-ukraine-peace-on-europe-2025-02-15.md)

This matters because February is not purely solo-thin. It has at least one non-solo Mercouris lane on disk already.

The currently landed February `Duran / Mercouris co-hosted` capture is:

- `2025-02-07` - [trumps-ukraine-dilemma-escalate-or-walk-away](/C:/dev/strategy-codex/source-archive/statecraft/2025-02-07/source-duran-mercouris-trumps-ukraine-dilemma-escalate-or-walk-away-2025-02-07.md)
- `2025-02-10` - [us-carrot-and-stick-offer-to-russia](/C:/dev/strategy-codex/source-archive/statecraft/2025-02-10/source-duran-mercouris-us-carrot-and-stick-offer-to-russia-2025-02-10.md)
- `2025-02-21` - [uk-eu-take-ownership-of-project-ukraine-preemptive-coup](/C:/dev/strategy-codex/source-archive/statecraft/2025-02-21/source-duran-mercouris-uk-eu-take-ownership-of-project-ukraine-preemptive-coup-2025-02-21.md)
- `2025-02-25` - [ukraine-frontline-update-and-peace-deal-stanislav-krapivnik-live](/C:/dev/strategy-codex/source-archive/statecraft/2025-02-25/source-duran-mercouris-ukraine-frontline-update-and-peace-deal-stanislav-krapivnik-live-2025-02-25.md)

That means February now has all three visible Mercouris lanes on disk in minimal form:

- `native solo Mercouris`
- `Glenn Diesen / Mearsheimer / Mercouris` panel presence
- `Duran / Mercouris co-hosted`

## Opening Recovery Position

February now has an opening recovered `Duran / Mercouris co-hosted` queue.

The strongest currently recovered candidates are:

- `2025-02-07` - [Trump's Ukraine dilemma, escalate or walk away](https://podcasts.apple.com/us/podcast/trumps-ukraine-dilemma-escalate-or-walk-away/id1442883993?i=1000690042205)
This does not yet give February a closure-ready queue. But it does prove that the co-hosted lane is real on disk and now carries a genuine February run rather than a single stray object.

## Watchlist Judgment

On current evidence, the honest watchlist judgment is:

- `healthy`: none
- `thin but present`: the two landed solo Mercouris captures plus the one landed Diesen/Mearsheimer/Mercouris panel object and the four landed Duran/Mercouris co-hosted objects
- `needs backfill attention`: the broader February Mercouris month, especially the now clearly recoverable `Duran / Mercouris` co-hosted lane

## Month Shape

February does not yet read like a naturally complete light month.

It reads like:

- a smaller month than June, but still too thin for Mercouris
- a month where only fragments of the solo lane are preserved
- a month where both panel and Duran co-hosted Mercouris lanes are present, but only minimally
- a month where the Duran co-hosted lane is now visibly recoverable from external evidence

## Use Rule

Use this note when:

- someone asks which weak month should be opened next for Mercouris recovery
- someone wants the current honest February Mercouris month truth
- someone needs the distinction between February's `solo-thin` condition and its existing non-solo panel presence

Do not use this note to claim that February has a closure-ready repair verdict yet.
