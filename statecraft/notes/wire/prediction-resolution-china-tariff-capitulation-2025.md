---
note_id: prediction-resolution-china-tariff-capitulation-2025
note_type: wire
authority_level: draft
source_basis: mixed
essay_candidate: false
event_id: china_tariff_capitulation_2025
prediction_lane: freeman-pilot
created_at: 2026-06-29
updated_at: 2026-06-30
links:
  - statecraft/data/event-registry.json
  - statecraft/voices/freeman/freeman-predictions.md#china_tariff_capitulation_2025
---

# Prediction resolution — china_tariff_capitulation_2025


**Event question:** Will China capitulate to Trump's tariff pressure (major substantive yield, not cosmetic)?  
**Resolution criteria:** Operator resolves from documented trade/policy concessions.  
**Freeman arc:** [freeman-predictions.md#china_tariff_capitulation_2025](../voices/freeman/freeman-predictions.md#china_tariff_capitulation_2025)  
**Status:** open · **Outcome:** null · **Horizon:** Freeman timeless negation (Apr 22 capture)

## Closure hooks (tier 3)

| ID | Hook | Skill | Verdict | Cite |
| --- | --- | --- | --- | --- |
| R1 | Documented Chinese **substantive yield** vs holding lines under tariff pressure | fact-check | | |

## Resolution decision (operator)

- **Outcome:**
- **Resolved date:**
- **Registry paste:** `resolution_source: fact-check: … — see prediction-resolution-china-tariff-capitulation-2025.md#resolution-decision`
