...
|--------|--------------------------|--------------------------|--------------|
| Substack archive / post body | `scripts/backfill_substack_raw_input.py` | `backfill_*_substack_raw_input.py` | Capture substantial posts into `provenance/<pub_date>/`; source adapters should mostly pin host, thread, and defaults. |
...
| Short-form screenshot / OCR bundles | `scripts/backfill_shortform_bundle_raw_input.py` | `backfill_*_x_shortform_bundle_raw_input.py`, `backfill_*_shortform_bundle_raw_input.py` | Generic daily bundle capture for X, Threads, Truth Social, Locals, and similar sources; source adapters should mostly pin platform, account, and thread routing. |
