# CIV-STATE vs Statecraft

This note distinguishes two adjacent but different layers in academy-statecraft.

`Civilizational Statecraft` is the governing discipline. `Civilization and Empire` is the conceptual helix that opens the book. `civ-state` is the repo-root source-memory substrate that preserves that helix for live drafting.

Short form:

- `Civilization and Empire` is the reference historical helix and source world
- `statecraft` is the menu, workflow, and output layer built on top of that source base

Another short form:

- `civ-state` = source memory and imperial pattern base
- `statecraft` = lane activity, drafting, routing, and state-bearing instrument design
- `civ-state` = the civilization-state object the downstream layer is trying to understand and draft for

## Civilization and Empire

`Civilization and Empire` is the historical reference corpus behind academy-statecraft.

In repo terms, `civ-state` is the compact working source base derived from that corpus.

It asks how civilization projects, organizes, and sustains power outward across time.

Its center of gravity is:

- empire
- corridors
- chokepoints
- administrative scale
- maritime control
- sanctions
- alliance structure
- system-shaping leverage

Its governing question is:

`How does a civilization-bearing power extend order beyond itself?`

This layer is upstream. Its job is to preserve patterns, counterweights, and historical mechanism.

## Statecraft

`statecraft` is the statecraft menu and all related activity and output.

In repo terms, this includes the statecraft workshop surface, deployer/lane routing, lane-local judgment, drafting, comparison, objections, compacts, and transactions.

It asks how a civilization inhabits a state form and makes statecraft legible from inside that grammar now.

Its center of gravity is:

- legitimacy
- memory
- dignity
- authority
- restraint
- recognition
- settlement language
- what the state can actually draft or accept without violating its own civilizational identity

Its governing question is:

`What can this civilization-backed state legitimately carry now?`

This layer is downstream. Its job is to turn source memory into usable judgment, routing, and instrument design.

## Repo Relationship

The clean constitutional relationship is:

`civ-state` is the upstream source-base repo.  
`statecraft` is the downstream operating layer that consumes it.

Short doctrine:

`civ-state remembers; statecraft drafts.`

Slightly fuller:

`civ-state` supplies historical pattern memory and imperial structure. `statecraft` turns that source base into lane judgment, routing, and instruments.

### `civ-state` owns

- Civilization and Empire reference patterns
- source-object schemas
- retrieval logic
- historical counterweights
- source-base proof objects

### Statecraft owns

- menu surfaces
- `state-deploy`
- `state-*` lane skills
- lane-local routing
- objection matrices
- compact and treaty drafting
- live workshop output

### Membrane

- `civ-state` sends patterns, hooks, and retrieval structure downstream
- statecraft sends reviewable update candidates upstream
- neither layer should silently rewrite the other through ambient drift

## Upstream-Candidate Workflow

Use this workflow when live statecraft work discovers something durable.

1. **Classify the discovery**
   - source pattern
   - retrieval rule
   - historical counterweight
   - source-object schema
   - lane judgment
   - routing rule
   - transaction pattern
2. **Decide the owning layer**
   - if it changes source memory, retrieval architecture, counterweight logic, or object contract, it belongs upstream in `civ-state`
   - if it changes menus, deployers, lane-local judgment, objections, or instrument design, it belongs downstream in statecraft
3. **Draft downstream first when the live object needs it**
   - do not block a real instrument waiting for source-base perfection
4. **Stage the upstream candidate explicitly**
   - name the source insight
   - name the current proof object or live draft that exposed it
   - name the counterweight or failure mode if the source base stays unchanged
5. **Do not dual-write by drift**
   - no ambient patching of both layers just because the same idea is visible in each
6. **Review upstream on its own terms**
   - `civ-state` should accept only what improves source memory, retrieval, or historical mechanism rather than whatever happened to be rhetorically useful in one draft

Default test:

- `Would another lane benefit from this as source memory?` Send it upstream.
- `Is this mainly about how we route, draft, or phrase an instrument here?` Keep it downstream.

## Why The Difference Matters

A draft can fail the **civilization-state test** even when it is strategically clever, because it humiliates the actor, breaks legitimacy, or asks a state to speak in a foreign grammar.

A settlement can fail `civ-state` even when it sounds morally or diplomatically acceptable, because it ignores corridors, enforcement geometry, balance structure, or chokepoint control.

If `civ-state` is how power travels, the **civilization-state** question is how authority speaks.

Both are necessary. Reach without legitimacy overextends. Legitimacy without reach becomes ornamental.

## Working Split In Academy-Statecraft

In this repo, the clean operating split is:

- `civ-state/` holds the compact source base derived from the Civilization and Empire corpus
- statecraft is the downstream activity layer built on top of that base
- lane-local `civilization/`, `empire/`, `state/`, `helix.md`, and `transactions/` convert those patterns into drafting judgment
- `state-deploy` decides lane ownership inside statecraft
- `state-*` skills perform lane work once ownership is settled

So:

- `Civilization and Empire` is the reference historical corpus
- `civ-state` is its local working source base
- `statecraft` is the menu, workflow, and output layer above it

## Four-Lane Stress Test

### America

`civ-state` question:
How much alliance, financial, maritime, and institutional reach can America still organize and sustain?

**civilization-state** question:
What can American authority still lawfully carry tomorrow without proving constitutional exhaustion or imperial fraud?

Failure modes:

- fail `civ-state`: ignore burden-sharing, coercive-center limits, or system-maintenance costs
- fail the civilization-state test: draft a clause that sounds tactically strong but illegitimate to American successor authority

### China

`civ-state` question:
How does China shape flows, corridors, supply continuity, and anti-disorder order without overowning visible disorder?

**civilization-state** question:
What can the Chinese state carry as orderly, patient, and legitimate without being forced into theatrical coercion?

Failure modes:

- fail `civ-state`: miss substrate control, energy exposure, or commercial-route geometry
- fail the civilization-state test: force China into language or timing that violates administrative poise and anti-chaos legitimacy

### Persia

`civ-state` question:
What leverage exists in transit, geography, proxy depth, sanctions friction, and regional bargaining position?

**civilization-state** question:
What can Iran carry with dignity, sovereignty, recognition, and verification without humiliation?

Failure modes:

- fail `civ-state`: ignore Hormuz, deterrent reach, or regional pressure geometry
- fail the civilization-state test: draft a technically smart deal that reads in Tehran as submission disguised as procedure

### Russia

`civ-state` question:
How does Russia preserve strategic depth, disruption capacity, escalation leverage, and parity theater across the system?

**civilization-state** question:
What can the Russian state carry without accepting managed humiliation, entrapment, or false settlement language?

Failure modes:

- fail `civ-state`: ignore depth, buffers, force posture, and coercive timing
- fail the civilization-state test: ask Russia to sign a clause that erases parity or recodes defeat as peace

## Practical Rule

When a draft feels strategically sound but politically impossible, check the **civilization-state** test.

When a draft feels morally elegant but geopolitically naive, check `civ-state`.

The strongest statecraft objects survive both tests.

## Four-Lane Membrane Table

| Lane | Downstream statecraft keeps | Upstream `civ-state` candidate learns |
| --- | --- | --- |
| America | clause wording, carrier choice, ally sequencing, lawful-restraint framing in the live instrument | declining hegemons preserve bargaining legitimacy when restraint is framed as lawful continuity rather than concession |
| Russia | parity wording, verification acceptability, off-ramp sequencing, anti-humiliation settlement carriage | anti-humiliation powers reject settlement forms that require compliance before parity recognition |
| China | deconfliction language, review cadence, guarantor posture, commercial reassurance design | order-centered powers preserve legitimacy through predictable administrative flow management rather than theatrical coercion |
| Persia | dignity-preserving clause language, recognition sequencing, verification design, Gulf notification and suspension mechanics | anti-humiliation states under coercive pressure accept verification only when it is framed as reciprocal sovereign recognition rather than supervisory inspection |
