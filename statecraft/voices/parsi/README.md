This surface is non-authoritative and subject to revision.

# Parsi

This is the canonical Parsi statecraft speaker shelf.

Use it for speaker identity, authored/interview crossing, cross-context thread retrieval, bounded mature month support, and `civ-state`-aware historical interpretation.

## Open First

- [Speaker profile](parsi-profile.md) — identity, voice fingerprint, convergence/tension, links hub
- [parsi-arc.md](parsi-arc.md)
- [parsi-routing.md](parsi-routing.md)
- [parsi-2025-present-arc-threads.md](parsi-2025-present-arc-threads.md)
- [parsi-source-index.md](parsi-source-index.md)
- [parsi-helix.md](parsi-helix.md)
- [parsi-forecast-ledger-2025-2026.md](parsi-forecast-ledger-2025-2026.md)
- [parsi-interview-appearances-2025-2026.md](parsi-interview-appearances-2025-2026.md)
- [arc-march-2026-cross-context-parsi-host.md](parsi-march-2026-cross-context-arc.md)
- [arc-may-2026-cross-context-parsi-host.md](parsi-may-2026-cross-context-arc.md)
- [arc-june-2026-cross-context-parsi-host.md](parsi-june-2026-cross-context-arc.md)
- [parsi-themes.md](parsi-themes.md)
- [parsi-historical-examples-audit.md](parsi-historical-examples-audit.md)

## Canonical Structure

Parsi's canonical statecraft shelf is a **normalized cross-context shelf**, not a host-style month ladder.

Its main speaker-owned layers are:

- **Person arc:** [parsi-arc.md](parsi-arc.md)
- **Routing surface:** [parsi-routing.md](parsi-routing.md)
- **Recurring thread atlas:** [parsi-2025-present-arc-threads.md](parsi-2025-present-arc-threads.md)
- **Authored/interview crossing:** [parsi-helix.md](parsi-helix.md)
- **Authored mechanism spine:** [parsi-forecast-ledger-2025-2026.md](parsi-forecast-ledger-2025-2026.md)
- **Interview-pressure surface:** [parsi-interview-appearances-2025-2026.md](parsi-interview-appearances-2025-2026.md)
- **Bounded mature month support:** [arc-march-2026-cross-context-parsi-host.md](parsi-march-2026-cross-context-arc.md), [arc-may-2026-cross-context-parsi-host.md](parsi-may-2026-cross-context-arc.md), and [arc-june-2026-cross-context-parsi-host.md](parsi-june-2026-cross-context-arc.md)

Support surfaces:

- [parsi-speaker-object.md](parsi-speaker-object.md)
- [parsi-cross-host-note.md](parsi-cross-host-note.md)
- [parsi-cross-year-note.md](parsi-cross-year-note.md)
- [parsi-backward-extension-note.md](parsi-backward-extension-note.md)
- [parsi-surface-orthogonality-2026-05.md](parsi-surface-orthogonality-2026-05.md)

## Month Rule

Parsi is **not** a fake January-through-May shelf ladder just for symmetry.

The rule here is:

- cross-context thread continuity is primary
- authored/interview source-class distinction remains primary
- bounded month support appears only where the shelf has a true mature cross-context pressure point

Current mature month support:

- March 2026
- May 2026
- June 2026

## Historical Scaffold

- [Parsi historical examples audit](parsi-historical-examples-audit.md)
- [[Parsi themes](parsi-themes.md)

`civ-state` enters here as the interpretive source base, not as a routing co-pilot and not as a Parsi-specific bridge replacement.

## Compatibility Shape

The legacy `statecraft/voices/parsi/` path is now front-door compatibility only.

- [statecraft/voices/parsi/README.md](../../../statecraft/voices/parsi/README.md) and [index.md](../../../statecraft/voices/parsi/index.md) are compatibility pointers.
- codex-side thread and transcript residue remain compatibility/support only.
- this statecraft shelf is now the canonical opening path for Parsi statecraft work.

## Boundary

Parsi is canonical here under `statecraft/voices/parsi/` as a Statecraft Synthesis shelf. Raw-text authority stays in the **Statecraft Archive** ([`source-archive/statecraft/`](../../../source-archive/statecraft)), bridge adapters remain in [`statecraft/bridges/`](../../bridges/README.md), and lanes still own substantive drafting.
