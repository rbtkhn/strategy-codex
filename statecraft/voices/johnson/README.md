WORK only; not Record.

# Johnson

This is the canonical Johnson statecraft speaker shelf.

Use it for speaker identity, route maps, bounded 2026 monthly synthesis, dense-core strand retrieval, and `civ-state`-aware historical audit work.

## Open First

- [arc-johnson-continuity.md](arc-johnson-continuity.md)
- [johnson-routing.md](johnson-routing.md)
- [johnson-source-index.md](johnson-source-index.md)
- [johnson-helix.md](johnson-helix.md)
- [johnson-support-spine-2025-2026.md](johnson-support-spine-2025-2026.md)
- [stream/johnson-shelf-2026-03.md](johnson-shelf-2026-03.md)
- [stream/johnson-shelf-2026-04.md](johnson-shelf-2026-04.md)
- [stream/johnson-shelf-2026-05.md](johnson-shelf-2026-05.md)
- [stream/johnson-april-may-2026-arc-threads.md](johnson-april-may-2026-arc-threads.md)
- [johnson-historical-examples-audit.md](johnson-historical-examples-audit.md)
- [themes/README.md](README.md)

## Surface Grammar

- **Source surface name:** `johnson-source-index.md`
- **Source surface role:** source bench for breadth-first Johnson retrieval
- **Source surface interior:** materialized Johnson appearances and routeable appearance coverage
- **Recurring continuity surface name:** `stream/johnson-april-may-2026-arc-threads.md`
- **Recurring continuity surface role:** thread atlas for the dense April-May 2026 core
- **Recurring continuity surface interior:** recurring arc-threads braided by the April and May shelves and host-local arcs

## Canonical Structure

Johnson's canonical statecraft shelf now has six speaker-owned layers:

- **Person arc:** [arc-johnson-continuity.md](arc-johnson-continuity.md) holds whole-speaker continuity across hosts and time.
- **Routing surface:** [johnson-routing.md](johnson-routing.md) answers which host lane, month shelf, or dense-core strand surface to open first.
- **Source index:** [johnson-source-index.md](johnson-source-index.md) remains the canonical route map for materialized Johnson appearances.
- **Crossing surface:** [johnson-helix.md](johnson-helix.md) explains the stable host transformations inside the Johnson object.
- **Support spine:** [johnson-support-spine-2025-2026.md](johnson-support-spine-2025-2026.md) owns maturity law, month-status law, and the rule for how the bounded month ladder relates to host-owned chronology.
- **Bounded month ladder:** [stream/README.md](README.md) holds the canonical January through May 2026 synthesis shelves.

Additional canonical support surfaces:

- [johnson-cross-year-note.md](johnson-cross-year-note.md)
- [johnson-backward-extension-note.md](johnson-backward-extension-note.md)
- [johnson-surface-orthogonality-2026-05.md](johnson-surface-orthogonality-2026-05.md)
- [stream/johnson-april-may-2026-arc-threads.md](johnson-april-may-2026-arc-threads.md)

## Monthly Shelf Rule

Johnson now has a full canonical January through May 2026 month ladder for the live materialized run, and the monthly files are **bounded synthesis shelves**, not chronology owners.

- host-local arcs still own chronology
- the support spine still owns maturity law
- monthly shelves summarize what each month contributes to the Johnson object
- April-May dense-core strand retrieval is compressed further by the recurring-strand atlas, not replaced by it

## Historical Scaffold

- [Johnson historical examples audit](johnson-historical-examples-audit.md)
- [Johnson historical themes](README.md)

`civ-state` enters here as the primary interpretive source base, not as a routing co-pilot and not as a Johnson-specific bridge adapter.

## Compatibility Shape

The legacy `statecraft/voices/johnson/` path is now front-door compatibility only.

- [statecraft/voices/johnson/README.md](../../../statecraft/voices/johnson/README.md) and [index.md](../../../statecraft/voices/johnson/index.md) are compatibility pointers.
- codex-side thread, transcript, profile, page-template, and in-flight legacy notes remain compatibility residue or migration scaffolds.
- this statecraft shelf is now the canonical opening path for Johnson statecraft work.

## Host Lanes

- [Diesen x Johnson](../../notes/arc-johnson-diesen-host.md)
- [Davis host shelf](../../channels/daniel-davis/README.md)
- [Dialogue Works x Johnson](../../notes/arc-johnson-nima-host.md)
- [Napolitano host shelf](../../channels/judging-freedom/README.md)

## Boundary

Johnson is canonical here under `statecraft/voices/johnson/`. Raw-text authority stays in [`source-archive/statecraft/`](../../../source-archive/statecraft), bridge adapters still live under [`statecraft/bridges/`](../../bridges/README.md), and lanes still own substantive drafting.
