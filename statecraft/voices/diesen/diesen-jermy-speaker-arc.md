# Diesen x Jermy speaker arc

WORK only; not Record.

Purpose: compact reuse note for the `Diesen x Jermy` run inside the Diesen stream. This is not a substitute for the full raw-input shelf. It exists so future routing can quickly answer: what kind of Jermy lane Diesen elicits, which full-form episodes matter most, and how this branch differs from the denser Davis lane.

## Orthogonality guide

This file should be read as the **maritime-order and alliance-fracture** Jermy branch, not as the closure-geometry Davis branch and not as the public anti-hubris Peacemonger branch.

Quick separation rule:

- `Diesen x Jermy` = maritime-order fallout, NATO/U.S.-EU divorce, Europe-system breakdown, and geopolitical consequence
- [davis-jermy-speaker-arc.md](../../../statecraft/channels/daniel-davis/davis-jermy-speaker-arc.md = blockade mechanics, reopening lag, and shipping/energy propagation under feasibility pressure
- [peacemonger-jermy-speaker-arc.md](../../../statecraft/voices/jermy/peacemonger-jermy-speaker-arc.md = hubris, hypocrisy, and public indictment of failed strategy
- [jermy-speaker-object.md](../../../statecraft/voices/jermy/jermy-speaker-object.md = the higher-level triple-helix doctrine

If the notebook needs Jermy to widen maritime stress into civilizational, European, and alliance-system consequence, route here. If it needs operational closure logic or a sharper anti-elite indictment register, route to Davis or Peacemonger instead.

## Why this guest run matters

Steve Jermy matters here less as a generic navy voice than as a recurring **strategic-systemic, maritime-order, and Europe-breakdown** voice inside a host stream otherwise centered on civilizational order, multipolar transition, and institutional consequence.

Inside the Diesen stream, that makes the run especially useful when the notebook needs continuity between:

- NATO or Europe crisis and the maritime-strategic consequences of imperial separation
- Iran-war fallout and the wider global-system effects of U.S. decline
- Russia-war aims and Europe's own strategic exhaustion
- operational maritime stress and order-level political divorce

What makes this a real speaker arc is that the same host x guest shape keeps returning:

- Diesen frames the crisis through order transition, institutional fracture, and geopolitical consequence
- Jermy translates that crisis into maritime strategy, alliance stress, and system-level fallout
- the lane repeatedly turns military or geopolitical argument into a question of maritime order and civilizational separation

## Arc set

Filter note: this arc set excludes `Glenn Diesen Clips` and translated mirrors. It keeps only full-form English `Glenn Diesen` episodes as the canonical branch.

### Stream-discovered core

| Rank | Date | Title | Why it matters most |
|---|---|---|---|
| 1 | 2026-03-01 | *Iran War Endgame & the Global Fallout* | Best mature anchor. Strongest single file for why Jermy matters in this branch: Iran-war consequence widened into global maritime and order fallout. Open this first. |
| 2 | 2025-12-21 | *US-EU Conflict Is Coming - Time to Dissolve NATO* | Best alliance-fracture file. Strong on the branch's Europe-order and divorce logic. |
| 3 | 2025-11-27 | *Russia's War Objectives & Europe's Downfall* | Best Russia-Europe file. Important for seeing the branch before the 2026 Iran-war hardening. |

This is enough to treat `Diesen x Jermy` as a real recurring host-local branch rather than a stray reinforcement.

## Open first

If the operator needs only one file:

- open `2026-03-01` *Iran War Endgame & the Global Fallout*

If the operator needs the best paired read:

- open `2026-03-01` *Iran War Endgame & the Global Fallout*
- then `2025-12-21` *US-EU Conflict Is Coming - Time to Dissolve NATO*

That pair gives the clearest branch shape:

- maritime and global fallout
- Europe-order fracture
- alliance dissolution under strategic stress

## Routing use

Use this speaker arc when a Diesen ingest or batch pass is doing one or more of the following:

- widening maritime or Iran-war consequence into Europe-order and alliance consequence
- testing whether NATO and U.S.-EU strain should be read as strategic divorce rather than temporary policy dispute
- reading maritime stress at institutional and civilizational altitude
- comparing strategic-systemic fallout against Davis-side closure geometry

Typical pairings around the guest lane:

- `diesen`
- `sachs`
- `krainer`
- `crooke`

Distinct from:

- [davis-jermy-speaker-arc.md](../../../statecraft/channels/daniel-davis/davis-jermy-speaker-arc.md, which is stronger on closure geometry, reopening timelines, and immediate system knock-on
- [peacemonger-jermy-speaker-arc.md](../../../statecraft/voices/jermy/peacemonger-jermy-speaker-arc.md, which is stronger on public anti-hubris indictment than on order-transition consequence
- the legacy [jermy-thread.md](../../../statecraft/voices/jermy/jermy-thread.md, which is compatibility continuity rather than a competing canonical arc strand

## Boundaries

- Jermy is strongest here on strategic-systemic and alliance fallout, not on the detailed reopening geometry that dominates the Davis lane.
- This branch is clearly real, but less dense than `Davis x Jermy`.
- Prefer this guest run when the notebook needs maritime consequence widened into Europe-order or NATO-fracture language.
- Do not let this branch silently absorb the Davis closure-geometry lane or the Peacemonger anti-hubris lane.
