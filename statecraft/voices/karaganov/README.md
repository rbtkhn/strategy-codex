WORK only; not Record.

# Karaganov

This is the canonical Karaganov statecraft speaker shelf.

Use it for Russian elite strategic-doctrine routing: nuclear-threshold argument, deterrence restoration, Greater Eurasia / Siberization, post-European identity, and civilizational-security vocabulary fused with hard coercive warning.

Karaganov is intentionally a **lighter first-pass, single-helix shelf**. The direct corpus is Diesen-anchored; chronology still lives primarily in the host arc and archive captures.

## Open First

- [karaganov-profile.md](karaganov-profile.md)
- [karaganov-arc.md](karaganov-arc.md)
- [karaganov-routing.md](karaganov-routing.md)
- [karaganov-source-index.md](karaganov-source-index.md)
- [karaganov-helix.md](karaganov-helix.md)
- [karaganov-support-spine-2025-2026.md](karaganov-support-spine-2025-2026.md)

## Surface Grammar

- **Surface name:** `karaganov-source-index.md`
- **Surface role:** source bench for Diesen-anchored Karaganov retrieval
- **Surface interior:** direct Karaganov appearances plus labeled reaction surfaces

## Canonical Structure

Karaganov's canonical statecraft shelf currently has five speaker-owned layers:

- **Person arc:** [karaganov-arc.md](karaganov-arc.md) holds whole-speaker continuity for the Diesen doctrinal run.
- **Routing surface:** [karaganov-routing.md](karaganov-routing.md) answers which file or host lane to open first.
- **Source index:** [karaganov-source-index.md](karaganov-source-index.md) is the canonical route map for materialized appearances.
- **Single-helix note:** [karaganov-helix.md](karaganov-helix.md) explains why the object is Diesen-anchored rather than multi-host.
- **Support spine:** [karaganov-support-spine-2025-2026.md](karaganov-support-spine-2025-2026.md) owns maturity law and migration pressure.

## Current Shelf Shape

The real hierarchy on disk is:

- `Diesen x Karaganov` is the **only mature direct branch** — solo interviews plus one Mearsheimer triangulation panel.
- [2026-05-10 / Diesen x Karaganov solo](../../../source-archive/statecraft/2026-05-10/source-glen-diesen-sergey-karaganov-world-war-nuclear-doctrine-eurasia-2026-05-10.md) is the best single-file anchor.
- [2026-06-13 / Diesen x Mearsheimer x Karaganov](../../../source-archive/statecraft/2026-06-13/source-glenn-diesen-mearsheimer-karaganov-nuclear-strike-europe-restore-deterrence-2026-06-13.md) is the best paired-read with structural realism.
- 2025 Diesen interviews remain on legacy `codex/years/` paths until archive promotion.
- Ritter's "dark sage" read is a **reaction surface**, not a Karaganov appearance.

## Compatibility Shape

The legacy `codex/speakers/karaganov/` path is now front-door compatibility only.

- [codex/speakers/karaganov/README.md](../../../codex/speakers/karaganov/README.md) and [karaganov-speaker-object.md](../../../codex/speakers/karaganov/karaganov-speaker-object.md) are compatibility residue.
- this statecraft shelf is the canonical opening path for Karaganov statecraft work.

## Host Lanes

- [Diesen x Karaganov](../diesen/stream/diesen-karaganov-speaker-arc.md)

## Boundary

Karaganov is canonical here under `statecraft/voices/karaganov/`. Raw-text authority stays in [`source-archive/statecraft/`](../../../source-archive/statecraft/) (and legacy `codex/years/` until promoted). Do not treat references to "Karaganov doctrine" inside other speakers as direct Karaganov appearances. Do not equate his escalation doctrine with settled Kremlin policy without separate evidence.
