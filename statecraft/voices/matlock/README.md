# Jack Matlock

WORK only; not Record.

This folder is the compact speaker-object entry surface for Jack Matlock.

Open here when the question is not simply "which Diesen episode contains Matlock?" but how Matlock functions in the Codex as a U.S. diplomatic-memory witness: negotiated end of the Cold War, Reagan/Bush-era Soviet diplomacy, NATO expansion warnings, and the lost post-Cold War European security settlement.

## Start here

- [matlock-speaker-object.md](matlock-speaker-object.md)
- Diesen host arc: [arc-matlock-diesen-host.md](../../notes/arc-matlock-diesen-host.md)

## Compatibility shape

This shelf already has a real canonical structure:

- [matlock-speaker-object.md](matlock-speaker-object.md) defines the object
- [arc-matlock-diesen-host.md](../../notes/arc-matlock-diesen-host.md) is the mature host-local branch
- the direct appearance set below preserves the anchored source spine

Matlock is therefore best read as a **single-helix, Diesen-anchored diplomatic-memory object**, not as a generic profile stub and not as a mature multi-host helix.

## Direct appearance set

- [2026-04-19 Diesen x Matlock](../../../source-archive/statecraft/2026-04-19/source-diesen-matlock-how-nato-expansionism-broke-european-security-2026-04-19.md)
- [2025-03-27 Diesen x Matlock](../../../source-archive/statecraft/2025-03-27/day-index.md)
- [2025-12-10 Diesen x Matlock](../../../source-archive/statecraft/2025-12-10/day-index.md)
- [2025-06-21 Diesen x Matlock](../../../source-archive/statecraft/2025-06-21/day-index.md)
- [2025-09-01 Diesen x Matlock](../diesen/diesen-matlock-speaker-arc.md)

## Boundary

Matlock is currently `object_shape: single-helix`. The direct corpus is Diesen-anchored. Use him as a settlement-memory and diplomacy-architecture object, not as a live battlefield or force-employment analyst.
