WORK only; not Record.

# Wilkerson

This is the canonical Wilkerson statecraft speaker shelf.

Open here first when the question is not merely "what did Wilkerson say," but **which host lane, warning altitude, or continuity surface should carry the Wilkerson reading**.

## Shelf class

Wilkerson is a canonical `host-led mature-month exception shelf`.

That means:

- the speaker object is genuinely mature
- month support is real
- but chronology still belongs more cleanly to host arcs than to a native speaker-owned month ladder
- this `statecraft/voices` shelf is now the canonical front door, while heavier cross-year and month-support surfaces still live in the legacy shelf during migration

## Open First

- [wilkerson-arc.md](wilkerson-arc.md)
- [wilkerson-routing.md](wilkerson-routing.md)
- [wilkerson-helix.md](wilkerson-helix.md)
- [wilkerson-source-index.md](wilkerson-source-index.md)
- [legacy Wilkerson surface orthogonality](../../../statecraft/voices/wilkerson/wilkerson-surface-orthogonality-2026-05.md)

## Structural notes

- [wilkerson-cross-year-note.md](wilkerson-cross-year-note.md)
- [wilkerson-may-2026-note.md](wilkerson-may-2026-note.md)

## Host lanes

- [Dialogue Works x Wilkerson](../../../statecraft/channels/nima/stream/nima-wilkerson-speaker-arc.md)
- [Diesen x Wilkerson](../../../statecraft/voices/diesen/stream/diesen-wilkerson-speaker-arc.md)
- [Judging Freedom x Wilkerson](../../../statecraft/channels/napolitano/stream/napolitano-wilkerson-speaker-arc.md)

## Compatibility shape

This shelf is now the canonical statecraft-side opening path for Wilkerson.

- [statecraft/voices/wilkerson/README.md](../../../statecraft/voices/wilkerson/README.md) remains a compatibility and support doorway during migration
- the codex-side structural audit, helix, and raw-input bench remain linked support surfaces until a fuller transfer pass
- do not treat the codex shelf as a second competing authority tree; use it as linked support beneath this statecraft shelf

## Boundary

Wilkerson is canonical here under `statecraft/voices/wilkerson/`. Raw-text authority stays in `source-archive/statecraft/`, host-local chronology still belongs to host arcs, and heavier legacy support surfaces remain linked support until migrated.
