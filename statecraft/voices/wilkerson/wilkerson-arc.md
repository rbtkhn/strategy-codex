WORK only; not Record.

# Wilkerson arc

Purpose: define `wilkerson-arc` as the canonical statecraft-side person-level continuity surface for Larry Wilkerson across hosts, contexts, and time.

## Orthogonality guide

Quick separation rule:

- `wilkerson-arc` = Wilkerson's canonical continuity across hosts, contexts, and time
- [legacy Wilkerson helix](../../../statecraft/voices/wilkerson/wilkerson-helix.md) = cross-host comparison of the two main host transformations
- [wilkerson-routing.md](wilkerson-routing.md) = fastest host-selection surface
- host-local arcs = bounded realizations of Wilkerson under one host's pressure

## Arc shape

`wilkerson-arc` = a **cross-host person arc with a stable double-helix core and auxiliary reinforcement**.

Its core embodied lanes are:

1. [Diesen x Wilkerson](../../../statecraft/voices/diesen/stream/diesen-wilkerson-speaker-arc.md) - imperial-decline altitude and systemic consequence
2. [Dialogue Works x Wilkerson](../../../statecraft/hosts/nima/stream/nima-wilkerson-speaker-arc.md) - live crisis pressure and military-statecraft warning

Auxiliary reinforcement:

- [Napolitano x Wilkerson](../../../statecraft/hosts/napolitano/stream/napolitano-wilkerson-speaker-arc.md)

## What makes this an arc

Wilkerson has enough cross-year recurrence that the notebook keeps reusing the same underlying voice for several stable jobs:

- imperial-decline warning
- military-statecraft realism under elite delusion
- live blockade, Hormuz, and escalation impossibility
- legitimacy-collapse reading of U.S. strategic self-harm

The strongest current motion is:

`early-2025 warning voice -> late-2025 double-helix thickening -> 2026 blockade impossibility and legitimacy-collapse continuity`

That motion is broad enough that Wilkerson deserves a canonical statecraft-side person arc even though chronology still routes primarily through host shelves and bounded support notes.

## Open first

If the operator needs the strongest whole-speaker comparative entry:

- open [legacy Wilkerson helix](../../../statecraft/voices/wilkerson/wilkerson-helix.md)

If the operator needs the fastest host-conditioned entry:

- open [wilkerson-routing.md](wilkerson-routing.md)

If the operator needs the cleanest structural anchor:

- open [Diesen x Wilkerson](../../../statecraft/voices/diesen/stream/diesen-wilkerson-speaker-arc.md)

## Boundary

- Do not let either Diesen or Dialogue Works stand in for the whole Wilkerson continuity.
- Do not quietly promote auxiliary reinforcement into a third canonical strand unless it thickens materially.
- Open raw-input before quoting or making claim-grade factual use.
