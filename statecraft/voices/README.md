...
**Agent dispatch:** For analyst/source-index routing, open [`statecraft/voices/voice-index.md`](voice-index.md) — canonical voices router front door (not other repo `INDEX.md` hubs). **Audit:** `python scripts/audit_statecraft_archive_index.py --voice-index` — skill **`audit index`**. **Author/guest shelf:** `{slug}-index.md` + `--shelf-index SLUG`; post-land via `shelf_index_from_capture.py` — [speaker-shelf-vocabulary.md](speaker-shelf-vocabulary.md). For repo-wide LLM routing, open [LLM-ROUTING.md](../../LLM-ROUTING.md).
...
Current migrated profiles: [Barnes](barnes/barnes-profile.md), [Mercouris](mercouris/mercouris-profile.md), [Mearsheimer](mearsheimer/mearsheimer-profile.md), [Pape](pape/pape-profile.md), [Crooke](crooke/crooke-profile.md), [Ritter](ritter/ritter-profile.md), [Parsi](parsi/parsi-profile.md), [Diesen](diesen/diesen-profile.md), [Davis](davis/davis-profile.md), [Alkorshid](alkorshid/alkorshid-profile.md), [Weichert](weichert/weichert-profile.md) (seed). Profile-only lanes: [codex/profiles/README.md](../../README.md).
