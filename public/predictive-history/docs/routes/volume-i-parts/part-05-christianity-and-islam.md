# Part V — Christianity and Islam

*This Part is a **spine slice** of Volume I — Civilization. It follows [interwoven-reader](../../archive/two-volume-reader-order-interwoven.md) order; it is not a pure historical period box.*

## Law-discovery question

How does sacred legitimacy migrate across revelations and empires?

## Chapter anchors

| Chapter | Lecture |
|---------|---------|
| `civ-24` | Resurrecting the Gnostic Jesus |
| `civ-25` | Paul of Tarsus, Messiah of Rome |
| `civ-26` | Constantine's Monotheistic Revolution |
| `civ-27` | Augustine's Empire of God |
| `civ-28` | Muhammad's Revolution of God |

## Companion weave

- `sh-18` @ `civ-28` (deepening) — [sh-18](../secret-history-support/sh-18/README.md)

## Corridor cross-links

_No corridor touchpoints in this Part._

## Apparatus

- [Part V commentary](part-05-christianity-and-islam-commentary.md) — cross-chapter synthesis, `sh-18` pointer, prediction ledger
- [Part V bibliography](part-05-christianity-and-islam-bibliography.md) — external sources (tagged by chapter; interfaith guardrails)
- [Part V hybrid readiness](PART-05-HYBRID-READINESS.md) — hybrid pilot complete through Phase 3
- **Ingress:** [Part IV `civ-23`](./part-04-ancient-foundations-commentary.md#civ-23) Zoroastrian/Jewish merge → `civ-24`

## Navigation

- **Previous:** [Part IV — The Ancient Foundations](part-04-ancient-foundations.md)
- **Next:** [Part VI — The Medieval Imagination](part-06-medieval-imagination.md)
