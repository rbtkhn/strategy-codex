# Part I — The Dawn of Civilization

*This Part is a **spine slice** of Volume I — Civilization. It follows [interwoven-reader](../../archive/two-volume-reader-order-interwoven.md) order; it is not a pure historical period box.*

## Law-discovery question

How do form, religion, and destruction precede empire?

## Chapter anchors

| Chapter | Lecture |
|---------|---------|
| `civ-01` | Explaining Humanity's Transition to Agriculture |
| `civ-02` | Religion and the Dawn of Society |
| `civ-03` | The Religious Imagination |
| `civ-04` | The Paradise Lost of Marija Gimbutas |
| `civ-05` | The Yamnaya Conquest of Europe |
| `civ-06` | Elite Overproduction and the Bronze Age Collapse |

## Companion weave

- `gb-01` @ `civ-01` (interwoven) — [gb-01](../great-books-evidence/gb-01/README.md)
- `gb-03` @ `civ-02` (interwoven) — [gb-03](../great-books-evidence/gb-03/README.md)
- `gb-04` @ `civ-03` (interwoven) — [gb-04](../great-books-evidence/gb-04/README.md)
- `sh-11` @ `civ-01` (prologue) — [sh-11](../secret-history-support/sh-11/README.md)

## Corridor cross-links

_No corridor touchpoints in this Part._

## Apparatus

- [Part I commentary](part-01-dawn-of-civilization-commentary.md) — **Phase 3 complete** (`civ-01`–`06` pin-cited; gb/sh pointers; ledger closed)
- [Part I bibliography](part-01-dawn-of-civilization-bibliography.md) — `supports:`/`counters:` for `civ-01`–`06`
- [Part I hybrid readiness](PART-01-HYBRID-READINESS.md) — inventory + rollout phases
- Cross-Part egress: [Part II `civ-07`](part-02-hellenic-world-commentary.md#civ-07)

## Navigation

- _Start of Volume I._
- **Next:** [Part II — The Hellenic World](part-02-hellenic-world.md)
