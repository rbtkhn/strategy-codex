# Provenance

This folder names the old source-family scaffold as provenance. In the staged pass, the old `book/volume-*` folders still carry the underlying packets, but they no longer define the intended public reading order.

- old Volume I: `book/volume-i/` for Geo-Strategy residue
- old Volume II: `book/volume-ii/` for Civilization residue
- old Volume III: `book/volume-iii/` for Game Theory residue
- old Volume V: `book/volume-v/` for Great Books residue
- old Volume VI: `book/volume-vi/` for Secret History residue
- old Volume VII: `book/volume-vii/` for Essays residue
- [From The Old Seven Volumes To The Current Two](../seven-volume-to-two-volume.md)

## Interviews (public provenance packets)

Fifteen workshop interviews (`vi-01` … `vi-15`) plus one external Diary of a CEO packet are promoted to repo-root [`interviews/`](../../interviews/README.md) with date-slug public IDs (`interview-YYYY-MM-DD-{host-slug}`). Catalog: [`docs/predictive-history-index.md`](../../docs/predictive-history-index.md) · **Provenance** section.

Doorways:

- [`interview-2025-10-30-cyrus-janssen`](../../interviews/interview-2025-10-30-cyrus-janssen/README.md)
- [`interview-2025-11-24-glenn-diesen`](../../interviews/interview-2025-11-24-glenn-diesen/README.md)
- [`interview-2026-01-05-glenn-diesen`](../../interviews/interview-2026-01-05-glenn-diesen/README.md)
- [`interview-2026-01-18-danny-haiphong`](../../interviews/interview-2026-01-18-danny-haiphong/README.md)
- [`interview-2026-01-22-dimitri-lascaris`](../../interviews/interview-2026-01-22-dimitri-lascaris/README.md)
- [`interview-2026-01-26-glenn-diesen`](../../interviews/interview-2026-01-26-glenn-diesen/README.md)
- [`interview-2026-03-02-breaking-points`](../../interviews/interview-2026-03-02-breaking-points/README.md)
- [`interview-2026-03-07-nima`](../../interviews/interview-2026-03-07-nima/README.md)
- [`interview-2026-03-09-sneako`](../../interviews/interview-2026-03-09-sneako/README.md)
- [`interview-2026-03-16-endgame`](../../interviews/interview-2026-03-16-endgame/README.md)
- [`interview-2026-03-20-tucker-carlson`](../../interviews/interview-2026-03-20-tucker-carlson/README.md)
- [`interview-2026-04-01-jay-shapiro`](../../interviews/interview-2026-04-01-jay-shapiro/README.md)
- [`interview-2026-04-07-patrick-bet-david`](../../interviews/interview-2026-04-07-patrick-bet-david/README.md)
- [`interview-2026-04-13-glenn-diesen`](../../interviews/interview-2026-04-13-glenn-diesen/README.md)
- [`interview-2026-04-13-sneako-dugin`](../../interviews/interview-2026-04-13-sneako-dugin/README.md)
- [`interview-2026-05-07-diary-of-a-ceo`](../../interviews/interview-2026-05-07-diary-of-a-ceo/README.md)
