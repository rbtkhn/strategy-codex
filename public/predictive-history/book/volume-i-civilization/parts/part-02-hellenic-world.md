# Part II — The Hellenic World

*This Part is a **spine slice** of Volume I — Civilization. It follows [interwoven-reader](../interwoven-reader/README.md) order; it is not a pure historical period box.*

## Law-discovery question

How does epic → institution → theater → philosophy → conquest-carrier build a civilizational grammar?

## Chapter anchors

| Chapter | Lecture |
|---------|---------|
| `civ-07` | Homer's Iliad and the Birth of Greek Civilization |
| `civ-08` | Rat Utopia and the Peloponnesian War |
| `civ-09` | Aeschylus, Sophocles, and Euripides as Prophets of Democracy |
| `civ-10` | The Trial of Socrates and Plato's Allegory of the Cave |
| `civ-11` | The Greatness of Philip II of Macedon |
| `civ-12` | The Tyranny of Alexander the Great |
| `civ-13` | Aristotle and the Greek Legacy |

## Companion weave

- `gb-02` @ `civ-07` (interwoven) — [gb-02](../great-books-evidence/gb-02/README.md)
- `gb-05` @ `civ-07` (interwoven) — [gb-05](../great-books-evidence/gb-05/README.md)
- `gb-07` @ `civ-07` (interwoven) — [gb-07](../great-books-evidence/gb-07/README.md)
- `gb-03` @ `civ-07` (support_ring_optional) — [gb-03](../great-books-evidence/gb-03/README.md) — anchor civ-02 in Part I

## Corridor cross-links

- [homer-to-tolstoy](../../../data/corridors/homer-to-tolstoy.md)
- [homer-to-tolstoy-support-ring](../../../data/corridors/homer-to-tolstoy-support-ring.md)
- [homer-to-dante](../../../data/corridors/homer-to-dante.md)

## Apparatus

- [Part II commentary](part-02-hellenic-world-commentary.md) — cross-chapter synthesis, Great Books sections, prediction ledger
- [Part II bibliography](part-02-hellenic-world-bibliography.md) — external sources (tagged by chapter/gb)
- Cross-Part ingress: [Part I commentary](part-01-dawn-of-civilization-commentary.md) (`civ-01`–`06` origin arc; `gb-03` dual-placement @ `civ-02`)

## Navigation

- **Previous:** [Part I — The Dawn of Civilization](part-01-dawn-of-civilization.md) · [Part I apparatus](part-01-dawn-of-civilization-commentary.md)
- **Next:** [Part III — The Roman Imperium](part-03-roman-imperium.md)
