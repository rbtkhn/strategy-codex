# Rome — Table of Contents

Human-readable navigation for **Civilizational Statecraft**, Volume III. This file is a **reader map** — not a file inventory and not the machine registry.

**Thematic entry:** [Pax Romana](introduction.md) · **Volume spine (era blocks, diagnostic):** [README.md](README.md)

**Authoritative detail elsewhere:** full essay-rome lattice → [`essay-rome.registry.yaml`](essays/essay-rome.registry.yaml) · term membrane → [Connectivity Rome](essays/connectivity-rome.md) · essays hub → [essays/README.md](essays/README.md)

---

## How to choose a path

| If you need… | Start here |
|--------------|------------|
| Whole-volume constitutional read | [Path A — Constitutional](#path-a--constitutional) |
| Republic → principate as standalone civic essays | [Path B — Civic chain](#path-b--civic-chain-essay-rome) |
| One friction node × six governing terms | [Path C — hex-frame](#path-c--hex-frame) |
| Chain placement or spine rosters | [rome-memory](rome-memory.md) |
| Sources to retrieve or cite | [Sources](#sources) |

---

## Path A — Constitutional

Read in order when the volume itself is the object:

1. [Pax Romana](introduction.md) — volume door
2. [Civilization Rome](essays/civilization-rome.md) — Part 1, civilizational form
3. [Empire Rome](essays/empire-rome.md) — Part 2, outward machinery

Then open **optional sub-lenses** only when one dimension needs essay-length depth ([essays README](essays/README.md)):

- [Dual language heritage](essays/dual-language-heritage-rome.md)
- [Military history](essays/military-history-rome.md)
- [Faith history](essays/faith-history-rome.md)
- [Science history](essays/science-history-rome.md)
- [Geo-strategy](essays/geo-strategy-rome.md) · [Secret-history](essays/secret-history-rome.md) · [Game-theory](essays/game-theory-rome.md)

---

## Path B — Civic chain (essay-rome)

Standalone lattice essays — **no global read order** except this narrative chain when continuity helps:

```text
genesis → republic → caesar → augustus
```

| Essay | Period (approx.) | Role |
|-------|------------------|------|
| [Genesis](essays/essay-rome-genesis.md) | foundation through early Republic (~753–390 BC) | formation proof |
| [Republic](essays/essay-rome-republic.md) | Italian expansion through late Republic (~390–49 BC) | scale proof |
| [Caesar](essays/essay-rome-caesar.md) | 49–44 BC | command hinge |
| [Augustus](essays/essay-rome-augustus.md) | 44 BC–AD 14 | institutionalizer |

Each active node has a `.meta.yaml` sidecar (scope, deferrals, return path). **Planned lattice nodes** (carthage, constantinople, schism, and others — 17 planned): see [`essay-rome.registry.yaml`](essays/essay-rome.registry.yaml).

**hex-frame peer** for this chain: [Republic → principate](essays/hexagonal-republic-principate-rome.md) — six-term read of the same hinge, not a substitute for the civic essays.

---

## Path C — hex-frame

One friction node read through all six `rome-{term}.md` lenses. Template: [_template-hexagonal-rome.md](essays/_template-hexagonal-rome.md).

| Node | File |
|------|------|
| 1453 East terminus | [hexagonal-1453-rome.md](essays/hexagonal-1453-rome.md) |
| Italian Wars / Sack 1527 | [hexagonal-italian-wars-rome.md](essays/hexagonal-italian-wars-rome.md) |
| Justinian codification (529) | [hexagonal-justinian-codification-rome.md](essays/hexagonal-justinian-codification-rome.md) |
| 1054 Great Schism | [hexagonal-1054-schism-rome.md](essays/hexagonal-1054-schism-rome.md) |
| Republic → principate | [hexagonal-republic-principate-rome.md](essays/hexagonal-republic-principate-rome.md) |
| Colonial procedural tail | [hexagonal-colonial-tail-rome.md](essays/hexagonal-colonial-tail-rome.md) |

---

## Term depth (volume root)

Six whole-work term projections — link rosters here; do not duplicate in essays:

- [rome-civilization](rome-civilization.md) · [rome-empire](rome-empire.md) · [rome-entropy](rome-entropy.md)
- [rome-faith](rome-faith.md) · [rome-science](rome-science.md) · [rome-memory](rome-memory.md)

**Spine index** on memory: [chain](rome-memory.md#chain-spine) · [language](rome-memory.md#language-spine) · [military](rome-memory.md#military-spine) · [faith](rome-memory.md#faith-spine) · [science](rome-memory.md#science-spine)

**Cross-term routing:** [Connectivity Rome](essays/connectivity-rome.md)

---

## Sources

| Shelf | File | Use |
|-------|------|-----|
| Retrieve (upstream corpus) | [source-shelf.md](source-shelf.md) → [`sources/rome/bibliography.md`](../../sources/rome/bibliography.md) | Era doors, full retrieve |
| Essay citation inventory (PD URLs) | [rome-bibliography.md](rome-bibliography.md) | Active essay-rome nodes only — append on ship |
| Navigation apparatus | [shelf-reader.md](shelf-reader.md) | How to move through Rome sources |

---

## Return path

- [Rome volume README](README.md)
- [Book TOC](../../docs/table-of-contents.md#volume-iii-rome)
- [Volume map](../README.md)
- [Theory shelf](../../theory/README.md)
