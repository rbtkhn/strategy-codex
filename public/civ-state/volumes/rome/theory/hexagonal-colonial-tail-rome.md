# Hexagonal colonial procedural tail (Rome)

## Where This Sits

**Hexagonal demonstrator** — one friction node read through all six `rome-{term}.md` lenses. Routing SSOT: [Connectivity Rome](../essays/connectivity-rome.md). Machine law: [essay types — do not conflate](../essays/connectivity-rome.md#essay-types-do-not-conflate). Do **not** duplicate term-file rosters; link segment anchors only.

**Not:** constitutional continuity ([Civilization Rome](../essays/civilization-rome.md) · [Empire Rome](../essays/empire-rome.md)) · single-dimension depth ([history essays](../essays/README.md#optional-sub-lenses)).

## Reading Posture

Ask six governing questions in order — **memory placement first**, then the lens that owns the seam:

1. **Memory** — where on the spine?
2. **Civilization** — what inheritance is at stake?
3. **Empire** — what outward instrument carries this?
4. **Faith** — what sacred truth-order authorizes?
5. **Science** — what procedural truth-order encodes?
6. **Entropy** — which shock broke carry (if any)?

**Membrane laws:** Empire overreach ≠ entropy · ethic (civ) ≠ institution (emp) · rupture-primary on entropy except **1054** on faith · **`contested-weight` mandatory** on colonial rows.

**Archetype:** **F — Instrument tail / wind-down** *(wave 2 complete)* — Iberian **instrument** terminates; carry **disperses**; not a new chain head.

## Friction node

| Field | Value |
|-------|-------|
| **Node** | **Colonial procedural tail** (Iberian instrument → Latin-American partial inheritance → decolonization wind-down) |
| **Era** | Colonial → Industrial → Cybernetic |
| **One-line stake** | Spanish/Portuguese colonial **instruments** export civil-law and Catholic grammar overseas; decolonization winds down imperial instruments while **partial inheritance** disperses to Latin America — **Papacy remains** present chain head. |

## Six-lens read

| Lens | Governing question | This node — owns | Defers to | Term anchor |
|------|-------------------|------------------|-----------|-------------|
| **Memory** | Where on the spine? | **Instrument** + **partial-inheritance** rows — 1492 Atlantic · Iberian Union · Latin-American civil-law dispersion; oceanic-colonial index (`contested-weight`) | Full chain-spine political head | [chain-spine](rome-memory.md#chain-spine) · [language-spine](rome-memory.md#language-spine) · [science-spine — western](rome-memory.md#science-spine-western) |
| **Civilization** | What inheritance is at stake? | **Partial inheritance** — Latin-American civil-law + Catholic dispersion; **not** chain head; geographic Latin-America ≠ classical Latin revival | Ethic rosters | [sovereignty chain — civilization](rome-civilization.md#sovereignty-chain-civilization-lens) |
| **Empire** | What outward instrument? | **Owns instrument chronicle** — 1492 Castilian Atlantic · Portuguese discovery · Charles V parallel-habsburg · colonial instrument merge — **`instrument` not co-primary** | Peninsular branch roster | [sovereignty chain — empire](rome-empire.md#sovereignty-chain-empire-lens) · [military history — empire](rome-empire.md#military-history-empire-lens) |
| **Faith** | What sacred truth-order? | **Colonial mission tail** — patronato · Jesuit/Franciscan mission instrument; **`contested-weight` mandatory** | Papal present-carrier | [faith history — faith](rome-faith.md#faith-history-faith-lens) · [faith history — entropy](rome-entropy.md#faith-history-entropy-lens) |
| **Science** | What procedural truth-order? | **Colonial codification export** — Siete Partidas / Ordenações · Indies transplantation · Latin-American civil-law **carry** (dispersion, not live head beside Papacy) | `#science-spine` western tail | [science history — science](rome-science.md#science-history-science-lens) · [science-spine western](rome-memory.md#science-spine-western) |
| **Entropy** | Which shock broke carry? | **Instrument wind-down** — colonial overreach / decolonization; carry disperses to Latin America; **`contested-weight`** parallel to science tail (M5) — **not** 476-class universal rupture | Full rupture-primary rows | [sovereignty chain — entropy](rome-entropy.md#sovereignty-chain-entropy-lens) · [science history — entropy](rome-entropy.md#science-history-entropy-lens) |

## Cross-ref law (this node)

- Do **not** duplicate Iberian branch tables, mission rosters, or full science-spine western export prose.
- **Not chain heads:** Spain, Portugal, colonial empires = **`instrument`** of branches — present political head = **Papacy only**.
- **Latin-America ≠ classical Latin** — partial inheritance / dispersion, not revival of republican chain opener.
- **Shared `contested-weight`:** colonial history mainstream; operator Roman framing — mandatory on faith mission tail, science export, memory oceanic-colonial index.
- **Empire overreach ≠ entropy (delineation node 4):** overreach thins inheritance while machinery expands — **this node** is **wind-down** (node 6), not expansion phase.
- **Choreography:** [Delineation — Civilization ↔ Empire ↔ Entropy](../essays/civilization-rome.md#delineation--civilization--empire--entropy) node **6** · contrast [Republic→principate](hexagonal-republic-principate-rome.md) (transformation) and [1453](hexagonal-1453-rome.md) (bridge rupture).

## Short synthesis

The **colonial procedural tail** shows Rome's grammar **exporting** through **instruments** that are explicitly **not** sovereign chain heads. **Empire** names the maritime-colonial machinery — Castile/Portugal Atlantic openers, personal unions, codified export — while **civilization** names what **disperses** as **partial inheritance** in Latin America.

**Science** owns the **procedural** story: civil-law transplantation, Siete Partidas lineage, post-independence carry without a second papal-class head. **Faith** authorizes through **mission instruments** under **`contested-weight`** — sacred grammar travels, but colonial history cannot be naively Romanized. **Entropy** reads **wind-down**: instruments terminate, carry scatters — parallel to science's contested export tail, **not** a replacement for **476** or **1453** rupture-primary rows.

**Memory** indexes without elevating colonial branches to chain head. The membrane lesson: **procedural and sacred carry can outlive the instrument** that carried them — dispersion is not restoration.

## Return Path

- [Connectivity Rome](../essays/connectivity-rome.md) · [Essays index](../essays/README.md)
- [Hexagonal demonstrators registry](../theory/README.md#hex-frame-hexagonal-demonstrators)
- Wave 1: [1453](hexagonal-1453-rome.md) · [Italian Wars / Sack](hexagonal-italian-wars-rome.md) · [Justinian 529](hexagonal-justinian-codification-rome.md)
- Wave 2: [1054](hexagonal-1054-schism-rome.md) · [Republic→principate](hexagonal-republic-principate-rome.md)
- Five spines: [chain](rome-memory.md#chain-spine) · [language](rome-memory.md#language-spine) · [military](rome-memory.md#military-spine) · [faith](rome-memory.md#faith-spine) · [science](rome-memory.md#science-spine)
- Constitutional essays: [civilization](../essays/civilization-rome.md) · [empire](../essays/empire-rome.md)
