...
**Singularity loop signals:** On successful apply, `auto_dream.py` refreshes `runtime/artifacts/singularity-signals.json` from the loop registry so tomorrow's coffee picks up current orchestrator state. When non-empty, the dream summary may include one **`Singularity loops — …`** line. See [docs/singularity/loop-system.md](../../../docs/singularity/loop-system.md).
...
**Optional — fold learning ledger:** After **EOD compose**, the operator may append one line with [`scripts/log_strategy_fold.py`](../../../scripts/log_strategy_fold.py) and run [`scripts/report_strategy_fold_learning.py`](../../../scripts/report_strategy_fold_learning.py) — see [FOLD-LEARNING.md](../../../continuity/FOLD-LEARNING.md). **Not** required for `dream` to complete.

**Optional — thread touch on compose:** When composing inbox into **`days.md`**, if lines contain **`thread:<id>`**, the agent **may** add **one bullet** under **`### Foresight`** (from [strategy-commentator-threads.md](../../../continuity/strategy-commentator-threads.md)) **only when the operator directed that compose** — not by default at `dream`.
