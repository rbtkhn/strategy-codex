...
When the operator asks for the daily main-stream update from Coffee **C** (Davis, Diesen, Alkorshid/Dialogue Works, Napolitano/Judging Freedom, Mercouris), route **C-A / Intel** through the **`check streams`** skill. Treat `check streams` as the list-first daily roster check and hand approved URLs down to the lower-layer YouTube transcript workflow only after selection. `cognition streams` remains a legacy alias.
