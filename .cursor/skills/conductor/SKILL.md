...
| Do | Don't |
|----|--------|
| **Conductor session** (outside **`coffee`** **Step 2** **hub** **menu** **lines**): standalone = no `operator_coffee.py` / no **Recent rhythm**; direct Conductor activation opens by conductor name or `conductor <name>`; bare `conductor` asks for a name. — [coffee § Conductor session](../coffee/SKILL.md#conductor-only-no-coffee) | Re-spec full **COFFEE-CADENCE** **movements** — [protocol](../../../docs/skill-work/work-strategy/strategy-notebook/COFFEE-CADENCE-CONDUCTOR-PROTOCOL.md) is SSOT. |
...
| **Close (optional):** [CONDUCTOR-IMPROVEMENT-LOOP](../../../codex/CONDUCTOR-IMPROVEMENT-LOOP.md) · [CONDUCTOR-CLOSE-TEMPLATE](../../../codex/CONDUCTOR-CLOSE-TEMPLATE.md) · `coffee_conductor_outcome` with `conductor=<slug>` + `verdict=` + `notebook_ref=` or `falsify=` | Treat log lines as full memory. |
...
| Slug (anchor) | Shape hints (each of the 3 should reflect **this** row — mix as appropriate) |
|---------------|-----------------------------------------------------------------------------|
| **toscanini** | Verify the seam, pin the receipt, and force the claim back to the evidence; when facts are stale, offer one exact-date `last30days` scan. |
