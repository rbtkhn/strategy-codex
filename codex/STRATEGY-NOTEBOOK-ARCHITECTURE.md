...
**Two-layer SSOT (capture vs notebook work):** **[`source-archive/statecraft/`](../source-archive/statecraft/README.md)** holds the **literal** author/host/guest text (archived capture). **codex-pages** in the owning **`codex/<year>/<channel>/`** folder per [`strategy-codex-template-page.md`](strategy-codex-template-page.md) are the **default handle** for **strategy-page**, **`thread`**, and **`days.md`** ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â lane, sibling links, and **`### Verbatim`** on the page. **Disputes on wording** ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ correct the source archive and the pageÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢s **`### Verbatim`** together. **Disputes on judgment** ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ **`### Reflection` / `### Predictive Outlook`** and downstream compose, not a second paraphrase of the raw in **`thread`**.
...
**Rule of thumb:** **Inbox + `raw-input/`** = capture; **pages** (marker-fenced blocks in time-scoped author thread files under `codex/years/2026/<channel>/`) = judgment after the **end-of-day strategy session**; **`days.md`** = chronology and continuity.
...
**Purpose:** Fit a long author transcript into the **ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â°Ãƒâ€šÃ‚Â¤ ~2000 words per ingest** target for the time-scoped transcript file under `codex/years/2026/<channel>/` **without changing any of the authorÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢s words** ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â by **dropping whole sentences only**, selected for **argument weight**.
...
| **Strategy-page** | `<!-- strategy-page:start` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¦ `<!-- strategy-page:end -->` in time-scoped thread files under `codex/years/2026/<channel>/` | **Author-thread judgment** unit (primary written unit for **EOD compose**); **many** per month are normal; multi-author sessions reuse the same **`id=`** across files. |
...
**Per-author rolling mirror:** Ingest lines that carry **`thread:<expert_id>`** are automatically triaged into the time-scoped transcript file under `codex/years/2026/<channel>/` and extracted for thread distillation into the matching thread file via **`python3 scripts/strategy_thread.py`** (operator **`thread`**); crossing rules and optional `verify:` tails stay in [strategy-commentator-threads.md](strategy-commentator-threads.md).
...
- **Parallel PH overlay:** Keep in sync with [`codex/predictive-history/operator-polyphony.md`](predictive-history/operator-polyphony.md) (same section structure); see **Parallel to Predictive History** table above.
