...
| **Inbox vs notebook** | **Raw paste and URL dumps** → [daily-strategy-inbox.md](daily-strategy-inbox.md); **full verbatim** → [`raw-input/`](raw-input/README.md). **Notebook** (`days.md` + **`strategy-page`** blocks in author **`thread.md`**) = **synthesis + key links**, not mirrors of the inbox or **`raw-input/`**. |
| **Strategy input (ingest contract)** | **Strategy input** (explicit **or** clear from context: pasted essay/excerpt + URL in work-strategy) → **must** create **`raw-input/<pub_date>/<slug>.md`** per [raw-input/README](raw-input/README.md) **in the same turn** as synthesis (verbatim SSOT first). Optional inbox stub. Cursor: [`.cursor/rules/strategy-input-raw-ingest.mdc`](../../../../.cursor/rules/strategy-input-raw-ingest.mdc). |
| **Skill-strategy capture order** | On a **`strategy`** pass, **`raw-input/`** verbatim **before** optional inbox when the ingest contract applies — [skill-strategy § Page-first write contract](../../../../.cursor/skills/skill-strategy/SKILL.md#page-first-write-contract). **Three capture channels** (norm vs recovery): [raw-input/README § Three capture channels](raw-input/README.md#three-capture-channels-normative-vs-recovery). Optional advisory heuristic: [`scripts/strategy_raw_input_gap_hint.py`](../../../../scripts/strategy_raw_input_gap_hint.py). |
