# Zangezur Transit / Sovereignty Framework

This surface is non-authoritative and subject to revision.

Purpose: compare one corridor crisis across the America, Russia, China, and Iran lanes. The framework tests how transit access can be separated from territorial severance, extraterritorial control, and humiliating sovereignty loss.

## Crisis Object

Zangezur is a corridor-class problem: access, inspection, customs, security, and mediation can be drafted as transit rules, but the same words can become territorial severance if they imply extraterritorial authority.

## Lane Package

| lane | transaction role | open |
| --- | --- | --- |
| America | access guarantee without appearing to impose corridor sovereignty | [america.md](america.md) |
| Russia | security-depth and mediation role without overclaiming control | [russia.md](russia.md) |
| China | infrastructure continuity without owning a border dispute | [china.md](china.md) |
| Iran | sovereignty firewall against extraterritorial corridor logic | [iran.md](iran.md) |

## Shared Source Frame

- [Crisis test casebook](../../sheets/crisis-test-casebook.md) for the corridor classification rule.
- [Civilization and Empire cycle](../../sheets/civilization-empire-cycle.md) for amplification and entropy checks.
- Relevant lane helix files for successor-stable interests.

## Comparison

Use [comparison.md](comparison.md) to test which wording preserves transit access while preventing sovereignty collapse.

## Validity

Status: `draft`. The framework is a classification and settlement scaffold until paired with current source packets.
