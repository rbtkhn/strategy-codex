...
| Diesen guest-stream ledger input | `scripts/update_diesen_ledger.py` | n/a | Rebuilds the Mearsheimer / Sachs / Ritter / Macgregor / Daniel Davis / Max Blumenthal ledger tables in `codex/diesen/diesen-profile.md` from URL batches; canonicalizes watch URLs, fetches exact metadata, dedupes by video ID, and refreshes mirror status from `raw-input/`. |
