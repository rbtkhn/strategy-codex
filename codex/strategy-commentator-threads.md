...
| `ritter` | Scott Ritter | U.S. **military dissent**: Hormuz **sea control**, blockade ops, Vance frame; **faith-politics** register when **Ritter** is the speaking expert. Guest-canon inside Diesen stream: [diesen-ritter-canon-note.md](2026/diesen/diesen-ritter-canon-note.md) | `JDVance`, `IRAN`, or `Ritter` | × `marandi`, × `barnes`, × `rome-invective` (split from ecumenical) |
...
| `macgregor` | Douglas Macgregor (`@DougAMacgregor`) | Importers / **Asia–Europe** distance from U.S.–Israel kinetic frame. Guest-canon inside Diesen stream: [diesen-macgregor-canon-note.md](2026/diesen/diesen-macgregor-canon-note.md) | `IRAN` or Macgregor in cold | × `pape`, × `mearsheimer`, × `parsi` |
...
| `mearsheimer` | John Mearsheimer | **Offensive realism**: security dilemma, Israel structural, great-power geometry. Guest-canon inside Diesen stream: [diesen-mearsheimer-canon-note.md](2026/diesen/diesen-mearsheimer-canon-note.md) | `MEARSHEIMER` or `Mearsheimer` in cold | × `davis`, × `mercouris`, × `diesen`, × `sachs` |
...
| `matlock` | Jack Matlock | U.S. **diplomatic memory / Cold War settlement** lane: Reagan/Bush-era negotiation memory, anti-NATO-expansion warning lineage, and the later failure of the post-Cold War European security architecture. Guest-canon inside Diesen stream: [diesen-matlock-canon-note.md](2026/diesen/diesen-matlock-canon-note.md) | `Matlock`, `Cold War`, `NATO`, or `Ukraine` in cold | × `diesen`, × `mearsheimer`, × `sachs`, × `crooke` |
...
| `karaganov` | Sergey Karaganov | Russian elite **strategic / civilizational-state** voice: **deterrence doctrine**, **Greater Eurasia**, **Siberization**, and **post-West** identity framing. Guest-canon inside Diesen stream: [diesen-karaganov-canon-note.md](2026/diesen/diesen-karaganov-canon-note.md) | `Karaganov`, `RU-NUC`, or `Eurasia` in cold | × `diesen`, × `mercouris`, × `mearsheimer` |
...
| `jiang` | Jiang Xueqin (Predictive History) | **Long-horizon civilizational / game-theory** lectures; PH is the sole upstream for notebook-facing Jiang ingest. Guest-canon inside Diesen stream: [diesen-jiang-canon-note.md](2026/diesen/diesen-jiang-canon-note.md) | `Jiang`, `PH`, or `predictive-history` in cold | × `mearsheimer`, × `diesen`, × `sachs` |
