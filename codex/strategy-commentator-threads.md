...
| `freeman` | Charles (“Chas”) Freeman | **Primary civ-china (strategy) expert**: **U.S.–China** **diplomatic** **memory** **&** **PRC** **order** **talk**; **inconclusive** talks + **alliance/material** framing; **seam** from **`jiang`** **PH** **by** **default**. Guest speaker-arc inside Diesen stream: [diesen-freeman-speaker-arc.md](2026/diesen/diesen-freeman-speaker-arc.md) | `Freeman` or `ChasFreeman` in cold | × `parsi`, × `mercouris`, × [rome-persia-legitimacy-signal-check.md](rome-persia-legitimacy-signal-check.md) (**seam**, not merge); × `diesen`, × `jiang`, × `sachs` |
