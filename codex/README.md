...
- **Polyphonic cognition streams:** [COGNITION-STREAMS-POINTER.md](COGNITION-STREAMS-POINTER.md) is the quick eight-stream roster; [COGNITION-LATTICE-SPEAKERS.md](COGNITION-LATTICE-SPEAKERS.md) is the alphabetical recurring-speakers index; [COGNITION-STREAMS.md](COGNITION-STREAMS.md) is the longer scaffold. The current lattice uses `Alkorshid` / `Diesen` / `Davis` / `Mercouris` / `Crooke` / `Parsi` / `Pape` / `Ritter`, with count-neutral graph exports at [artifacts/skill-work/work-strategy/interview-graph/README.md](../artifacts/skill-work/work-strategy/interview-graph/README.md).
...
- **Pages (primary analysis):** [strategy-codex-template-page.md](strategy-codex-template-page.md) - **`strategy-page`** fences in **time-scoped thread files under `codex/2026/<channel>/`**; `python3 scripts/validate_strategy_pages.py`. Optional [strategy-notebook-page-template.md](strategy-notebook-page-template.md) is a **superseded** placeholder (on-disk name unchanged for path compatibility; prefer `strategy-codex-template-page.md`).
