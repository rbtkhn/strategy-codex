...
| Inbox registry stub | [`codex/daily-strategy-inbox.md`](../daily-strategy-inbox.md) — pointer to archive path |
| Gap check (advisory) | `python3 scripts/strategy_raw_input_gap_hint.py` |

## Legacy files on disk

Existing captures under `codex/raw-input/` and `codex/years/*/raw-input/` remain **read-only archaeology**. Do not delete or migrate unless the operator explicitly requests migration.
