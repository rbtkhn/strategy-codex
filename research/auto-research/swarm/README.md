...
accepted artifacts from `research/auto-research/self-proposals/accepted/` and promotes
them into the live gate only through the shared artifact-promotion helper.

## Contract

- keep experiments and read models under `research/auto-research/swarm/`
...
```bash
python3 research/auto-research/swarm/orchestrator.py status
```

Show the latest swarm-visible artifact:

```bash
python3 research/auto-research/swarm/orchestrator.py last
```
...
```bash
python3 research/auto-research/swarm/orchestrator.py promote latest --review-note "Operator reviewed grounding and wants gate visibility."
```
