CIV–ARC–FRANCE — v3.1
Civilizational Memory Codex · Academic Reference Canon
Author Admissibility & Temporal Precedence

Status: ACTIVE · CANONICAL
Supersedes: CIV–ARC–FRANCE v3.0
Upgrade Type: ADDITIVE · Jiang Xueqin (Civilization series), ARC-T-MODERN
Scope: Author admissibility and temporal precedence ONLY
Compatibility: CIV–MEM–CORE v3.0 · CIV–MEM–TEMPLATE v3.0 · NAMESPACE–CLARIFICATION v1.0
Last Updated: February 2026

────────────────────────────────────────────────────────────
UPGRADE NOTE (v1.9) — AUTHOR EXPANSION (SEVEN YEARS' WAR)
────────────────────────────────────────────────────────────
This version adds Fred Anderson to ARC-T-MODERN for Seven Years' War and
global-conflict coverage, resolving ARC compliance for MEM–FRANCE–WAR–SEVEN–YEARS.

v1.9 changes:
• Added Fred Anderson (b. 1949) — Crucible of War: The Seven Years' War and the Fate of Empire in British North America, 1754–1766
• Topical scope: global war, maritime/fiscal dimension, Franco-British rivalry, colonial theaters, fiscal credibility
• Supports MEM–FRANCE–WAR–SEVEN–YEARS, pre-revolutionary legitimacy diagnostics

────────────────────────────────────────────────────────────
UPGRADE NOTE (v1.8) — SYNC (MIRRORING RULE COMPLIANCE)
────────────────────────────────────────────────────────────
v1.8 documents audit sync; no author-list changes.

Additive (author scope): Justine Firnhaber-Baker — *House of Lilies: The
Dynasty That Made Medieval France* (2024) added to topical scope (Capetian
dynasty 987–1328, medieval Francia).

• CIV–CORE–FRANCE v2.0 updated to reference ARC–FRANCE and INDEX–FRANCE
  (Governed by + XXI MEM Authoring Governance Clause)
• Mirroring rule satisfied: ARC–FRANCE referenced in CORE and SCHOLAR
• INDEX–FRANCE v1.7 (115 MEMs) confirmed as canonical MEM scope

────────────────────────────────────────────────────────────
UPGRADE NOTE (v1.7) — ARC-T-ANCIENT EXPANSION
────────────────────────────────────────────────────────────
v1.7 substantially expands ancient Roman sources for Gallic/pre-Frankish period:

• Added Livy — Ab Urbe Condita (Gallic invasions, sack of Rome)
• Added Polybius — Histories (Gallic Wars, Greek perspective)
• Added Diodorus Siculus — Bibliotheca historica (Celtic ethnography)
• Added Posidonius — fragments (primary Celtic ethnography source)
• Added Pliny the Elder — Natural History (Gallic geography)
• Added Suetonius — Lives of the Caesars (Caesar context)
• Added Cassius Dio — Roman History (Gallic campaigns)
• Added Appian — Roman History, Celtica section
• Added Orosius — Seven Books (late Roman Gaul)
• Added Salvian of Marseille — De Gubernatione Dei (5th c. social collapse)
• Added Prosper of Aquitaine — Chronicle (5th c. Gaul)

ARC-T-ANCIENT expanded from 5 to 16 authors.
Temporal precedence now robust for pre-Frankish Gaul.

────────────────────────────────────────────────────────────
UPGRADE NOTE (v1.6) — AUTHOR EXPANSION
────────────────────────────────────────────────────────────
v1.6 adds diplomatic history specialist:

• Added Henry Kissinger (1923–2023) — Diplomacy; A World Restored; World Order
  French diplomatic tradition, Richelieu, Talleyrand, Vienna settlement
  Supports SYNTHESIS 0002, DOCTRINE v0.4 (Post-Hegemonic Strategic Anxiety)

────────────────────────────────────────────────────────────
UPGRADE NOTE (v1.5) — AUTHOR EXPANSION
────────────────────────────────────────────────────────────
v1.5 adds Enlightenment, Napoleonic, and Revolutionary specialists:

• Added Roger Pearson — Voltaire Almighty (Enlightenment intellectual culture)
• Added Andrew Roberts (b. 1963) — Napoleon: A Life (comprehensive biography)
• Added Jeremy D. Popkin (b. 1948) — A New World Begins (Revolution narrative)
• Note: David Chandler already present in v1.0 (no addition needed)

────────────────────────────────────────────────────────────
UPGRADE NOTE (v1.4) — AUTHOR EXPANSION
────────────────────────────────────────────────────────────
v1.4 adds medieval/early modern specialists for popular revolt and Burgundian history:

• Added Justine Firnhaber-Baker — The Jacquerie of 1358
  Peasant revolt specialist, supports DOCTRINE v0.3 (Rupture as Renewal)
• Added Bart van Loo (b. 1973) — The Burgundians
  Burgundian Netherlands, Francia-Germania interface, Valois dukes

────────────────────────────────────────────────────────────
UPGRADE NOTE (v1.3) — AUTHOR EXPANSION
────────────────────────────────────────────────────────────
v1.3 adds medieval specialist for Carolingian foundations:

• Added Johannes Fried (b. 1942) — Charlemagne; The Middle Ages
  Definitive modern Charlemagne biography, supports ENTRY 0001 and AXIOM-001

────────────────────────────────────────────────────────────
UPGRADE NOTE (v1.2) — AUTHOR EXPANSION
────────────────────────────────────────────────────────────
v1.2 adds comprehensive civilization historians and Thirty Years' War specialists:

• Added Will Durant (1885–1981) — The Story of Civilization
  Full Francia coverage across 11 volumes (Charlemagne through Napoleon)
• Added Peter H. Wilson (b. 1963) — The Thirty Years War; The Holy Roman Empire
  Supports SYNTHESIS 0002 (Thirty Years' War) and cross-civ analysis

────────────────────────────────────────────────────────────
UPGRADE NOTE (v1.1) — AUTHOR EXPANSION
────────────────────────────────────────────────────────────
v1.1 adds military historians for WWI/WWII and financial history coverage:

• Added John Keegan (1934–2012) — Military historian
  Works: The Face of Battle, Six Armies in Normandy, WWI/WWII histories
• Added Niall Ferguson (b. 1964) — Financial and imperial historian
  Works: The Pity of War, The War of the World, Empire, The House of Rothschild

────────────────────────────────────────────────────────────
CREATION NOTE (v1.0) — INITIAL GOVERNANCE ESTABLISHMENT
────────────────────────────────────────────────────────────
This file establishes the Academic Reference Canon for Francia.

v1.0 establishes:
• Core author admissibility lists across all four temporal periods
• Temporal precedence rule (immutable)
• Discovery framework for source expansion
• Governance alignment with CIV–ARC–TEMPLATE v3.0

Creation context:
• SCHOLAR–FRANCE reached v2.0 with 11 entries, 6 syntheses, 5 doctrines
• ARC gap identified during cross-civilizational analysis session
• Author lists derived from canonical Francia historiography

────────────────────────────────────────────────────────────
I. PURPOSE & SCOPE
────────────────────────────────────────────────────────────
ARC governs TWO and ONLY TWO things:

1. AUTHOR ADMISSIBILITY
   Which historians, chroniclers, and scholars may be quoted
   for Francia.

2. TEMPORAL PRECEDENCE
   When sources conflict, earlier testimony has priority.

ARC does NOT govern:
• How much to quote (see MEM–TEMPLATE)
• How to evaluate quote quality (see EQS in MEM–TEMPLATE)
• Evidence role classification (see ERC in MEM–TEMPLATE)

**Indigenous-language quote requirement (MEM–FRANCE):** Each
MEM–FRANCE file MUST contain at least one verbatim quote (≥25 words)
from an indigenous-language source admissible for this requirement.
Admissible sources: Gregory of Tours (Historia Francorum), Fredegar,
Einhard, Nithard, Jean de Joinville, Jean Froissart, Philippe de
Commines (ARC-T-MEDIEVAL); Montaigne, Descartes, Voltaire, Rousseau,
Guizot, Michelet, Tocqueville (L'Ancien Régime), Taine, Sorel
(ARC-T-EARLY-MOD); Marc Bloch, Fernand Braudel, Georges Lefebvre,
Albert Soboul (ARC-T-MODERN). English translation with clear source
attribution is sufficient. Per CIV–ARC–TEMPLATE § IV-A.

────────────────────────────────────────────────────────────
II. ARC-TEMPORAL CATEGORIES (ARC-T)
────────────────────────────────────────────────────────────
Authors are classified by the HISTORICAL PERIOD in which they wrote.

ARC-T-ANCIENT
Authors from Gaul's classical/foundational period (pre-500 AD).
Direct witnesses to Gallic tribes, Roman conquest, late Roman Gaul.

ARC-T-MEDIEVAL
Authors from the Frankish and French medieval period (c. 500–1500).
Continuity witnesses to Merovingian, Carolingian, Capetian, Valois eras.

ARC-T-EARLY-MOD
Authors from the early modern critical period (c. 1500–1900).
Historiographic interpretation, typically out of copyright.

ARC-T-MODERN
Authors from contemporary scholarship (c. 1900–present).
Current academic interpretation, under copyright.

────────────────────────────────────────────────────────────
III. TEMPORAL PRECEDENCE RULE (ABSOLUTE)
────────────────────────────────────────────────────────────
PRECEDENCE ORDER:
1. ARC-T-ANCIENT (highest authority)
2. ARC-T-MEDIEVAL
3. ARC-T-EARLY-MOD
4. ARC-T-MODERN (lowest authority)

RULE: Later periods may CLARIFY or FRAME earlier testimony.
      Later periods may NOT OVERRIDE earlier testimony.

If a modern scholar contradicts ancient or medieval testimony:
• The earlier testimony stands
• The contradiction must be explicitly noted
• Resolution is NOT required

────────────────────────────────────────────────────────────
IV. AUTHOR ADMISSIBILITY LISTS
────────────────────────────────────────────────────────────

ARC-T-ANCIENT:
• Julius Caesar — Commentarii de Bello Gallico (Gallic Wars), 1st c. BC
  PRIMARY SOURCE: Direct witness to Gallic conquest, tribal descriptions
• Strabo — Geographica, 1st c. BC–1st c. AD
  Geographic and ethnographic description of Gaul
• Livy — Ab Urbe Condita, 1st c. BC–1st c. AD
  Gallic invasions of Italy, sack of Rome (390 BC), early Gallic-Roman contact
• Polybius — Histories, 2nd c. BC
  Gallic Wars from Greek perspective, Cisalpine Gaul
• Diodorus Siculus — Bibliotheca historica, 1st c. BC
  Celtic/Gallic ethnography, customs, society, warfare
• Posidonius — (via fragments in Athenaeus, Strabo), 1st c. BC
  Celtic ethnography, Gallic society, primary source for later writers
• Pliny the Elder — Natural History, 1st c. AD
  Geographic, economic, and cultural description of Gaul
• Tacitus — Annals; Histories; Agricola, 1st–2nd c. AD
  Gallic revolts, Roman Gaul, Germanic-Gallic frontier
• Suetonius — Lives of the Caesars, 2nd c. AD
  Caesar's Gallic campaigns context, imperial Gaul
• Cassius Dio — Roman History, 2nd–3rd c. AD
  Gallic campaigns, late Roman Gaul, Gallic emperors
• Appian — Roman History (Celtica/Gallic section), 2nd c. AD
  Gallic Wars narrative, Roman conquest
• Ammianus Marcellinus — Res Gestae, 4th c. AD
  Late Roman Gaul, Julian's campaigns, Germanic incursions
• Orosius — Seven Books of History Against the Pagans, 5th c. AD
  Late Roman Gaul, barbarian invasions, Christian interpretation
• Salvian of Marseille — De Gubernatione Dei, 5th c. AD
  Social conditions in late Roman Gaul, taxation, collapse
• Sidonius Apollinaris — Letters; Poems, 5th c. AD
  Gallo-Roman aristocracy, Visigothic Gaul, transition period
• Prosper of Aquitaine — Chronicle, 5th c. AD
  Gallic chronology, barbarian settlements, ecclesiastical history

ARC-T-MEDIEVAL:
• Gregory of Tours — Historia Francorum (History of the Franks), 6th c.
• Fredegar — Chronicle of Fredegar, 7th c.
• Einhard — Vita Karoli Magni (Life of Charlemagne), 9th c.
• Nithard — Histories, 9th c.
• Annals of Saint-Bertin — 9th c.
• Richer of Reims — Historiae, 10th c.
• Suger of Saint-Denis — Vita Ludovici Grossi Regis, 12th c.
• Odo of Deuil — De profectione Ludovici VII, 12th c.
• Jean de Joinville — Vie de Saint Louis, 13th c.
• Jean Froissart — Chroniques, 14th c.
• Christine de Pizan — Various works, 14th–15th c.
• Philippe de Commines — Mémoires, 15th c.

ARC-T-EARLY-MOD:
• Jacques-Auguste de Thou (1553–1617) — Historia sui temporis
• Michel de Montaigne (1533–1592) — Essais (Essays)
• René Descartes (1596–1650) — Discours de la méthode; Méditations
  métaphysiques
• François-Marie Arouet (Voltaire) (1694–1778) — Le Siècle de Louis XIV;
  Essai sur les mœurs
• Jean-Jacques Rousseau (1712–1778) — Du contrat social; Discours sur
  l'origine et les fondements de l'inégalité; Émile
• François Guizot (1787–1874) — Histoire de la civilisation en France;
  Histoire de France
• Jules Michelet (1798–1874) — Histoire de France (19 vols.);
  Histoire de la Révolution française
• Alexis de Tocqueville (1805–1859) — L'Ancien Régime et la Révolution
• Hippolyte Taine (1828–1893) — Les Origines de la France contemporaine
• Albert Sorel (1842–1906) — L'Europe et la Révolution française

ARC-T-MODERN:
• Marc Bloch (1886–1944) — La Société féodale; Les Rois thaumaturges
  TOPICAL SCOPE:
  - Feudal Society: Medieval Francia social structure, vassalage, serfdom
  - The Royal Touch: Sacral kingship, healing power, legitimacy
• Fernand Braudel (1902–1985) — Civilisation matérielle; L'Identité de la France
  TOPICAL SCOPE:
  - Civilization and Capitalism: Long-duration economic patterns
  - The Identity of France: Geographic and historical foundations
• Georges Lefebvre (1874–1959) — La Révolution française
  TOPICAL SCOPE:
  - The French Revolution: Social origins, peasant role, 1789 dynamics
• Albert Soboul (1914–1982) — Les Sans-culottes parisiens
  TOPICAL SCOPE:
  - Parisian revolutionary movement, popular politics, Jacobinism
• François Furet (1927–1997) — Penser la Révolution française
  TOPICAL SCOPE:
  - Interpreting the French Revolution: Critique of Marxist orthodoxy
  - Revolutionary ideology, Terror as political logic
• Simon Schama (b. 1945) — Citizens: A Chronicle of the French Revolution
  TOPICAL SCOPE:
  - Narrative history of Revolution, cultural dynamics, violence
• Robert Darnton (b. 1939) — The Great Cat Massacre
  TOPICAL SCOPE:
  - Cultural history, Old Regime mentalities, print culture
• David A. Bell (b. 1961) — The First Total War
  TOPICAL SCOPE:
  - Revolutionary and Napoleonic warfare, nationalism, mobilization
• T.C.W. Blanning (b. 1942) — The French Revolutionary Wars; The Pursuit of Glory
  TOPICAL SCOPE:
  - Revolutionary warfare 1787–1802, European context
  - Eighteenth-century Europe, Francia in continental system
• Fred Anderson (b. 1949) — Crucible of War: The Seven Years' War and the Fate of Empire in British North America, 1754–1766
  TOPICAL SCOPE:
  - Seven Years' War as global conflict, maritime/fiscal dimension
  - Franco-British rivalry, North American and colonial theaters
  - French fiscal credibility, naval inferiority, legitimacy shock
  - Supports MEM–FRANCE–WAR–SEVEN–YEARS, pre-revolutionary diagnostics
• David G. Chandler (1934–2004) — The Campaigns of Napoleon
  TOPICAL SCOPE:
  - Napoleonic military history, operational analysis
• Jean Tulard (b. 1933) — Napoléon, ou le mythe du sauveur
  TOPICAL SCOPE:
  - Napoleon: The Myth of the Saviour, Napoleonic administration
• Robert Tombs (b. 1949) — France 1814–1914
  TOPICAL SCOPE:
  - Nineteenth-century Francia, Restoration through Third Republic
• Eugen Weber (1925–2007) — Peasants into Frenchmen
  TOPICAL SCOPE:
  - Nation-building, rural France 1870–1914, modernization
• Robert Gildea (b. 1952) — The Past in French History
  TOPICAL SCOPE:
  - French historical memory, uses of the past
• John J. Mearsheimer (b. 1947) — "Bound to Fail: The Rise and Fall of the Liberal International Order," *International Security* 43, no. 4 (Spring 2019); structural realism, international orders (Cold War, liberal order 1990–2019), bipolar/unipolar, great-power competition. As applicable for MEMs for structural/power framing and order-type analysis.
• Dominic Lieven (b. 1952) — Russia Against Napoleon
  TOPICAL SCOPE:
  - 1812 campaign from Russian perspective, Napoleonic defeat
  - Supports cross-civilizational analysis (FRANCE ↔ RUSSIA)
• John Keegan (1934–2012) — The Face of Battle; Six Armies in Normandy;
  The First World War; The Second World War
  TOPICAL SCOPE:
  - The Face of Battle: Agincourt, Waterloo, Somme — Francia's wars from soldier perspective
  - Six Armies in Normandy: D-Day and liberation, Allied/German/French forces
  - WWI/WWII comprehensive military histories with Francia focus
• Niall Ferguson (b. 1964) — The Pity of War; The War of the World;
  Empire; The House of Rothschild
  TOPICAL SCOPE:
  - The Pity of War: WWI, French warfare, Western Front, total war
  - The War of the World: Twentieth-century conflict, Fall of France 1940
  - Empire: Comparative imperial history (Francia colonial context)
  - The House of Rothschild: French branch, Paris banking, Napoleonic finance
• Jiang Xueqin (b. 1976) — "Civilization" series (YouTube lectures); e.g. "Civilization #53: Dostoevsky and the Soul of Russia" and Francia-relevant episodes. Civilizational contrast, pedagogical synthesis; admissible for cross-civilizational encoding, comparative redemption/identity, Catholic vs Orthodox. May supplement but not substitute for Francia-primary or specialist scholarship. [0]
• Will Durant (1885–1981) — The Story of Civilization (11 vols.)
  TOPICAL SCOPE:
  - Vol. 1 (Our Oriental Heritage): Pre-classical foundations
  - Vol. 4 (The Age of Faith): Charlemagne, Crusades, medieval Francia
  - Vol. 5 (The Renaissance): French Renaissance, Francis I
  - Vol. 6 (The Reformation): French Wars of Religion, Catherine de' Medici
  - Vol. 7 (The Age of Reason Begins): Thirty Years' War, Richelieu
  - Vol. 8 (The Age of Louis XIV): Absolutism apex, Versailles
  - Vol. 9 (The Age of Voltaire): Enlightenment, philosophes
  - Vol. 10 (Rousseau and Revolution): Pre-revolutionary France
  - Vol. 11 (The Age of Napoleon): Revolution, Napoleon, Empire
• Peter H. Wilson (b. 1963) — The Thirty Years War; The Holy Roman Empire;
  Heart of Europe
  TOPICAL SCOPE:
  - The Thirty Years War: Francia's intervention, Richelieu's strategy
  - The Holy Roman Empire: Francia's eastern neighbor, Habsburg rivalry
  - Heart of Europe: Central European context for French expansion
  - Supports cross-civilizational analysis (FRANCE ↔ GERMANY)
• Johannes Fried (b. 1942) — Charlemagne; The Middle Ages
  TOPICAL SCOPE:
  - Charlemagne (2016): Definitive modern biography, Carolingian foundations
  - The Middle Ages: Medieval European civilization, Frankish origins
  - Carolingian court culture, imperial ideology, legitimacy construction
  - Supports ENTRY 0001 (Charlemagne), AXIOM FRANCE-001
• Justine Firnhaber-Baker — The Jacquerie of 1358; Violence and the State;
  House of Lilies: The Dynasty That Made Medieval France (2024)
  TOPICAL SCOPE:
  - The Jacquerie of 1358: Peasant revolt, popular violence, medieval Francia
  - Violence and the State in Languedoc: Regional governance, royal authority
  - House of Lilies: Capetian dynasty 987–1328, medieval France, Crusades,
    wars with England, Gothic architecture, courtly culture; Hugh Capet to
    Eleanor of Aquitaine
  - Medieval French political culture, legitimacy crises, rupture patterns
  - Supports DOCTRINE v0.3 (Rupture as Renewal), AXIOM FRANCE-003
• Bart van Loo (b. 1973) — The Burgundians: A Vanished Empire
  TOPICAL SCOPE:
  - The Burgundians (2021): Burgundian Netherlands, Valois dukes
  - Francia-Germania interface, Low Countries, dynastic politics
  - Hundred Years' War context, Charles the Bold, Burgundian court culture
  - Supports cross-civilizational analysis (FRANCE ↔ GERMANY)
• Roger Pearson — Voltaire Almighty: A Life in Pursuit of Freedom
  TOPICAL SCOPE:
  - Voltaire biography, Enlightenment intellectual culture
  - Old Regime critique, philosophe movement, religious tolerance
  - Supports analysis of pre-Revolutionary intellectual foundations
• Andrew Roberts (b. 1963) — Napoleon: A Life; The Storm of War
  TOPICAL SCOPE:
  - Napoleon: A Life (2014): Comprehensive biography, political and military
  - The Storm of War: WWII with Francia context (Fall of France, Vichy)
  - Napoleonic administration, legitimacy construction, imperial system
  - Supports ENTRY 0003 (Napoleon), SYNTHESIS 0005
• Jeremy D. Popkin (b. 1948) — A New World Begins; A Short History of the
  French Revolution
  TOPICAL SCOPE:
  - A New World Begins (2019): Comprehensive Revolution narrative
  - Revolutionary politics, Terror, Thermidor, Directory
  - Supports DOCTRINE v0.3 (Rupture as Renewal), AXIOM FRANCE-003
• Henry Kissinger (1923–2023) — Diplomacy; A World Restored; World Order
  TOPICAL SCOPE:
  - Diplomacy (1994): French diplomatic tradition, Richelieu, Louis XIV, Napoleon
  - A World Restored (1957): Congress of Vienna, Talleyrand, post-Napoleonic settlement
  - World Order: French revolutionary impact on international system
  - Raison d'état origins, balance of power, legitimacy in international relations
  - Supports SYNTHESIS 0002, DOCTRINE v0.4 (Post-Hegemonic Strategic Anxiety)

RULE: Only listed authors may be quoted without discovery flag.

────────────────────────────────────────────────────────────
V. ARC DISCOVERY FRAMEWORK
────────────────────────────────────────────────────────────
When a MEM requires sources beyond the approved list:

DISCOVERY SEQUENCE:
1. Search listed authors first
2. If insufficient, expand to adjacent authors/traditions
3. Flag all non-listed sources

DISCOVERY FLAG (REQUIRED):
"ARC–DISCOVERY: EXPANDED SEARCH
Source: [Author/Work]
Justification: [Why this source is necessary]"

Expanded discovery does NOT:
• Add authors to the permanent canon
• Alter temporal precedence
• Remove the requirement to flag

────────────────────────────────────────────────────────────
V-A. CROSS-ARC CITATION FOR TSP MEMs
────────────────────────────────────────────────────────────
Trans-Sovereign Pattern (TSP) MEMs may require sources from multiple
civilizations' ARCs. Per CIV–MEM–CORE § XXIX.

TSP PATTERN TYPES: TRANSMISSION, NETWORK, RECONSTITUTION

CROSS-ARC CITATION RULES:
1. Primary ARC (ARC–FRANCE) governs MEMs filed under FRANCE.
2. TSP MEMs MAY cite sources from connected civilizations' ARCs.
3. Cross-ARC citations MUST declare: [Author], [Work] (ARC–[CIV]-T-[PERIOD]) [ERC-ROLE]
4. Temporal precedence applies within each ARC independently.
5. Unlisted sources require standard discovery flag.

TSP EXAMPLES (FRANCE): MEM–FRANCE–ROTHSCHILD (NETWORK — cross-ARC: GERMANY,
ANGLIA); MEM–FRANCE–CAROLINGIAN (TRANSMISSION — cross-ARC: ROME, GERMANY).

────────────────────────────────────────────────────────────
VI. ORTHOGONALITY WITH ERC
────────────────────────────────────────────────────────────
ARC-T (temporal categories) and ERC (evidence role categories)
are ORTHOGONAL classification systems.

A single quotation has BOTH classifications:

EXAMPLE:
A quote from Voltaire's Le Siècle de Louis XIV:
• ARC-T: ARC-T-EARLY-MOD (18th century author)
• ERC: ERC-SECONDARY (scholarly interpretation)

EXAMPLE:
A quote from Gregory of Tours' Historia Francorum:
• ARC-T: ARC-T-MEDIEVAL (6th century author)
• ERC: ERC-PRIMARY (direct witness to Merovingian period)

EXAMPLE:
A quote from Einhard's Vita Karoli Magni:
• ARC-T: ARC-T-MEDIEVAL (9th century author)
• ERC: ERC-PRIMARY (direct witness to Carolingian court)

See NAMESPACE–CLARIFICATION v1.0 for full disambiguation.

────────────────────────────────────────────────────────────
VII. WHAT ARC DOES NOT GOVERN
────────────────────────────────────────────────────────────
Governed by CIV–MEM–TEMPLATE, NOT by ARC:
• 20% verbatim quote minimum
• Evidence Quality Standards / EQS
• Analytical content floor / 70%
• Quote word counting rules
• Subject-type adaptive requirements
• ERC evidence-role classification

ARC determines WHO may be quoted.
MEM–TEMPLATE determines HOW MUCH and HOW WELL.

────────────────────────────────────────────────────────────
VIII. MIRRORING RULE
────────────────────────────────────────────────────────────
ARC–FRANCE must be referenced in:
• CIV–CORE–FRANCE
• CIV–SCHOLAR–FRANCE

Reference by version pointer is sufficient.

────────────────────────────────────────────────────────────
IX. VERSIONING
────────────────────────────────────────────────────────────
• Additive upgrades only for author lists
• Temporal precedence rule is immutable
• Discovery framework may be refined
• Namespace alignment is permanent

Future versions may add:
• Colonial/imperial period specialists
• Military historians beyond Napoleonic scope
• Economic historians
• Regional specialists (Provence, Brittany, Alsace)

────────────────────────────────────────────────────────────
museum knowledge section B. ARC-T-INSTITUTIONAL (INSTITUTIONAL SOURCES)
────────────────────────────────────────────────────────────
Authorized institutional sources for STATE mode analysis of
Francia. Covers both real-time news sources and longer-term
official analysis and planning. STATE draws Layer 3 information
through these sources first. Non-listed sources are permitted
with ARC discovery flag.

ARC-T-INSTITUTIONAL (OFFICIAL):
• Élysée — elysee.fr
  Authoritative For: Presidential policy, foreign affairs, defence posture
  Editorial Note: French presidency; primary authority on executive intent

• Assemblée Nationale — assemblee-nationale.fr
  Authoritative For: Legislation, parliamentary debates, committee reports
  Editorial Note: French lower house; official legislative record

• Sénat — senat.fr
  Authoritative For: Legislation, territorial governance, foreign affairs reports
  Editorial Note: French upper house; notable foreign affairs committee reports

• Banque de France — banque-france.fr
  Authoritative For: Monetary policy, eurozone economics, financial stability
  Editorial Note: French central bank; ECB Governing Council member

• Ministère des Armées — defense.gouv.fr
  Authoritative For: Defence policy, nuclear doctrine, military operations
  Editorial Note: French defence ministry; primary on force de frappe posture

ARC-T-INSTITUTIONAL (RESEARCH):
• IFRI (Institut Français des Relations Internationales) — ifri.org
  Authoritative For: French foreign policy, European strategy, security
  Editorial Note: Independent; France's leading foreign policy think tank

• Sciences Po / CERI — sciencespo.fr
  Authoritative For: Political science, international relations, European politics
  Editorial Note: Elite university; academic; shapes French policy class

• Institut Montaigne — institutmontaigne.org
  Authoritative For: Economic reform, education, digital policy
  Editorial Note: Business-funded; centre-liberal; pro-reform orientation

• Fondation pour la Recherche Stratégique (FRS) — frstrategie.org
  Authoritative For: Nuclear strategy, defence economics, arms control
  Editorial Note: Government-linked; key source on French nuclear doctrine

• Institut de France — institut-de-france.fr
  Authoritative For: French cultural and intellectual heritage, national archives
  Editorial Note: Five academies including Académie française; state-funded

ARC-T-INSTITUTIONAL (NEWS):
• Le Monde — lemonde.fr
  Authoritative For: Politics, international affairs, investigative reporting
  Editorial Note: Centre-left; French paper of record; reader-owned

• Le Figaro — lefigaro.fr
  Authoritative For: Politics, economics, foreign affairs
  Editorial Note: Centre-right; Dassault-owned; French establishment

• AFP (Agence France-Presse) — afp.com
  Authoritative For: Wire reporting, breaking news, global coverage
  Editorial Note: French national wire service; semi-public; aims for neutrality

• France 24 — france24.com
  Authoritative For: International news, francophone world, European affairs
  Editorial Note: State-funded; English/French/Arabic; France's global voice

• Mediapart — mediapart.fr
  Authoritative For: Investigative journalism, political scandals, civil liberties
  Editorial Note: Independent; subscription-funded; left-leaning; high-impact investigations

ARC-T-INSTITUTIONAL (SPECIALIST):
• FRS (Fondation pour la Recherche Stratégique) — frstrategie.org
  Authoritative For: Nuclear deterrence, defence technology, arms control
  Editorial Note: See RESEARCH entry; listed here for specialist defence domain

• IRSEM (Institut de Recherche Stratégique de l'École Militaire) — irsem.fr
  Authoritative For: Military strategy, information warfare, geopolitics
  Editorial Note: French MOD research institute; academic military analysis

• Cour des Comptes — ccomptes.fr
  Authoritative For: Public spending audits, fiscal oversight, institutional reform
  Editorial Note: French supreme audit institution; independent; data-authoritative

• INSEE (Institut national de la statistique) — insee.fr
  Authoritative For: Economic statistics, demographics, social indicators
  Editorial Note: National statistics office; official data source

────────────────────────────────────────────────────────────
XII. BLACKLIST (EXCLUDED INSTITUTIONAL SOURCES)
────────────────────────────────────────────────────────────
The following institutional sources MUST NOT be cited or used for
STATE mode analysis of Francia. They are excluded from ARC-T-INSTITUTIONAL
and from ARC discovery. Per CIV–ARC–TEMPLATE § IX-D, the following
entry is mandatory for all ARCs.

BLACKLISTED:
• Wikipedia — wikipedia.org (all language editions)
  Reason: Not an institutional or primary source. Crowd-sourced;
  no single authorship or editorial accountability; variable
  quality and sourcing; unsuitable for admissibility. Do not
  cite Wikipedia as evidence in STATE or SCHOLAR for this
  civilization. Use listed institutional sources or ARC
  discovery with justification instead.

────────────────────────────────────────────────────────────
END OF FILE — CIV–ARC–FRANCE v3.1
────────────────────────────────────────────────────────────
