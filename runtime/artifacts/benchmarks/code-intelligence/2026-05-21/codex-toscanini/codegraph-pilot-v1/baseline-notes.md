...
- Primary entry point is [`create_app`](C:/dev/strategy-codex/platform/src/grace_mar/presentations/service.py), which exposes `POST /v1/bundles/render` and owns bundle validation, Presenton render dispatch, record write, and manifest append.
- Main downstream dependency is [`PresentonClient.generate_presentation`](C:/dev/strategy-codex/platform/src/grace_mar/presentations/presenton_client.py), which builds multipart form data and POSTs to the external Presenton service.
- Supporting architecture files surfaced cleanly:
  - [contract.py](C:/dev/strategy-codex/platform/src/grace_mar/presentations/contract.py)
  - [intents.py](C:/dev/strategy-codex/platform/src/grace_mar/presentations/intents.py)
...
| `wall_minutes` | `3` |
| `tool_calls` | `4` |
| `output_path` | `runtime/artifacts/benchmarks/code-intelligence/2026-05-21/codex-toscanini/codegraph-pilot-v1/baseline-notes.md` |
...
- [presenton_client.py](C:/dev/strategy-codex/platform/src/grace_mar/presentations/presenton_client.py) because `create_app` instantiates and calls `PresentonClient.generate_presentation`
- [contract.py](C:/dev/strategy-codex/platform/src/grace_mar/presentations/contract.py) because `render_bundle` validates and hashes bundles
- [intents.py](C:/dev/strategy-codex/platform/src/grace_mar/presentations/intents.py) because template resolution and markdown generation happen there
