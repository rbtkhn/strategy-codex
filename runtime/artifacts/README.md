...
| `runtime/artifacts/repo-convergence-report.json` | **`scripts/run_repo_convergence.py`** | **Derived** last convergence run report (per-loop status, command tails); advisory; [doc](../docs/repo-convergence.md). |
| `runtime/artifacts/repo-convergence-state.json` | **`scripts/run_repo_convergence.py`** | **Derived** committed input-hash baseline for dirty-loop detection; not Record; [doc](../docs/repo-convergence.md). |
