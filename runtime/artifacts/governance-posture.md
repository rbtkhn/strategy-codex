...
- **Voice:** `archive/grace-mar-instance/bot/` renders the Record when queried; it does **not** replace the pipeline. See [docs/conceptual-framework.md](../docs/conceptual-framework.md) for the full distinction.
...
3. **OpenClaw and assistants stage only** — they do not write SELF, EVIDENCE, or `archive/grace-mar-instance/bot/prompt.py` without that merge. Chat is not proof of Record truth.
...
python3 scripts/run_voice_benchmark.py -o grace-mar/runtime/artifacts/voice_benchmark_results.json
