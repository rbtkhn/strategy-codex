...
from temporal.trends import compute_trend  # noqa: E402
...
def test_compute_trend_increase_decrease_stable() -> None:
    stable = [{"time_index": 0, "confidence": 0.5}, {"time_index": 1, "confidence": 0.55}]
    increase = [{"time_index": 0, "confidence": 0.4}, {"time_index": 1, "confidence": 0.8}]
    decrease = [{"time_index": 0, "confidence": 0.9}, {"time_index": 1, "confidence": 0.5}]
    assert compute_trend(stable) == "stable"
    assert compute_trend(increase) == "slight_increase"
    assert compute_trend(decrease) == "slight_decrease"
...
def test_build_temporal_view_schema() -> None:
    ...
    assert set(record) == {"event_id", "timeline", "trend", "ordering_confidence"}
...
def test_run_temporal_layer_integration(tmp_path: Path) -> None:
    ...
    assert len(temporal_by_event) == 1
    assert summary["ordering_confidence_avg"] == 1.0
