"""Tests for PR8 Epistemic Intelligence Core."""

from __future__ import annotations

import sys
from pathlib import Path

SCRIPTS = Path(__file__).resolve().parents[1] / "scripts"
if str(SCRIPTS) not in sys.path:
    sys.path.insert(0, str(SCRIPTS))

from prediction.epistemic_intelligence_core import (  # noqa: E402
    build_eic_payload,
    build_epistemic_object,
    classify_regime,
    compute_distributional_signals,
    project_trajectories,
    soft_align,
)


def _registry() -> dict:
    return {
        "evt_a": {"question": "Will A happen?", "falsifier": "falsifier a", "status": "open"},
        "evt_b": {"question": "Will B happen?", "falsifier": "falsifier b", "status": "open"},
    }


def _terms_index() -> dict:
    return {
        "evt_a": ["alpha", "capitulation"],
        "evt_b": ["beta", "tariff"],
    }


def test_soft_align_multi_event_mass() -> None:
    dist, entropy = soft_align(
        "alpha capitulation and beta tariff overlap",
        capture_event_id="evt_a",
        terms_index=_terms_index(),
        registry=_registry(),
    )
    assert len(dist) >= 2
    assert sum(float(d["weight"]) for d in dist) == 1.0
    assert entropy >= 0.0


def test_soft_align_capture_prior() -> None:
    dist, _ = soft_align(
        "unrelated claim text",
        capture_event_id="evt_a",
        terms_index=_terms_index(),
        registry=_registry(),
    )
    assert dist[0]["event_id"] == "evt_a"
    assert dist[0]["weight"] == 1.0


def test_classify_regime_escalation() -> None:
    regime = classify_regime(
        {"directional": 0.72, "volatility": 0.2, "drift": 0.15},
        [{"event_id": "evt_a", "weight": 1.0}],
        0.5,
    )
    assert regime["label"] == "escalation"


def test_classify_regime_fragmentation() -> None:
    regime = classify_regime(
        {"directional": 0.4, "volatility": 0.3, "drift": 0.02},
        [{"event_id": "evt_a", "weight": 0.5}, {"event_id": "evt_b", "weight": 0.5}],
        1.5,
    )
    assert regime["label"] == "fragmentation"


def test_macgregor_volatility_dampening() -> None:
    trajectories = [
        {
            "event_id": "evt_a",
            "voice": "macgregor",
            "trajectory": [
                {"timestamp": "2025-01-01", "probability": 0.2, "stance": "no"},
                {"timestamp": "2025-06-01", "probability": 0.8, "stance": "yes"},
            ],
        }
    ]
    projections = [{"event_id": "evt_a", "weight": 1.0, "probability": 0.8, "stance": "yes", "confidence": 0.7}]
    high_ent = compute_distributional_signals(
        projections,
        trajectories=trajectories,
        voice="macgregor",
        primary_event_id="evt_a",
        semantic_entropy=0.9,
    )
    low_ent = compute_distributional_signals(
        projections,
        trajectories=trajectories,
        voice="macgregor",
        primary_event_id="evt_a",
        semantic_entropy=0.0,
    )
    assert high_ent["volatility"] <= low_ent["volatility"]
    assert high_ent["volatility"] < low_ent["volatility"]


def test_build_eic_payload_shape() -> None:
    mvel = {
        "trajectories": [
            {
                "event_id": "evt_a",
                "voice": "freeman",
                "trajectory": [
                    {
                        "timestamp": "2025-01-01",
                        "claim": "alpha capitulation likely",
                        "stance": "yes",
                        "probability": 0.75,
                        "confidence": 0.7,
                        "capture": "source-archive/statecraft/2025-01-01/source-a.md",
                    }
                ],
            }
        ]
    }
    bundle = build_eic_payload(mvel_dataset=mvel, registry=_registry(), semantic_scores={})
    assert bundle["core"]["interpretation"] == "epistemic_intelligence_core"
    assert bundle["object_count"] == 1
    obj = bundle["core"]["objects"][0]
    assert obj["interpretation"] == "unified_epistemic_state"
    assert "event_distribution" in obj
    assert "trajectory_signals" in obj
    assert "regime" in obj


def test_project_trajectories() -> None:
    trajectories = [
        {
            "event_id": "evt_a",
            "voice": "freeman",
            "trajectory": [
                {"timestamp": "2025-01-01", "probability": 0.75, "stance": "yes", "confidence": 0.7},
            ],
        }
    ]
    projections = project_trajectories(
        [{"event_id": "evt_a", "weight": 1.0}],
        trajectories,
        {"voice": "freeman", "timestamp": "2025-01-01"},
    )
    assert len(projections) == 1
    assert projections[0]["probability"] == 0.75
