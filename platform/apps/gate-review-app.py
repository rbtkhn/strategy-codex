...
Run from repo root (so bot.core and scripts resolve paths correctly):
    OPERATOR_SECRET=your_secret python platform/apps/gate-review-app.py
    # Or: FLASK_APP=platform/apps/gate-review-app.py flask run --port 5001

Deploy alongside platform/apps/miniapp_server.py on Render; protect with OPERATOR_SECRET (Bearer token).
"""
...
assert_canonical_record_layout(USER_ID, context="gate-review-platform/app")
    ...
    document.querySelectorAll('.btn.suggested-class').forEach(function(btn) {{
      btn.onclick = function() {{
        ...
        doAction(btn.getAttribute('data-id'), 'reclassify', btn, {{
          ...
          review_note: 'Applied suggested boundary class (gate-review-platform/app)'
        }});
      }};
    }});
  </script>
...
def action():
    ...
    channel_key = "operator:gate-review-platform/app"
    source = "gate-review-platform/app"
