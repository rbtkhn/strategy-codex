...
Usage:
    ...
    OPENAI_API_KEY=... python platform/apps/miniapp_server.py
...
load_dotenv(REPO_ROOT / "archive/grace-mar-instance/bot" / ".env")
...
app = Flask(
    __name__,
    static_folder=str(REPO_ROOT / "platform/miniapp"),
    static_url_path="",
)
OPENAI_API_KEY = os.getenv("OPENAI_API_KEY")
OPENAI_MODEL = os.getenv("OPENAI_MODEL", "gpt-4o")
USER_DIR = profile_dir(USER_ID)
ARTIFACTS_DIR = USER_DIR / "runtime/artifacts"
...
def telegram_webhook():
    ...
    try:
        ...
        update = Update.de_json(data, _telegram_app.archive/grace-mar-instance/bot)
...
def operator_artifacts():
    """Upload a file to runtime/artifacts/ and optionally submit an observation. No markdown editing."""
    ok, err = _operator_auth()
    if not ok:
        return err
    ...
    rel_path = f"runtime/artifacts/{base}"
...
def ask():
    ...
    data = request.get_json() or {}
    try:
        out = _process_ask_body(data, channel_key="platform/miniapp", archive=True)
        if "_error" in out:
            msg, code = out["_error"]
            return jsonify({"error": msg}), code
        return jsonify(out)
