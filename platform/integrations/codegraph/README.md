...
- code exploration against `platform/src/`, `scripts/`, and integration code
...
python -m integrations.codegraph.export_code_context \
  ...
  --changed-file "platform/src/grace_mar/presentations/service.py" \
...
python -m integrations.codegraph.generate_architecture_bundle \
  --input runtime/artifacts/codegraph/service-architecture.json \
  --output runtime/artifacts/presentations/codegraph-service-architecture.bundle.json \
...
- `runtime/artifacts/codegraph/` outputs are rebuildable WORK artifacts, not canonical truth.
