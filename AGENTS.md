...
**Active repo identity:** `strategy-codex` is the active development repo and operator surface. Historical **Grace-Mar** and **companion-self** references may remain in archived docs, legacy examples, runtime bundles, or embedded Record material, but they are **not** active routing defaults for strategy-codex work.

**Deprecation guard:** Do not route live strategy-codex stewardship through `companion-self` template sync, `grace-mar` repo assumptions, or Grace-Mar-vs-template reconciliation unless the operator explicitly asks for that legacy/archive lane. In ordinary `coffee`, `A. Steward`, warmup, boundary, git, and integrity work, prefer strategy-codex-native paths and current repo state.
