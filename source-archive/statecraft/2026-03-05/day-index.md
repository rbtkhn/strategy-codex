# Statecraft Archive - Day Index - 2026-03-05

_Generated inventory note. Rebuild with `python scripts/build_statecraft_day_indices.py`._

## Stats

- Source files: `6`
- Channel sources: `2`
- Writer sources: `4`
- Other sources: `0`
- Helper notes (excluded from source count): `0`
- Body kind mix: `substack-post` (4), `operator-transcript` (1), `transcript` (1)
- Source form mix: `newsletter` (4), `interview` (1), `solo` (1)
- Distinct channels/shows: `4`
- Distinct hosts: `3`
- Distinct guests: `1`
- Distinct threads: `5`

## Channel / Show Rollup

- `escalationtrap.substack.com` (3), `Alexander Mercouris` (1), `Dialogue Works` (1), `simplicius76.substack.com` (1)

## Host / Guest / Thread Rollup

- Hosts: `Robert Pape` (3), `Alexander Mercouris` (1), `Nima Alkhorshid` (1)
- Guests: `Andrei Martyanov` (1)
- Threads: `pape` (3), `martyanov` (1), `mercouris` (1), `nima` (1), `simplicius` (1)

## Channel sources

_YouTube / check-sources roster captures for this day (`is_youtube_capture`)._

| Channel slug | Show | Thread | Source link | File |
| --- | --- | --- | --- | --- |
| `alexander-mercouris` | Alexander Mercouris | `mercouris` | [watch](https://www.youtube.com/watch?v=tUbc17SsVS8) | `source-alexander-mercouris-iran-tightens-hormuz-blockade-us-strikes-intensify-kurds-in-ukraine-out-2026-03-05.md` |
| `dialogue-works` | Dialogue Works | `martyanov`, `nima` | [watch](https://www.youtube.com/watch?v=V7W6hk-vyok) | `source-dialogue-works-andrei-martyanov-its-over-for-the-bases-radar-destroyed-f-15-fighters-sh-2026-03-05.md` |

## Writer sources

_Configured prose / check-written roster captures for this day._

| Writer slug | Thread | Feed host | Source link | File |
| --- | --- | --- | --- | --- |
| `pape` | `pape` | `escalationtrap.substack.com` | [watch](https://escalationtrap.substack.com/p/day-5-the-war-is-widening-from-gulf) | `source-pape-day-5-the-war-is-widening-from-gulf-2026-03-05.md` |
| `pape` | `pape` | `escalationtrap.substack.com` | [watch](https://escalationtrap.substack.com/p/parallel-strategic-attack-stage-ii) | `source-pape-parallel-strategic-attack-stage-ii-2026-03-05.md` |
| `pape` | `pape` | `escalationtrap.substack.com` | [watch](https://escalationtrap.substack.com/p/the-nation-building-trap) | `source-pape-the-nation-building-trap-2026-03-05.md` |
| `simplicius` | `simplicius` | `simplicius76.substack.com` | [watch](https://simplicius76.substack.com/p/us-set-to-escalate-war-into-ground) | `source-simplicius-us-set-to-escalate-war-into-ground-2026-03-05.md` |

## Other sources

_Prose or archive lands outside channel-index and writer-index rosters._

| Voice / show | Thread | Source link | File |
| --- | --- | --- | --- |
| _none_ | — | — | — |

## Files

- `source-alexander-mercouris-iran-tightens-hormuz-blockade-us-strikes-intensify-kurds-in-ukraine-out-2026-03-05.md`
- `source-dialogue-works-andrei-martyanov-its-over-for-the-bases-radar-destroyed-f-15-fighters-sh-2026-03-05.md`
- `source-pape-day-5-the-war-is-widening-from-gulf-2026-03-05.md`
- `source-pape-parallel-strategic-attack-stage-ii-2026-03-05.md`
- `source-pape-the-nation-building-trap-2026-03-05.md`
- `source-simplicius-us-set-to-escalate-war-into-ground-2026-03-05.md`
