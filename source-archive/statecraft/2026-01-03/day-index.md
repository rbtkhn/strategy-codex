# Statecraft Archive - Day Index - 2026-01-03

_Generated inventory note. Rebuild with `python scripts/build_statecraft_day_indices.py`._

## Stats

- Source files: `4`
- Channel sources: `1`
- Writer sources: `2`
- Other sources: `1`
- Helper notes (excluded from source count): `0`
- Body kind mix: `operator-transcript` (1), `substack-post` (1), `transcript` (1), `youtube` (1)
- Source form mix: `newsletter` (2), `interview` (1), `solo` (1)
- Distinct channels/shows: `3`
- Distinct hosts: `2`
- Distinct guests: `2`
- Distinct threads: `4`

## Channel / Show Rollup

- `Alexander Mercouris` (1), `Jeffrey Sachs` (1), `simplicius76.substack.com` (1)

## Host / Guest / Thread Rollup

- Hosts: `Alexander Mercouris` (1), `Jeffrey Sachs` (1)
- Guests: `Jeffrey Sachs` (1), `Scott Ritter` (1)
- Threads: `mercouris` (1), `ritter` (1), `sachs` (1), `simplicius` (1)

## Channel sources

_YouTube / check-sources roster captures for this day (`is_youtube_capture`)._

| Channel slug | Show | Thread | Source link | File |
| --- | --- | --- | --- | --- |
| `alexander-mercouris` | Alexander Mercouris | `mercouris` | [watch](https://www.youtube.com/watch?v=PTMHONppOHA) | `source-alexander-mercouris-us-raids-venezuela-seizes-maduro-no-resistance-government-retains-contro-2026-01-03.md` |

## Writer sources

_Configured prose / check-written roster captures for this day._

| Writer slug | Thread | Feed host | Source link | File |
| --- | --- | --- | --- | --- |
| `ritter` | `ritter` | `scottritter.substack.com` | [watch](https://scottritter.substack.com/p/russias-dark-sage) | `source-ritter-russia-dark-sage-karaganov-2026-01-03.md` |
| `simplicius` | `simplicius` | `simplicius76.substack.com` | [watch](https://simplicius76.substack.com/p/sitrep-1226-odessa-pressure-peaks) | `source-simplicius-sitrep-1226-odessa-pressure-peaks-2026-01-03.md` |

## Other sources

_Prose or archive lands outside channel-index and writer-index rosters._

| Voice / show | Thread | Source link | File |
| --- | --- | --- | --- |
| Jeffrey Sachs | `sachs` | [watch](https://www.jeffsachs.org/im-archive-by-date) | `source-sachs-us-attacks-venezuela-and-kidnaps-president-maduro-2026-01-03.md` |

## Files

- `source-alexander-mercouris-us-raids-venezuela-seizes-maduro-no-resistance-government-retains-contro-2026-01-03.md`
- `source-ritter-russia-dark-sage-karaganov-2026-01-03.md`
- `source-sachs-us-attacks-venezuela-and-kidnaps-president-maduro-2026-01-03.md`
- `source-simplicius-sitrep-1226-odessa-pressure-peaks-2026-01-03.md`
