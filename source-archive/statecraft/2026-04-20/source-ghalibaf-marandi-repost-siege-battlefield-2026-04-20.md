---
ingest_date: 2026-04-20
pub_date: 2026-04-20
kind: x-post-text
source_form: post
account_author: "@mb_ghalibaf"
account_reposter: "@s_m_marandi"
source_url_profile_author: https://x.com/mb_ghalibaf
source_url_profile_reposter: https://x.com/s_m_marandi
note: Operator capture; post dated **2026-04-20** (local). X “Translated from Persian” English; optional-pin per-post status URL for cite-grade archive.
---
# Ghalibaf (@mb_ghalibaf) — reposted by Seyed Mohammad Marandi (@s_m_marandi)

**Post date (operator):** 2026-04-20

**Capture metadata (as pasted):**

Seyed Mohammad Marandi reposted

محمدباقر قالیباف | MB Ghalibaf  
@mb_ghalibaf  
· 4h  

Translated from Persian

Trump, by imposing a siege and violating the ceasefire, seeks to turn this negotiating table— in his own imagination— into a table of surrender or to justify renewed warmongering.  
We do not accept negotiations under the shadow of threats, and in the past two weeks, we have prepared to reveal new cards on the battlefield.

---

## Slice anchor (registry)

<a id="slice-mb_ghalibaf-2026-04-20"></a>

**Slice registry:** [strategy-state-iran/chapters/2026-04/april-2026-slice-registry.md](../../strategy-state-iran/chapters/2026-04/april-2026-slice-registry.md).
