# Statecraft Archive - Writer Index

_Generated inventory note. Rebuild with `python scripts/refresh_statecraft_archive_indices.py`._

Flat registry of **configured written-source outlets** (Substack feeds in v1). YouTube channels live in [channel-index.md](./channel-index.md). Cross-modal speaker rollups: [thread-index.md](./thread-index.md).

Inclusion law: [writer-index-spec.md](./writer-index-spec.md). Roster SSOT: [statecraft_writer_discovery.json](../../platform/config/statecraft_writer_discovery.json).

## Stats

- Configured writers: `6`
- Archive files mapped: `196`
- Machine roster: [writer-index.json](./writer-index.json)

## Writers

| Writer slug | Label | Files | Days | Thread | Feed URL | First day | Last day |
| --- | --- | ---: | ---: | --- | --- | --- | --- |
| `simplicius` | Simplicius | 63 | 62 | `simplicius` | [open](https://simplicius76.substack.com) | `2026-01-02` | `2026-04-27` |
| `pape` | Prof Robert Pape / Escalation Trap | 53 | 44 | `pape` | [open](https://escalationtrap.substack.com) | `2026-02-25` | `2026-06-17` |
| `crooke` | Alastair Crooke / Conflicts Forum | 29 | 29 | `crooke` | [open](https://conflictsforum.substack.com) | `2026-01-08` | `2026-06-18` |
| `ritter` | Scott Ritter / Real Scott Ritter | 26 | 25 | `ritter` | [open](https://scottritter.substack.com) | `2026-01-03` | `2026-06-10` |
| `parsi` | Trita Parsi | 23 | 21 | `parsi` | [open](https://tritaparsi.substack.com) | `2026-01-09` | `2026-06-17` |
| `jiang` | Professor Jiang (Predictive History) | 2 | 2 | `jiang` | [open](https://predictivehistory.substack.com) | `2026-05-23` | `2026-06-13` |

## Return

- YouTube roster: [channel-index.md](./channel-index.md)
- Spec: [writer-index-spec.md](./writer-index-spec.md)
- Root archive: [source-archive/statecraft/README.md](./README.md)
