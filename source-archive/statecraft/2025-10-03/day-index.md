# Statecraft Archive - Day Index - 2025-10-03

_Generated inventory note. Rebuild with `python scripts/build_statecraft_day_indices.py`._

## Stats

- Source files: `2`
- Channel sources: `2`
- Writer sources: `0`
- Other sources: `0`
- Helper notes (excluded from source count): `0`
- Body kind mix: `transcript` (2)
- Source form mix: `interview` (2)
- Distinct channels/shows: `1`
- Distinct hosts: `1`
- Distinct guests: `3`
- Distinct threads: `4`

## Channel / Show Rollup

- `Dialogue Works` (2)

## Host / Guest / Thread Rollup

- Hosts: `Nima Alkhorshid` (2)
- Guests: `Larry C. Johnson` (1), `Lawrence Wilkerson` (1), `Martin Armstrong` (1)
- Threads: `alkhorshid` (2), `armstrong` (1), `johnson` (1), `wilkerson` (1)

## Channel sources

_YouTube / check-sources roster captures for this day (`is_youtube_capture`)._

| Channel slug | Show | Thread | Source link | File |
| --- | --- | --- | --- | --- |
| `dialogue-works` | Dialogue Works | `alkhorshid`, `johnson`, `wilkerson` | [watch](https://www.youtube.com/watch?v=8x0SXBeMxm8) | `source-dialogue-works-larry-c-johnson-col-larry-wilkerson-charlie-kirk-update-iran-war-fallout-putin-ignores-trump-2025-10-03.md` |
| `dialogue-works` | Dialogue Works | `alkhorshid`, `armstrong` | [watch](https://www.youtube.com/watch?v=M_fdBX9yM7M) | `source-dialogue-works-martin-armstrong-trump-taking-us-into-world-war-iii-2025-10-03.md` |

## Writer sources

_Configured prose / check-written roster captures for this day._

| Writer slug | Thread | Feed host | Source link | File |
| --- | --- | --- | --- | --- |
| _none_ | — | — | — | — |

## Other sources

_Prose or archive lands outside channel-index and writer-index rosters._

| Voice / show | Thread | Source link | File |
| --- | --- | --- | --- |
| _none_ | — | — | — |

## Files

- `source-dialogue-works-larry-c-johnson-col-larry-wilkerson-charlie-kirk-update-iran-war-fallout-putin-ignores-trump-2025-10-03.md`
- `source-dialogue-works-martin-armstrong-trump-taking-us-into-world-war-iii-2025-10-03.md`
