# Statecraft Archive - Day Index - 2026-03-01

_Generated inventory note. Rebuild with `python scripts/build_statecraft_day_indices.py`._

## Stats

- Source files: `9`
- Channel sources: `4`
- Writer sources: `4`
- Other sources: `1`
- Helper notes (excluded from source count): `0`
- Body kind mix: `operator-transcript` (3), `substack-post` (3), `transcript` (2), `cleaned-transcript` (1)
- Source form mix: `interview` (4), `newsletter` (4), `solo` (1)
- Distinct channels/shows: `5`
- Distinct hosts: `3`
- Distinct guests: `5`
- Distinct threads: `8`

## Channel / Show Rollup

- `Glenn Diesen` (2), `escalationtrap.substack.com` (2), `Alexander Mercouris` (1), `Glenn Diesen (channel / program — pin canonical series title when known)` (1), `simplicius76.substack.com` (1)

## Host / Guest / Thread Rollup

- Hosts: `Glenn Diesen` (2), `Robert Pape` (2), `Alexander Mercouris` (1)
- Guests: `Alexander Mercouris` (1), `Chas Freeman` (1), `Commodore Steve Jermy` (1), `Daniel Davis` (1), `Scott Ritter` (1)
- Threads: `diesen` (2), `mercouris` (2), `pape` (2), `davis` (1), `freeman` (1), `jermy` (1), `ritter` (1), `simplicius` (1)

## Channel sources

_YouTube / check-sources roster captures for this day (`is_youtube_capture`)._

| Channel slug | Show | Thread | Source link | File |
| --- | --- | --- | --- | --- |
| `alexander-mercouris` | Alexander Mercouris | `mercouris` | [kUy3soYsmKY](https://www.youtube.com/watch?v=kUy3soYsmKY) | `source-alex-mercouris-iran-defiant-after-khamenei-death-picks-new-leaders-closes-hormuz-contin-2026-03-01.md` |
| `glenn-diesen` | Glenn Diesen | `diesen`, `freeman` | [watch](https://www.youtube.com/watch?v=Xt2e_79Rua4) | `source-glenn-diesen-chas-freeman-the-war-against-iran-could-destroy-the-u-s-republic-2026-03-01.md` |
| `glenn-diesen` | Glenn Diesen | `jermy`, `diesen` | [ALZxMPIjg6M](https://www.youtube.com/watch?v=ALZxMPIjg6M) | `source-glenn-diesen-steve-jermy-iran-war-endgame-global-fallout-2026-03-01.md` |
| `alexander-mercouris` | — | `mercouris` | [watch](https://www.youtube.com/watch?v=TBD-mercouris-2026-03-01) | `source-khamenei-strike-hormuz-russia-2026-03-01.md` |

## Writer sources

_Configured prose / check-written roster captures for this day._

| Writer slug | Thread | Feed host | Source link | File |
| --- | --- | --- | --- | --- |
| `pape` | `pape` | `escalationtrap.substack.com` | [watch](https://escalationtrap.substack.com/p/smart-bomb-trap-confirmed-decapitation) | `source-pape-smart-bomb-trap-confirmed-decapitation-2026-03-01.md` |
| `pape` | `pape` | `escalationtrap.substack.com` | [watch](https://escalationtrap.substack.com/p/what-vox-couldnt-publish) | `source-pape-what-vox-couldnt-publish-2026-03-01.md` |
| `ritter` | `ritter` | `scottritter.substack.com` | [watch](https://scottritter.substack.com/p/regime-change-the-double-edged-sword) | `source-ritter-regime-change-double-edged-sword-2026-03-01.md` |
| `simplicius` | `simplicius` | `simplicius76.substack.com` | [watch](https://simplicius76.substack.com/p/all-hell-set-to-break-loose-irgc) | `source-simplicius-all-hell-set-to-break-loose-irgc-2026-03-01.md` |

## Other sources

_Prose or archive lands outside channel-index and writer-index rosters._

| Voice / show | Thread | Source link | File |
| --- | --- | --- | --- |
| Daniel Davis | `davis` | — | `source-glenn-diesen-daniel-davis-iran-miscalculation-2026-03-01.md` |

## Files

- `source-alex-mercouris-iran-defiant-after-khamenei-death-picks-new-leaders-closes-hormuz-contin-2026-03-01.md`
- `source-glenn-diesen-chas-freeman-the-war-against-iran-could-destroy-the-u-s-republic-2026-03-01.md`
- `source-glenn-diesen-daniel-davis-iran-miscalculation-2026-03-01.md`
- `source-glenn-diesen-steve-jermy-iran-war-endgame-global-fallout-2026-03-01.md`
- `source-khamenei-strike-hormuz-russia-2026-03-01.md`
- `source-pape-smart-bomb-trap-confirmed-decapitation-2026-03-01.md`
- `source-pape-what-vox-couldnt-publish-2026-03-01.md`
- `source-ritter-regime-change-double-edged-sword-2026-03-01.md`
- `source-simplicius-all-hell-set-to-break-loose-irgc-2026-03-01.md`
