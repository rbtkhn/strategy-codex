# Statecraft Archive - 2026-05-21

_Generated inventory note. Rebuild with `python scripts/build_statecraft_day_indices.py`._

## Stats

- Source files: `6`
- Type mix: `transcript` (5), `predictive` (1)
- Distinct channels/shows: `3`
- Distinct hosts: `3`
- Distinct guests: `4`
- Distinct threads: `5`

## Channel / Show Rollup

- `Dialogue Works` (3), `Glenn Diesen` (1), `Judging Freedom` (1)

## Host / Guest / Thread Rollup

- Hosts: `Nima Alkhorshid` (3), `Andrew Napolitano` (1), `Glenn Diesen` (1)
- Guests: `Douglas Macgregor` (2), `Alex Krainer` (1), `Andrei Martyanov` (1), `Pepe Escobar` (1)
- Threads: `[diesen, macgregor]` (1), `escobar` (1), `krainer` (1), `macgregor` (1), `martyanov` (1)

## Filename Family Fallbacks

- `predictive-history-*` (1)

## Files

- `predictive-history-game-theory-27-putin-enters-the-chat-2026-05-21.md`
- `transcript-dialogue-works-alex-krainer-the-middle-east-and-europe-just-shifted-overnight-2026-05-21.md`
- `transcript-dialogue-works-andrei-martyanov-hezbollah-annihilates-air-defense-its-all-blowing-up-2026-05-21.md`
- `transcript-dialogue-works-pepe-escobar-iran-wipes-out-trumps-attack-russia-chinas-massive-move-2026-05-21.md`
- `transcript-macgregor-diesen-nato-attacked-russia-us-being-pushed-out-of-the-middle-east-2026-05-21.md`
- `transcript-napolitano-macgregor-the-pentagon-and-decapitation-strikes-2026-05-21.md`
