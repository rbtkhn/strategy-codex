...
**Working folders (ephemeral):** For heavy multi-file EXECUTE, the [**context-folder-assembly**](../skills-portable/_drafts/context-folder-assembly/SKILL.md) draft ritual assembles bounded task context under `prepared-context/working/<slug>/` (gitignored). PLAN uses [questions-as-spec](../docs/skill-work/questions-as-spec-template.md); shipped work still commits to normal repo paths. Orthogonal to bridge (session continuity) and harvest (midstream paste).
