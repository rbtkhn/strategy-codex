...
Operator WORK lane. **Not** Record / Voice knowledge. This file summarizes **naming**, **front matter**, **volume stubs**, and **`sources.yaml`** shape across Predictive History corpora under `continuity/predictive-history/`.
...
**Book-track operator docs:** `continuity/predictive-history/volume-ii-book-track-conventions.md` through `volume-vii-book-track-conventions.md` — should stay aligned with `metadata/book-architecture.yaml` and `metadata/source-map.yaml`.
...
- **`analysis_path`** — relative path under `continuity/predictive-history/` to the memo; must exist on disk when `status.analysis` is `complete`
