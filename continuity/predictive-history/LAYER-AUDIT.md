...
**Purpose:** Name the distinct functions currently bundled inside `continuity/predictive-history/` so later doctrine can point to the right **sub-template** instead of treating the whole tree as one reusable shape.
...
| **Pedagogy / voice** | Defines how Jiang’s material should be taught or explained in a Jiang-like classroom register. | `docs/templates/JIANG–VOICE.md`, `continuity/predictive-history/jiang-voice.md`, pedagogy tasks in `MULTI-AGENT.md` |
