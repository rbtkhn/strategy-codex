...
**Corpus:** `continuity/predictive-history/substack/essays/<slug>.md` — filename = Substack post slug (`/p/<slug>`). Each file has YAML front matter (`source_kind: substack_essay`, `publication_date`, `canonical_url`, …). See [substack/essays/README.md](../substack/essays/README.md).
...
**Analysis memos:** `continuity/predictive-history/analysis/essay-<slug>-analysis.md` — same memo sections as lecture analyses (`thesis`, `claims`, lattice, cross-refs). `normalize_analysis_frontmatter.py` does **not** auto-fill these (no YouTube id in filename).
