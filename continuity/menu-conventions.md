# Menu conventions (operator) (Cursor operator)
...
**Scope:** Every **assistant-authored** labeled multi-choice block — WORK closing menus, lens/mind offers, steward forks, and other **A–E** pivots. **Not** required when copying **verbatim** coffee / steward text from [menu-reference](continuity/coffee/menu-reference.md), or when the operator says **no menu** / **no options**.
...
**When:** post-commit close, conductor `bravo`, intake archive-checkpoint, signing-off Step 1 tail ([menu-reference — signing-off](continuity/coffee/menu-reference.md#signing-off-intent)).
