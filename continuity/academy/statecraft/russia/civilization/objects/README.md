# Russia Civilization Objects
<!-- word_count: 120 -->

This surface is non-authoritative and subject to revision.

Civilization objects hold long-pattern authority grammars for the Russia lane. They are not biographies and not current officeholder profiles. Use them to connect civilizational memory to state carriers, diplomatic transmitters, and transaction authority checks.

## Objects

- [State memory](state-memory.md) - deep authority memory from Rurik / Varangian-Novgorod order through Kievan Rus, Mongol pressure, Muscovy, Empire, Soviet state, 1991 rupture, and the current Putin / Security Council continuity surface.

## Use

Use a civilization object before treating a current leader, minister, or institutional voice as sufficient evidence that a Russia-lane instrument can be carried by the state. The object asks what deeper authority burden the current carrier is inheriting.
