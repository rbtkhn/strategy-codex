...
In repo terms, `civ-emp` is the compact working source base derived from that corpus.
...
6. **Review upstream on its own terms**
   - `civ-emp` should accept only what improves source memory, retrieval, or historical mechanism rather than whatever happened to be rhetorically useful in one draft
