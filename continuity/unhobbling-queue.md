...
_Add rows above the example row, or remove the example row when real ids are present._

**Columns (v1.2):** `id` (slug), `item`, `what_changed` (one line), `evidence` (required), `date_noted` (`YYYY-MM-DD`), optional `status` (`open` \| `watch` \| `invest` \| `reject` \| `dropped`), optional `notes` (one line).
