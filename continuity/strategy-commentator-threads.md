...
**Compatibility files (mixed location model):** Each indexed stream handle keeps a year independent channel profile plus time scoped companion files. The old `experts/<expert_id>/` wording is legacy compatible routing language, not the current canonical profile home:
  ...
  **`continuity/profiles/<channel> profile.md`** â€” **cognitive profile** (operator authored, stable, year independent). Identity, convergence/tension fingerprints, signature mechanisms, failure modes, weave cues, introduction, and link hub.
...
**Published outlets (starter list):** Each **`continuity/profiles/<channel> profile.md`** includes **`## Links`** with **`### Social media`**, **`### Substack`**, and **`### Other links`** â€” at least three useful source surfaces when available; re verify handles and media URLs before cite grade use.
