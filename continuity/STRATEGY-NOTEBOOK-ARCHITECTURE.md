...
> [!IMPORTANT]
> Path contract update: `continuity/years/2026/` is no longer a live people-lane volume. It is reserved for dated provenance via `source-archive/statecraft/`. Canonical people shelves live under `statecraft/voices/<name>/` and host-family continuity under `statecraft/channels/<name>/`, with host-local guest arcs as flat files under each channel shelf. Legacy `continuity/speakers/` is terminated — [codex-speakers-deprecated.md](../docs/archive/codex-speakers-deprecated.md).

> [!NOTE]
> **Strategy-notebook namespace deprecated (2026-06-23):** Say **`strategy-codex`** / **`continuity/`** for new work. Compat **`docs/.../strategy-notebook/`** is redirect-only. Full spec: [STRATEGY-NOTEBOOK-DEPRECATED.md](../docs/archive/skill-work-legacy/work-strategy/STRATEGY-NOTEBOOK-DEPRECATED.md). Bulk inline **`strategy-notebook`** path tokens in this file are being retired in a follow-up pass.

> [!NOTE]
> **Raw-input deprecated (2026-06-23):** Verbatim strategy capture SSOT is **`source-archive/statecraft/`** via **`source-intake`**, not **`continuity/raw-input/`**. Inbox = registry; notebook = synthesis. Full spec: [RAW-INPUT-DEPRECATED.md](../docs/archive/skill-work-legacy/work-strategy/RAW-INPUT-DEPRECATED.md). Bulk inline `raw-input/` mentions in this file are being retired in a follow-up pass.

**Relation to `skill-strategy`:** [`.cursor/skills/skill-strategy/SKILL.md`](../docs/archive/skill-work-legacy/work-strategy/SKILL-STRATEGY-DEPRECATED.md) is the **activation surface** for **`strategy`**. **This document**, [NOTEBOOK-PREFERENCES.md](NOTEBOOK-PREFERENCES.md), and [daily-strategy-inbox.md](daily-strategy-inbox.md) (paste-ready line SSOT) are **incorporated by reference** into that skill ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â **one contract**, split across files for readability and maintenance, **not** a parallel ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã¢â‚¬Å“architecture-onlyÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â track beside the skill.
...
**Strategy-state model (short):** In strategy-codex, **knowledge** means governed understanding and owned judgment; **library** means the governed reference world; **memory** means resumable continuity; **archive** means the governed evidence and provenance spine. In practice, `source-archive/statecraft/` is the dated source archive, codex-pages bridge archive toward knowledge, `strategy-page` blocks are the main knowledge-production surface, and `days.md` is primarily memory.

**Two-layer SSOT (capture vs notebook work):** **[`source-archive/statecraft/`](../source-archive/statecraft/README.md)** holds the **literal** author/host/guest text (archived capture). **codex-pages** in the owning **`continuity/<year>/<channel>/`** folder per [`strategy-codex-template-page.md`](strategy-codex-template-page.md) are the **default handle** for **strategy-page**, **`thread`**, and **`days.md`** ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â lane, sibling links, and **`### Verbatim`** on the page. **Disputes on wording** ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ correct the source archive and the pageÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢s **`### Verbatim`** together. **Disputes on judgment** ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ **`### Reflection` / `### Predictive Outlook`** and downstream compose, not a second paraphrase of the raw in **`thread`**.
...
1. **Accumulate** through the day in [`daily-strategy-inbox.md`](daily-strategy-inbox.md) (paste-ready stubs) and **[`source-archive/statecraft/`](../source-archive/statecraft/README.md)** (full verbatim when needed) ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â see [Split ingest model](#split-ingest-model) and [source-archive/statecraft/README.md](../source-archive/statecraft/README.md).
2. **Once per local day (default): end-of-day strategy session** ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â **compose or revise** thread-embedded pages (`<!-- strategy-page:start` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¦ in time-scoped thread files under `continuity/years/2026/<channel>/` and their `## YYYY-MM` segments) and the matching `chapters/YYYY-MM/days.md` continuity block. **Inputs (operator read order):** that dayÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢s **[`source-archive/statecraft/`](../source-archive/statecraft/README.md)** files (**primary bulk verbatim**), **thread-embedded** **`strategy-page`**, inbox stubs, and briefs. **Optional context after `thread`:** 7-day **`transcript.md`** and the **machine layer** (inbox triage **+** source path pointers in inbox ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â **not** a second read SSOT). **Synthesis** (required **Chronicle / Reflection / Predictive Outlook** prose; **`### Appendix`** optional ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â see [strategy-codex-template-page.md](strategy-codex-template-page.md)), **not** an inbox or raw-input mirror. **Breaking glass:** rare intra-day notebook compose only when the operator explicitly overrides ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â same synthesis discipline.
...
**Rule of thumb:** **Inbox + `raw-input/`** = capture; **pages** (marker-fenced blocks in time-scoped author thread files under `continuity/years/2026/<channel>/`) = judgment after the **end-of-day strategy session**; **`days.md`** = chronology and continuity.
...
**End-of-day strategy session** (abbrev. **EOD strategy session**) is the **default sole window** for composing or revising **strategy pages** (thread-embedded `strategy-page` blocks in time-scoped thread files under `continuity/years/2026/<channel>/`) and updating **`chapters/YYYY-MM/days.md`**. **Primary evidence pile (operator consult):** that calendar dayÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢s **[`source-archive/statecraft/YYYY-MM-DD/`](../source-archive/statecraft/README.md)** and **author** **`strategy-page`** work in **`thread`**, **plus** inbox and **`batch-analysis`**. The **7-day** **`transcript.md`** and **machine layer** are **automation and triage** (optional extra context after **`thread`**); they are **not** a parallel SSOT you must open for the authorÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢s long verbatim if the source archive and pages already hold it ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â **synthesized**, not copied.
...
1. **Triage** (internal, not operator-facing) ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â routes **`thread:<expert_id>`** lines from **`daily-strategy-inbox.md`** to per-author, time-scoped transcript files under **`continuity/years/2026/<channel>/`** (append-only, 7-day rolling window, auto-pruned). The rolling file is a **triage sink**; **durable** verbatim for reading belongs in **[`source-archive/statecraft/`](../source-archive/statecraft/README.md)**. Operator may lightly edit transcripts for clarity; edits are preserved across runs.
2. **Extraction** ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â reads each author's **`transcript.md`**, **scanning the inbox** for **`raw-input/ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¦` paths** on the same `thread:` lines, and lists **`strategy-page`** blocks in that authorÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢s **thread file(s)**, then writes **machine-maintained** material **between** the HTML comment markers (see below). **Canonical layout:** time-scoped transcript and thread files live under **`continuity/years/2026/<channel>/`**. Thread files are month-bounded when split that far; legacy multi-month containers may still coexist during migration. Transcript lines and inbox **`raw-input/`** pointers are scoped to the matching **`YYYY-MM`**; optional legacy **index** file rows (if any) attach to the **current UTC month** file only. **Human narrative** lives **outside** that block.
...
**Three capture channels** (direct write vs **`populate_strategy_raw_input`** vs Cursor JSONL salvage vs RSS): see [Statecraft Source Archive](../source-archive/statecraft/README.md) for the canonical capture surface; **prune** is optional hygiene, not recovery.

**Problem:** **Durable** long verbatim for **reading and Chronicle** belongs in **[`source-archive/statecraft/`](../source-archive/statecraft/README.md)**, but **unified discovery** still needs a **grep-friendly registry** and a **single habitual action** (see operator preference: stub + corpus + one command). The **7-day** **`transcript.md`** is the **triage** target for `thread:` lines, **not** the operatorÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢s only bulk store.

### Publication vocabulary (formal pin)

Normative: the canonical source archive under **[`source-archive/statecraft/`](../source-archive/statecraft/README.md)** uses the **`pub_date:YYYY-MM-DD`** machine tag, YAML **`pub_date`**, and human **Published** / **publication day** wording rather than **`aired:`** for new work. Legacy inbox lines and the **`_aired-pending/`** folder name may persist until a bulk migration. **[`strategy-codex-template-page.md`](strategy-codex-template-page.md)** preambles follow the same pin.
...
| **`continuity/years/2026/<channel>/<channel>-transcript.md`** | **Rolling triage / machine echo (7 days)** ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â optional short or pointer-only blocks under **`## YYYY-MM-DD`** where that date is the **publication** day; continuation paragraphs follow the [long-form verbatim](daily-strategy-inbox.md#long-form-verbatim-thread) rules. **Full** unabridged text stays in **[`source-archive/statecraft/`](../source-archive/statecraft/README.md)**; this file is **not** a second SSOT the operator must consult for bulk verbatim. Triage targets **ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â°Ãƒâ€šÃ‚Â¤ ~2000 words** per ingest block (soft caps). |
| **[`source-archive/statecraft/`](../source-archive/statecraft/README.md)** | **Unabridged SSOT** ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â full transcripts, RSS merge files **`YYYY-MM-DD-<expert_id>.md`**, sidecars, bundles. **On disk:** **`source-archive/statecraft/<pub_date>/`** (publication / calendar day; **`ingest_date`** in YAML only). **`_aired-pending/`** remains the legacy pending folder name when **`pub_date`** is not fixed yet. **codex-pages** live in the owning **`continuity/<year>/<channel>/`** folder (including **`-<slug>`** suffixes when multiple pages share a date ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â see split-ingest **codex-pages** note below); **not** in this tree ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â inbox stays **stubs** + **`verify:`** pointers. **Prune** only when **you** run **`prune_strategy_raw_input.py`** (default window aligns with author transcript tooling); **`--apply`** is **blocked** while **[`.pruning-suspended`](raw-input/.pruning-suspended)** exists unless **`--override`**. |
...
**codex-pages (standalone files in the owning year/channel folder):** **Multiple** codex-pages may share the same **publication date** (same calendar anchor as YAML **`pub_date`**). Default basename **`<expert_id>-page-YYYY-MM-DD.md`**; when more than one primary capture warrants a separate judgment artifact for that date, use **`<expert_id>-page-YYYY-MM-DD-<slug>.md`** with **`<slug>`** from the primary **`raw-input`** stem (kebab-case). Store those files in the owning **`continuity/<year>/<channel>/`** folder. **Alternatively,** the operator may consolidate same-day captures into **one** file with **A / B / C** Verbatim blocks where [strategy-codex-template-page.md](strategy-codex-template-page.md) defines that pattern. **Canonical scaffold:** [strategy-codex-template-page.md](strategy-codex-template-page.md). **Per-expert compat stub:** **`<expert_id>-page-template.md`** (links the canonical file; stable **`Template:`** pointer in **`transcript.md`**).

**Unified search:** Treat **inbox + `continuity/years/2026/*/*-transcript.md` + `raw-input/`** as one habit: grep inbox for stubs; grep **`raw-input`** for full text when the operator stored it there; grep transcripts for triage-sized verbatim ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â **or** (future) a generated rollup listing pointers only. **Not** Record.
...
**Purpose:** Fit a long author transcript into the **ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â°Ãƒâ€šÃ‚Â¤ ~2000 words per ingest** target for the time-scoped transcript file under `continuity/years/2026/<channel>/` **without changing any of the authorÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢s words** ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â by **dropping whole sentences only**, selected for **argument weight**.
...
| **Strategy-page** | `<!-- strategy-page:start` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¦ `<!-- strategy-page:end -->` in time-scoped thread files under `continuity/years/2026/<channel>/` | **Author-thread judgment** unit (primary written unit for **EOD compose**); **many** per month are normal; multi-author sessions reuse the same **`id=`** across files. |
...
**Legacy library mirror:** Canonical files live here under `docs/archive/skill-work-legacy/work-strategy/strategy-notebook/`. A symlink under `strategy-notebook` is a legacy convenience surface only ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â keep mirrors in sync with edits to the canonical tree.
...
**Shared date key (work-cici):** The [Cici notebook](../README.md) uses the same **`YYYY-MM-DD`** identifier for daily files (`YYYY-MM-DD.md`). Strategy pages stay in month `days.md` (or `pages/YYYY-MM-DD.md`); the Cici notebook files live in `cici-notebook/` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â same calendar key, different folder and purpose.

## Author choreography

**Two planes:**

1. **Commentator / author threads** ([strategy-commentator-threads.md](strategy-commentator-threads.md)) Ã¢â‚¬â€ **longitudinal** lanes: what each named voice said over time so you can track **accuracy**, **narrative drift**, and **compare-contrast** across authors. Ingests use **`thread:<expert_id>`** (see that file). This is the **bookkeeping and evidence** plane for *who said what*. Each main channel has a year-independent profile at `continuity/profiles/<channel>-profile.md`, while transcripts, thread files, and pages stay in the active volume folders such as `continuity/years/2026/<channel>/`. Run operator **`thread`** Ã¢â‚¬â€ `python3 scripts/strategy_thread.py` Ã¢â‚¬â€ to auto-triage and extract.
...
**Per-author rolling mirror:** Ingest lines that carry **`thread:<expert_id>`** are automatically triaged into the time-scoped transcript file under `continuity/years/2026/<channel>/` and extracted for thread distillation into the matching thread file via **`python3 scripts/strategy_thread.py`** (operator **`thread`**); crossing rules and optional `verify:` tails stay in [strategy-commentator-threads.md](strategy-commentator-threads.md).
...
Dashed edge: operator-authored [history-notebook](../README.md) chapters supply **durable pattern IDs** and arcs; **thread-embedded pages** cite them in **`### History resonance`** (Lineage section) ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â not a second corpus dump.
...
**Maintenance:** When you move the active month in this notebook or change Predictive History queue / volume emphasis, update **`chapters/YYYY-MM/meta.md` ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§ Polyphony** and **`continuity/predictive-history/operator-polyphony.md`** in the **same session** so strategy-codex (`continuity/`) and predictive-history (`continuity/predictive-history/`) stay parallel. Do **not** put the polyphony overlay only in `continuity/predictive-history/STATUS.md` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â that file is **generated** by `scripts/work_jiang/render_status_dashboard.py`.

## Parallel to History notebook (`docs/archive/skill-work-legacy/work-strategy/history-notebook/`)

**North star:** civilization_memory is the **reservoir**; [History Notebook](../README.md) chapters (`hn-*` ids in [book-architecture.yaml](../history-notebook/book-architecture.yaml)) are **operator-distilled** mechanism text; this notebook holds **dated judgment** and **`### History resonance`** pointers (never full HN paste).

### HN and strategy-codex: one-way citations

**Draw only, no reverse path:** this notebook (daily pages, `days.md`, author threads) may **cite** History notebook ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â `hn-*` chapter ids, a single mechanism or warrant line, and optionally a civilization-thread `MC-*` tag when that tagÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢s SSOT remains in a `history-civ-*.md` file. There is **no** automatic sync, merge, or back-pressure that **writes** HN threads or `hn-*` draft text from here. [History notebookÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢s governed surfaces](../README.md#work-record-and-the-strategy-notebook) change only through **operator-originated** edits in `docs/archive/skill-work-legacy/work-strategy/history-notebook/` (WORK). Treat strategy judgment as a **reader** of HN, not an editor of it.
...
| [STATUS.md ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â distillation queue](STATUS.md) (single SSOT for ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã¢â‚¬Å“next `hn-*` to draftÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â) | **`chapters/YYYY-MM/meta.md`** ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â link to that queue (one line); **do not** maintain a second priority list here |
...
**Recursive loop:** **`HN gap:`** lines feed the single queue in [STATUS.md](STATUS.md). **Monthly strip** (manual): count `hn-*` cites vs deferred vs HN gap lines; reorder the queue. **Coverage coupling:** when [cross-book-map.yaml](../history-notebook/cross-book-map.yaml) bumps a thesis/concept row from **`stub` ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ `partial`**, add **at least one** **`### History resonance`** line in strategy that month so ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã¢â‚¬Å“HN advancedÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â and ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã¢â‚¬Å“strategy usedÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â stay linked.

**Volume priority:** Pick **one** primary drafting lane first (e.g. Vol V contemporary for same-day relevance, or Vol I problem spine for comparative mechanisms)ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Ânot all five eras at once. Record the choice in [STATUS.md](STATUS.md).

**Differentiator:** Most ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã¢â‚¬Å“strategic intelligenceÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â stacks aggregate **news**; this pair aggregates **dated judgment** (here) + **operator-owned mechanism library** (HN). See [history-notebook README](../README.md).
...
- **Tier 1:** **chapter id(s)** from [history-notebook](../README.md) (e.g. `hn-i-v1-04`) + **mechanism or arc** (Persian, Roman, ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¦) + link. **Tier 2:** MEM/STATE needed first ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â add **`HN gap: <mechanism> ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ hn-ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¦ (stub)`** (feeds [STATUS queue](STATUS.md)). **Tier 3:** surface / case-index only ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â label the tier. If the parallel is load-bearing for public or Islamabad copy, flag [analogy-audit](../analogy-audit-template.md). Use **none** or **deferred** if no wire this pass ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â same honesty rule as Jiang.
...
| **History resonance (optional)** | Chapter id(s) from [history-notebook](../README.md) when judgment uses durable mechanism language ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â not a second book dump. |
...
[`.cursor/skills/skill-strategy/SKILL.md`](../docs/archive/skill-work-legacy/work-strategy/SKILL-STRATEGY-DEPRECATED.md) defines how agents run a **strategy pass**. **Notebook remains primary**; three ideas belong in this architecture:
...
| **Grep-first passes** | Quick hygiene before deeper work | Search `verify:` in [`daily-strategy-inbox.md`](daily-strategy-inbox.md); `### Web verification` / `Primary pulls` in `days.md`; **http(s)** URLs for link rot; **proper nouns** (rosters, titles) aligned with **+ verify** / transcript capture rules in [skill-strategy](../docs/archive/skill-work-legacy/work-strategy/SKILL-STRATEGY-DEPRECATED.md). |
...
**History notebook (`docs/archive/skill-work-legacy/work-strategy/history-notebook/`)** ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â optional **`### History resonance`**: cite **chapter id(s)** and mechanism when judgment leans on [history-notebook](../README.md) patterns; **never** paste full HN chapters in-page. If no chapter applies, **none** or **deferred** beats filler. Validates against **cross-book-map** / arcs when the operator cares about coverage ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â **WORK only**, not Record.

#### Cross-artifact alignment (planes and layers)

- [Transcript digest planes](../docs/archive/skill-work-legacy/work-strategy/transcript-analysis-haiphong-ritter-johnson-iran-2026-04.md) (**A** negotiation scope ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· **B** material / Hormuz ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· **C** narrative) and [work-strategy-rome notes](../work-strategy-rome/notes/2026-04-03-modern-rome-papacy-thesis-stub.md) (**two layers ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â do not collapse**) share one habit: **document coupling** between registers, do not **merge planes in one sentence** without tagging (same discipline as **dual-register** Lebanon lines in `days.md`).
...
- **`meta.md`** holds slow-moving logic: **Theme**, **Open questions** spanning weeks, **Bets / watches** for month-end review, optional **Polyphony / lens tension** (see below), optional **one line** linking [History Notebook STATUS ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â distillation queue](STATUS.md) (single queue SSOT ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â do not duplicate a second HN priority list here). Touch `meta.md` when the **monthÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢s story** shifts, not necessarily every day.
...
- **Parallel PH overlay:** Keep in sync with [`continuity/predictive-history/operator-polyphony.md`](predictive-history/operator-polyphony.md) (same section structure); see **Parallel to Predictive History** table above.
...
The directory [`compiled-views/`](compiled-views/) holds **refreshable, non-authoritative** markdown snapshots for browsing (e.g. Obsidian) and handoff. They **do not** replace the source of truth: [`daily-strategy-inbox.md`](daily-strategy-inbox.md), [`source-archive/statecraft/`](../source-archive/statecraft/README.md), author [`transcript.md`](strategy-expert-template.md) files, journal-layer **`strategy-page`** blocks in author thread files, or [`chapters/YYYY-MM/days.md`](chapters/2026/2026-04/days.md) / `meta.md`.
...
- **Promotion:** non-authoritative; not recursion-gate/Record unless explicitly escalated.
...
- **Strategy session helpers** ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â compact index in [work-strategy README](../README.md#strategy-session-helpers-skill-strategy): Grok daily-brief layer, TrumpÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œreligionÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œpapacy arc, RomeÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œPersia legitimacy signal check, narrative-escalation stub, [skill-strategy SKILL](../docs/archive/skill-work-legacy/work-strategy/SKILL-STRATEGY-DEPRECATED.md).
