...
2. **`coffee_conductor_outcome`** in [cadence-events.md](../work-cadence/cadence-events.md) with `conductor=<slug>` + `verdict=` and `notebook_ref=` or `falsify=` (both preferred). If the pass closed without that line, add a repair outcome on the next turn rather than leaving attribution implicit.
