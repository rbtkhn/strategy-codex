...
| **Strategy notebook (strategy-codex (`continuity/`))** | **`dream` does not** own notebook production. Fold runs in **`strategy`** or on explicit **fold**; `auto_dream.py` may report **`strategy_notebook_missing_day_headers`** as optional FYI. See [STRATEGY-NOTEBOOK-ARCHITECTURE.md](../STRATEGY-NOTEBOOK-ARCHITECTURE.md) Â§ *Entry model* and [.cursor/skills/dream/SKILL.md](../../.cursor/skills/dream/SKILL.md) Â§ *Strategy notebook*. |
...
| `coffee_rollup_24h` | object | Rolling 24h summary of `coffee` lines from [`cadence-events.md`](../work-cadence/cadence-events.md) (`count`, `by_mode`, `by_picked`, `picks`, `first_ts`, `last_ts`, `runs`, â€¦) |
...
- **Lane breadcrumbs:** `continuity/dream/dream-history.md`
...
- [continuity/coffee/README.md](../work-coffee/README.md)
