...
- [`scripts/dream_coffee_rollup.py`](../../../scripts/dream_coffee_rollup.py) parses [`cadence-events.md`](../work-cadence/cadence-events.md) for `coffee` lines in a rolling UTC 24h window.
...
- Added Maintenance mode to AGENTS.md Operating Modes table — dream touches memory, runs integrity/governance, emits pipeline events, so it should be visible in the guardrails doc.
- Classified `last-dream.json` as runtime noise in `operator_handoff_check.py`.
- Created this territory (`continuity/dream/`) paralleling `work-coffee` — skill holds the executable contract, territory holds doctrine, boundaries, and history.
