#!/usr/bin/env python3
"""Refresh the vendored public/ph-civ workspace copy from rbtkhn/ph-civ (inbound pull only)."""
...
def write_receipt(dest: Path, upstream_sha: str, branch: str) -> None:
    text = (
        ...
        "Workspace staging copy. Edit under `public/ph-civ/`; push upstream only via publish script.\n"
    )
    (dest / RECEIPT_NAME).write_text(text, encoding="utf-8")


def sync(branch: str, fetch: bool) -> dict:
    with tempfile.TemporaryDirectory(prefix="ph-civ-sync-") as tmp:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = sync(branch=args.branch, fetch=True)
