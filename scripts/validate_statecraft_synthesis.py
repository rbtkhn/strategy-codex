#!/usr/bin/env python3
"""Validate the narrow structural contract for ``statecraft/synthesis`` day/month notes.
...
DEFAULT_SYNTHESIS_ROOT = REPO_ROOT / "statecraft" / "synthesis"
DEFAULT_DAY_DIR = DEFAULT_SYNTHESIS_ROOT / "day"
DEFAULT_MONTH_DIR = DEFAULT_SYNTHESIS_ROOT / "month"
# Legacy alias
DEFAULT_DAILY_DIR = DEFAULT_DAY_DIR
...
def validate_synthesis_dirs(day_dir: Path, month_dir: Path) -> list[str]:
    ...
    if not day_dir.is_dir():
        errors.append(f"{day_dir}: missing day synthesis directory")
    else:
    ...
    if not month_dir.is_dir():
        errors.append(f"{month_dir}: missing month synthesis directory")
    else:
        for path in sorted(month_dir.glob("*.md")):
            if MONTHLY_FILENAME_RE.fullmatch(path.name):
...
def validate_daily_dir(daily_dir: Path) -> list[str]:
    ...
    month_dir = daily_dir.parent / "month" if daily_dir.name == "day" else daily_dir
    if daily_dir.name == "day" and month_dir.is_dir():
        return validate_synthesis_dirs(daily_dir, month_dir)
    errors: list[str] = []
        ...
        for path in sorted(day_dir.glob("*.md")):
            if DAILY_FILENAME_RE.fullmatch(path.name):
                ...
                if is_migrated_daily_text(text):
                    ...
                    legacy_daily += 1
    scan_month = month_dir if month_dir is not None else day_dir
    if scan_month.is_dir():
        for path in sorted(scan_month.glob("*.md")):
            if MONTHLY_FILENAME_RE.fullmatch(path.name):
                monthly += 1
...
def main() -> int:
    ...
    parser.add_argument(
        "--day-dir",
        type=Path,
        default=DEFAULT_DAY_DIR,
        help="Directory containing day synthesis notes (YYYY-MM-DD.md)",
    )
    parser.add_argument(
        "--month-dir",
        type=Path,
        default=DEFAULT_MONTH_DIR,
        help="Directory containing month synthesis notes (YYYY-MM.md)",
    )
    parser.add_argument(
        "--daily-dir",
        type=Path,
        default=None,
        help="Legacy alias for --day-dir",
    )
    args = parser.parse_args()

    day_dir = (args.daily_dir or args.day_dir).resolve()
    month_dir = args.month_dir.resolve()
    errors = validate_synthesis_dirs(day_dir, month_dir)
    if errors:
        for err in errors:
            print(f"error: {err}", file=sys.stderr)
        print(
            f"validate_statecraft_synthesis: {len(errors)} error(s)",
            file=sys.stderr,
        )
        return 1
