#!/usr/bin/env python3
"""Rebuild statecraft/voices/krapivnik/krapivnik-index.md from archive Krapivnik captures."""
...
OUT = REPO / "statecraft" / "voices" / "krapivnik" / "krapivnik-index.md"
...
GUEST_PREFIX_RE = re.compile(
    r"^(?:stanislav\s+)?krapivnik:\s*",
    re.I,
)

HOST_LABEL: dict[str, str] = {
    "nima": "Dialogue Works × Krapivnik",
    "diesen": "Diesen × Krapivnik",
    "neutrality": "Neutrality Studies × Krapivnik",
    "duran_mercouris": "Mercouris × Krapivnik",
    "duran_triple": "Duran × Mercouris × Krapivnik",
    "other": "Krapivnik",
}
...
def host_bucket(path: Path, meta: dict) -> str:
    ...
    if "dialogue-works" in name:
        return "nima"
    if "glenn-diesen" in name or name.startswith("source-diesen-krapivnik"):
        return "diesen"
    if "duran-mercouris" in name or "mercouris" in name and "krapivnik" in name:
        if "peace-deal" in name or "frontline-update" in name:
            return "duran_triple"
        return "duran_mercouris"
    host = (meta.get("host") or "").casefold()
    show = (meta.get("show") or "").casefold()
    slug = (meta.get("channel_slug") or "").casefold()
    if "neutrality studies" in show or slug == "neutrality-studies":
        return "neutrality"
    if "dialogue works" in show or slug == "dialogue-works":
        return "nima"
    if "glenn diesen" in host or slug == "glenn-diesen":
        return "diesen"
    if "duran" in show.casefold() or slug == "duran":
        return "duran_mercouris"
    return "other"
...
def collect_rows() -> list[tuple[str, Path, dict]]:
    ...
    for path in iter_archive_captures_for_shelf("krapivnik", ARCHIVE):
        meta = parse_head(path)
        body = path.read_text(encoding="utf-8")[:8000]
        if shelf_utils.shelf_capture_excluded("krapivnik", path, meta, body):
            continue
        pub = pub_date_key(meta, path)
...
def render_month_section(
    ...
    if month == "2026-06" and any(r[1].name == JUNE_HINGE_FN for r in section_rows):
        ...
        for pub, path, meta in section_rows:
            if path.name == JUNE_HINGE_FN:
                ...
                lines.append("")
                break
        other = [r for r in section_rows if r[1].name != JUNE_HINGE_FN]
        if other:
            lines.append("**Other June anchors:**")
            lines.append("")
...
def render_index(
    ...
    neutrality_n = sum(1 for r in rows if host_bucket(r[1], r[2]) == "neutrality")
    duran_n = sum(
        1
        for r in rows
        if host_bucket(r[1], r[2]) in ("duran_mercouris", "duran_triple")
    )
    ...
    lines = [
        ...
        "- **Surface role:** source bench for breadth-first Krapivnik retrieval",
        "- **Surface interior:** materialized guest appearances across Dialogue Works, Diesen, Neutrality Studies, Duran, Mercouris co-host",
        "",
        "**Identity / voice hub:** [krapivnik-profile.md](krapivnik-profile.md)",
        "",
        "## Corpus note",
        "",
        f"- **{len(rows)}** eligible archive captures on disk ({date_span})",
        "- Rebuild: `python scripts/build_krapivnik_index.py`",
        f"- **{nima_n}** Dialogue Works/Nima · **{diesen_n}** Diesen · **{neutrality_n}** Neutrality Studies · **{duran_n}** Duran/Mercouris",
