#!/usr/bin/env python3
"""Generate runtime/artifacts/prediction-disagreement.json from prediction registry."""
...
DEFAULT_OUTPUT = REPO_ROOT / "runtime" / "artifacts" / "prediction-disagreement.json"

_SCRIPTS = Path(__file__).resolve().parent
if str(_SCRIPTS) not in sys.path:
    sys.path.insert(0, str(_SCRIPTS))

from prediction_lib import build_disagreement_payload, render_json  # noqa: E402
...
def check_artifact(*, output_path: Path, registry_path: Path) -> int:
    ...
    expected = render_json(build_disagreement_payload(registry))
    current = output_path.read_text(encoding="utf-8")
    if current != expected:
        print(
            f"error: {output_path.relative_to(REPO_ROOT)} is out of date; "
            "run build_prediction_disagreement.py",
            file=sys.stderr,
        )
        return 1
    print("[ok] prediction disagreement artifact matches generator output")
    return 0
...
def main() -> int:
    ...
    payload = build_disagreement_payload(registry)
