...
def transform_index_public(text: str, manifest: dict, volume_slugs: dict[str, str]) -> str:
    ...
    text = re.sub(
        ...
        "](essays/high-skill-labor-compression",
        text,
    )
    text = re.sub(
        r"\]\(essays/high-skill-labor-compression",
        "](essays/high-skill-labor-compression",
        text,
    )
...
def finalize_public_markdown(text: str, dest_rel: Path, volume_slugs: dict[str, str]) -> str:
    ...
    text = re.sub(
        r"\]\((?:\.\./)*comparative/continuity-mechanism\.md\)",
        f"]({continuity})",
        text,
    )
    text = re.sub(
    ...
    text = re.sub(
        r"\]\((?:\.\./)*comparative/(high-skill-labor-compression[^)]+\))",
        rf"]({prefix}essays/\1",
        text,
    )
    text = re.sub(
        r"\]\((?:\.\./)*essays/(hormuz-recognition[^)]+\))",
        rf"]({prefix}essays/\1",
        text,
    )
    text = re.sub(
        r"\]\((?:\.\./)*comparative/(hormuz-recognition[^)]+\))",
        rf"]({prefix}essays/\1",
        text,
    )
