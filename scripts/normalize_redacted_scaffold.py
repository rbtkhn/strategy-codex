#!/usr/bin/env python3
"""Normalize Redacted News archive scaffold in place.

Conservative lanes (mirrors Breaking Points / Davis close_promo pattern):
  1. close_promo — subscribe/share CTA tail (Clayton/Natali standard outro)
...
CLOSE_PROMO_ANCHOR_RES: list[tuple[str, re.Pattern[str]]] = [
    (
        "watching-redacted",
        re.compile(
            r"(?:>>\s*|\*\*Host:\*\*\s*)?Thank you so much for watching Redacted",
            re.IGNORECASE,
        ),
    ),
    (
        "subscribe-channel",
        re.compile(
            r"(?:>>\s*)?We(?:'d| would) love for you to subscribe to the channel",
            re.IGNORECASE,
        ),
    ),
    (
        "share-video-friend",
        re.compile(
            r"please consider sharing this video with a friend or a loved one",
            re.IGNORECASE,
        ),
    ),
    (
        "see-you-next-time",
        re.compile(
            r"Thanks so much and we(?:'ll| will) see you next time",
            re.IGNORECASE,
        ),
    ),
]
...
EDITORIAL_CLOSE_NOTE = (
    "Routine Redacted subscribe/share close trimmed in place; SSOT body otherwise preserved."
)
...
def is_redacted_capture(meta: dict[str, Any], path: Path) -> bool:
    name = path.name.lower()
    if name.startswith("source-redacted") or "-redacted-" in name:
        return True
    ...
    return (
        show == "redacted news"
        or channel == "redacted news"
        or channel_slug == "redacted-news"
        or "redacted news" in show
    )
...
def normalize_redacted(
    ...
    *,
...
) -> tuple[bool, str, RedactedChange | None]:
    meta, body = split_frontmatter(text)
    if not is_redacted_capture(meta, path):
    ...
    if allow_wrapper and not meta.get("redacted_wrapper_trim_applied"):
        ...
        if did:
            ...
            meta["redacted_wrapper_trim_applied"] = True
    ...
    patch_keys = {
        "editorial_note",
        "redacted_close_promo_trim_applied",
        "redacted_wrapper_trim_applied",
    }
...
def candidate_paths(root: Path, explicit: list[Path] | None = None) -> list[Path]:
    ...
    for path in root.rglob("*.md"):
        if ".cleaned." in path.name:
            continue
        if not (path.name.startswith("source-redacted") or "-redacted-" in path.name):
            continue
        try:
...
def main() -> int:
    ...
    parser.add_argument(
        "--force-close",
        action="store_true",
        help="Re-run close promo trim even when redacted_close_promo_trim_applied is set",
    )
    ...
    changes: list[RedactedChange] = []
    skipped = 0

    for path in paths:
        text = path.read_text(encoding="utf-8")
        changed, _, change = normalize_redacted(
            path,
            text,
            apply=args.apply,
    ...
    print(f"{mode}: {len(changes)} Redacted file(s); {skipped} skipped/no-op.")
