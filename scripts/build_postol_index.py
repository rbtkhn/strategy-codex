#!/usr/bin/env python3
"""Rebuild statecraft/voices/postol/postol-index.md from archive Postol captures."""
...
OUT = REPO / "statecraft" / "voices" / "postol" / "postol-index.md"
...
GUEST_PREFIX_RE = re.compile(
    r"^(?:(?:prof\.?|mit\s+profes+ssor)\s+)?ted\s+postol:\s*",
    re.I,
)
...
def host_bucket(path: Path, meta: dict) -> str:
    ...
    if "daniel-davis" in name or re.search(r"source-davis-.*postol", name):
        return "davis"
    if "dialogue-works" in name or "-postol-" in name and "daniel-davis" not in name:
        return "nima"
    if "glenn-diesen" in name or ("diesen" in name and "postol" in name):
        return "diesen"
...
def collect_rows() -> list[tuple[str, Path, dict]]:
    ...
    for path in iter_archive_captures_for_shelf("postol", ARCHIVE):
        meta = parse_head(path)
        body = path.read_text(encoding="utf-8")[:8000]
        if shelf_utils.shelf_capture_excluded("postol", path, meta, body):
            continue
        pub = pub_date_key(meta, path)
...
def render_index(
    ...
    diesen = [r for r in rows if host_bucket(r[1], r[2]) == "diesen"]
    if diesen:
        lines.extend(
