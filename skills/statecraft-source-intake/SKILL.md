...
- Prefer `source-dialogue-works-<guest>-*` when Nima Alkhorshid **hosts** on Dialogue Works (`channel_slug: dialogue-works`, `show: Dialogue Works`). Legacy `source-dialogue-works-*` / `source-dialogue-works-*` are read-compat only — do not create new captures under those prefixes.
...
- **Speaker turn labels (post-section):** intake keeps `>>` verbatim; after **`source-section`**, label turns per [source-section § Interview speaker labels](../source-section/SKILL.md#interview-speaker-labels) — `**Nima Alkhorshid:**` / guest full name, **no** `(host)` suffix.
- Person thread on all appearances: **`alkhorshid`** (plus guest thread). `thread:nima` is legacy compat; `expert_id: nima` remains MCQ/EOD compat only (host appendix may link full disambiguation law).
- **Display spelling:** YAML `host:` / `guest:` and post-section speaker labels use **Nima Alkhorshid** (surname with **h**); machine slug stays **`alkhorshid`**. Full law: [dialogue-works-disambiguation § Display spelling](../../../statecraft/channels/dialogue-works/dialogue-works-disambiguation.md).
