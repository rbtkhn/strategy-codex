...
4. **Choose the ladder rung.**
   - Weak or first sighting: add or recommend one row in `skills/skill-candidates.md`.
   - Strong repeatable pattern: draft `skills/_drafts/<name>/SKILL.md`.
   - Mature and operator-approved: promote to `skills/<name>/SKILL.md`, add `manifest.yaml`, sync Cursor skill, and validate.
