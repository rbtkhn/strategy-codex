...
4.5. When the day batch is real and the [news-verify ↔ synthesis gate](../../statecraft/synthesis/NEWS-VERIFY-SYNTHESIS-GATE.md) applies, run **`news verify`** batch **Ship** → `statecraft/notes/wire/YYYY-MM-DD-news-verify-matrix.md` before full synthesis (register-only may link matrix as OPEN).
