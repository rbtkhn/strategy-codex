...
- `codex/`
- `statecraft/`
- `artifacts/` (read-only sources; do not treat as SSOT)
- `skills-portable/`
...
Copy output to `prepared-context/working/<slug>/lane-orientation.md` if useful.
...
If the task spans sessions, note the active slug in the next **bridge** packet under **Open loops** — e.g. `prepared-context/working/<slug>/` in progress.

## Copy discipline

- Use file copy, not move.
- Record every copy in `sources-index.md`.
- Do not commit `prepared-context/working/` — shipped work lands in normal repo paths via EXECUTE.
