...
| F | `STATUS.md` (optional) | When the lane has **active state that changes frequently** (current chapter, last entry, next actions). Quick agent orientation without reading the full README. Not needed for dormant or low-cadence lanes. See [skeleton](#statusmd-skeleton) below. Example: [strategy-notebook/STATUS.md](../../continuity/STATUS.md). |
