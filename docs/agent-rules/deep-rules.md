...
**Predictive History boundary:** The canonical public Predictive History repo is **[`rbtkhn/ph-civ`](https://github.com/rbtkhn/ph-civ)**. It is the two-volume public artifact containing `ph-civ`, `ph-apo`, and `ph-mus` surfaces, including public lecture transcripts, companion commentaries, cards, routes, patterns, and museum manifests. Inside `strategy-codex`, **corpus edits belong only under [`public/ph-civ/`](public/ph-civ/)**; ship to GitHub only via **`python scripts/publish_public_ph_civ.py -m "…" --push`**. Legacy trees under `codex/predictive-history/` or `research/external/youtube-channels/predictive-history/` remain **frozen migration residue / reference only**. Assistants may **observe, review, critique, and cite public `ph-civ` IDs** elsewhere, but must **not** edit PH manuscript content outside `public/ph-civ/` or push upstream without the publish script. Treat `rbtkhn/ph-workshop` as legacy workshop/import provenance unless the operator explicitly invokes that archive lane. See [docs/predictive-history-external-boundary.md](docs/predictive-history-external-boundary.md).
