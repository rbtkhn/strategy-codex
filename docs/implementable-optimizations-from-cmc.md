...
**Proposal:** Add one bullet under `IMPORTANT CONSTRAINTS` in `archive/grace-mar-instance/bot/prompt.py` (after the LOOKUP RULE / MICRO-COPY block):
...
**Files:** `archive/grace-mar-instance/bot/prompt.py` (SYSTEM_PROMPT, in the IMPORTANT CONSTRAINTS section).
...
**Proposal:** In `archive/grace-mar-instance/bot/prompt.py`, in the ANALYST_PROMPT **Rules** section, add:
...
**Files:** `archive/grace-mar-instance/bot/prompt.py` (ANALYST_PROMPT, Rules section).
...
- Add a **whitelist** of paths/scripts that are allowed to write to `self.md`, `self-evidence.md`, or `recursion-gate.md` (e.g. `scripts/process_approved_candidates.py`, and optionally `archive/grace-mar-instance/bot/core.py` only for appending to RECURSION-GATE when staging). Any other code in `archive/grace-mar-instance/bot/` or `scripts/` that matches "write to self.md / self-evidence.md" (or open for write) and is **not** on the whitelist → violation.
...
**Proposal:** In `archive/grace-mar-instance/bot/core.py`, at the top of `get_response` (or just before the branch that calls `analyze_activity_report` / `_run_analyst_background`), add a one-line comment:
...
**Files:** `archive/grace-mar-instance/bot/core.py`.
...
**Files:** New `scripts/spot_check_response_boundary.py` (or similar), or a small function in `archive/grace-mar-instance/bot/core.py` that logs to a dedicated file when enabled via env (e.g. `GRACE_MAR_BOUNDARY_CHECK=1`). Not enabled by default.
...
|---|----------|---------|--------|
| 1 | Response contract (abstain or cite; multiple perspectives) | `archive/grace-mar-instance/bot/prompt.py` | 1 bullet in SYSTEM_PROMPT |
| 2 | Analyst contradiction-preservation rule | `archive/grace-mar-instance/bot/prompt.py` | 1 rule in ANALYST_PROMPT; optional output field |
...
| 5 | Relay-only comment in core | `archive/grace-mar-instance/bot/core.py` | 1 comment |
