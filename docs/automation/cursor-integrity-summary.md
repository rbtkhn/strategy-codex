...
- **Not** a second **CI** system ([test.yml](https://github.com/rbtkhn/strategy-codex/blob/main/.github/workflows/test.yml) / [governance.yml](https://github.com/rbtkhn/strategy-codex/blob/main/.github/workflows/governance.yml) own pass/fail).
- **Not** a **regeneration** bot; **regen** and any script that **writes** derived **artifacts** (including **library** **index** **regen**) is **operator**-**only** (see [work-cadence — stale derived](https://github.com/rbtkhn/strategy-codex/blob/main/docs/skill-work/work-cadence/README.md#when-integrity-reports-stale-derived-exports)).
...
- **No** **in-repo** **file** **edits** (including `**`, `bot/prompt.py`, `self-llm.txt`, manifest, **bundle** **JSON**).
...
| --- | --- | --- | --- |
| **Integrity validation** | `validate-integrity` **failure**; JSON / text lists stale paths | [validate-integrity.py](https://github.com/rbtkhn/strategy-codex/blob/main/scripts/validate-integrity.py) **output** | `python3 scripts/validate-integrity.py --require-proposal-class` (matches [test.yml](https://github.com/rbtkhn/strategy-codex/blob/main/.github/workflows/test.yml) step) or `--json` for readout ([work-cadence](https://github.com/rbtkhn/strategy-codex/blob/main/docs/skill-work/work-cadence/README.md#when-integrity-reports-stale-derived-exports)) |
| **Derived** **artifact** **drift** | manifest, **fork**-**manifest**, **llms.txt**, **runtime** **bundle** vs **sources** | [work-cadence — When integrity reports](https://github.com/rbtkhn/strategy-codex/blob/main/docs/skill-work/work-cadence/README.md#when-integrity-reports-stale-derived-exports) | **After** **operator** **readiness:** `bash scripts/regen_grace_mar_derived.sh` **then** re-**validate** — **automation** **never** runs **regen** |
| **Library** **index** **drift** | CI: `build_library_index` **diff** | [library-index workflow](https://github.com/rbtkhn/strategy-codex/blob/main/.github/workflows/library-index.yml) | `python3 scripts/build_library_index.py` (then `git diff` if needed — **operator** only) |
| **Prompt** / **PRP** **drift** | `self-llm.txt` or **export** **stale** vs **self** / **prompt** | [Regen script order in work-cadence](https://github.com/rbtkhn/strategy-codex/blob/main/docs/skill-work/work-cadence/README.md#when-integrity-reports-stale-derived-exports) | Same **regen** **path** as **row** 2; **automation** **suggests** only |
| **Strategy** / **history** **validation** | Failing **step** in [test.yml](https://github.com/rbtkhn/strategy-codex/blob/main/.github/workflows/test.yml) (notebook checks) | **Log** path / **script** name | **Rerun** the **exact** `python3 scripts/...` from the **failing** **CI** **step** **locally** |
...
*Reconcile this table with workflows when [test.yml](https://github.com/rbtkhn/strategy-codex/blob/main/.github/workflows/test.yml) **changes**.*
...
**Maintenance:** *Last reviewed: 2026-04* — realign with [work-cadence](https://github.com/rbtkhn/strategy-codex/blob/main/docs/skill-work/work-cadence/README.md#when-integrity-reports-stale-derived-exports) and [test.yml](https://github.com/rbtkhn/strategy-codex/blob/main/.github/workflows/test.yml) when they change.
