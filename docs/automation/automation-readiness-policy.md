...
2. **Any** **surface** **that** **writes** or **approves** **edits** to `self.md`, `self-archive.md`, `recursion-gate.md` (**as** **content** **changes**), `session-log.md`, `archive/grace-mar-instance/bot/prompt.py`, or **Record** / **EVIDENCE** **canonical** **paths** is **R5** **unless** a **specific** **companion**-**reviewed** **repo** **policy** **says** **otherwise** (default: **R5**).
