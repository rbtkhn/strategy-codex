...
**Narrative source of truth for the fork layout:** [architecture.md](architecture.md) § Core Principle. This glossary **locks terminology**; the one-line boundary rule below is **verbatim** from [archive/boundary-self-knowledge-self-library.md](archive/boundary-self-knowledge-self-library.md).
...
| **museum knowledge** | Identity-facing knowledge in SELF — what the companion knows *about herself* (museum knowledge section A and related); not domain corpora. See [archive/boundary-self-knowledge-self-library.md](archive/boundary-self-knowledge-self-library.md). |
...
| **Active lane compression** | One **WORK** lane (`docs/archive/skill-work-legacy/work-*`) squeezed into a short memo with recovery paths — `scripts/compress_active_lane.py`. **Not** the JSON paste caps in `platform/config/context_budgets/`. See [active-lane-compression.md](skill-work/active-lane-compression.md). |
...
| **Intelligence harness** | **Product term** — the governed structure around models in **strategy-codex**: source truth, context routing, artifact authority, review, artifact ceilings. Not a specific vendor or model. See [intelligence-harness.md](intelligence-harness.md). |
| **note** | Bounded, source-backed analytical work product under `statecraft/notes/` or `singularity/notes/`. Default promote path: [state-note skill](../.cursor/skills/state-note/SKILL.md). |
| **essay** | Polished cross-channel argument under repo-root [`essays/`](../essays/README.md). |
| **receipt** | Operational proof — ship, validation, handoff, migration, or agent-run record. Preferred term over legacy "transaction" for ops. |
| **transaction (narrow)** | Legacy compatibility pointer or receipt alias only — not the default durable-work ceiling. **Business transaction** = ledger entry (unrelated to statecraft notes). |
...
For full terminology and invariants, see [conceptual-framework.md](conceptual-framework.md), [archive/boundary-self-knowledge-self-library.md](archive/boundary-self-knowledge-self-library.md), and [canonical-paths.md](canonical-paths.md).
