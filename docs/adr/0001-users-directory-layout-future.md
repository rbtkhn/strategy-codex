...
A future migration **might** introduce subfolders under `` (e.g. `identity/`, `evidence/processed/`) for operator ergonomics or large binary layout. That would require:
...
- New instances should continue to use **`_template/`** as a **documentation-only** mirror of required filenames until a migration ships.
