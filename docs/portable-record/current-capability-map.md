...
| **Thin MCP adapter** | Read-only MCP server over governed export classes for tool-using agents. Wraps existing export machinery; stdio transport; no write-back. | Live â€” [`platform/integrations/mcp_adapter.py`](../../platform/integrations/mcp_adapter.py), [mcp-adapter.md](../platform/integrations/mcp-adapter.md) |
