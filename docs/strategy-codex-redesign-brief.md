...
| **P2** | Skill primitives + portable registry (fewer, composable) | **Refactor** | `skills-portable/` + manifest sync mid-flight |
| **P3** | Export-first packaging; import-always-stages | **Keep + extend** | `export_fork.py` already multi-format |
| **P3** | Voice render decoupled from prompt knowledge duplication | **Refactor** | Merge protocol still touches `bot/prompt.py` |
...
| Runtime ≠ Record | `self-memory.md`, `session-log.md`, `artifacts/context/`, [runtime-vs-record.md](runtime-vs-record.md) | Portable harness without vendor lock-in |
...
| Portable skill cores | `skills-portable/`, `skills-portable/manifest.yaml`, `scripts/sync_portable_skills.py` | Host-neutral methodology; Cursor appendix split |
...
|------|----------------|-----|
| List pending | `grace-mar gate list`, `scripts/build_gate_board.py` | Unified entrypoint via [src/grace_mar/cli.py](../src/grace_mar/cli.py) |
| Preview impact | `grace-mar gate diff CANDIDATE-XXXX`, `scripts/preview_candidate_impact.py` | PR-like diff in terminal |
| Review UI | `apps/gate-review-app.py` | Optional; CLI remains primary |
...
1. `grace-mar gate` facade: `board`, `list`, `diff <id>`, `merge` (wraps existing scripts via [src/grace_mar/cli.py](../src/grace_mar/cli.py)).
