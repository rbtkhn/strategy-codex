...
| **OpenClaw-specific provenance preserved into staged candidate metadata** | `implemented` | `integrations/openclaw_stage.py`, `scripts/handback_server.py`, `bot/core.py`, `recursion-gate.md` | When `source=openclaw_stage`, server passes staging_meta; _stage_candidate writes candidate_source, artifact_path, artifact_sha256, constitution_check_status, constitution_rule_ids into the candidate YAML block. |
