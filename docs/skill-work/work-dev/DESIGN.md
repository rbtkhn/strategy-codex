# DESIGN.md â€” Grace-Mar Companion Interface System
...
- Calm discipline with purposeful micro-affordances (inspired by Grokâ€™s clarity and Claudeâ€™s structured workspace)
...
- Background:          `#0F0F0F`
- Surface:             `#1A1A1A`
- Surface Elevated:    `#252525`          â† Sidebars, modals, active panels
- Workspace Panel:     `#1F1F1F`          â† Dedicated area for artifacts and structured outputs
- Text Primary:        `#E0E0E0`
- Text Secondary:      `#A0A0A0`
- Text Muted:          `#666666`

- Primary Accent:      `#0A84FF`          â† Primary actions, buttons, approvals
- Subtle Accent:       `#40C4FF`          â† Hover states, navigation highlights, live indicators (use sparingly)
- Success:             `#22C55E`
...
- Headings: Inter Bold / SemiBold (1.125rem â€“ 2.25rem)
...
The Workspace Panel is Grace-Marâ€™s dedicated, resizable structured output area, inspired by Claude Artifacts. It transforms conversation into tangible, previewable, and governable artifacts while enforcing strict recursion-gate governance.
...
- Can be manually triggered with â€œShow in workspaceâ€, â€œPreview as artifactâ€, or â€œOpen workspace panelâ€
- Default: Right-side split-view (resizable); supports bottom panel for wider content
- Keyboard shortcut: `Ctrl/Cmd + Shift + P` to toggle â€” **implementation:** choose a combo that does not conflict with the host app command palette (e.g. VS Code)
...
- Dismiss â†’ Close current artifact
- Save as Draft â†’ Store in `artifacts/drafts/`
- Preview Changes â†’ Show diff against current DESIGN.md
- **Apply to Record** â†’ Submit formal change proposal to recursion-gate (Primary Accent button; disabled until validation passes)

**Status Indicators**

- Top-right corner shows:
  - â€œDraftâ€ (muted)
  - â€œValidation Passedâ€ (Success color)
  - â€œDrift Detectedâ€ (Warning color)
- Prominent warning banner if validation fails, with direct link to validation report

**Keyboard & Accessibility**

- `Esc` â†’ Collapse or dismiss current artifact
- `Ctrl/Cmd + Enter` â†’ Trigger â€œApply to Recordâ€ when focused
...
Enable fluid movement from conversation â†’ live preview â†’ validation â†’ governed integration into the Record, without breaking focus or introducing intention drift.
...
- v1.0 â€” Initial dark minimal system
- v1.1 â€” Grok-inspired subtle accents and navigation improvements
- v1.2 â€” Claude-inspired Workspace Panels and structured output handling
- v1.3 â€” Language tightened, governance emphasis strengthened
- v1.4 (2026-03-27) â€” Full Workspace Panel interactions (triggers, actions, keyboard, governance flow); artifact paths aligned with canonical EVIDENCE
