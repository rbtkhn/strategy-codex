...
**`coffee` C - Statecraft** now opens a WORK-only repo-root [statecraft](../../../statecraft/README.md) router-first entry: **A. Deploy**, **B. Compact**, **C. Speaker-Bridge**, and **D. Lane Direct**. The second lane menu appears only after **D**, where **A. America**, **B. China**, **C. Persia**, and **D. Russia** remain the direct lane choices. Agents should **not** auto-run the daily brief, `check streams`, bookshelf elicitation, or archived Tri-Frame before this submenu. Those remain explicit named routes unless the operator asks to convert source material into a statecraft instrument.

Named follow-on: after **C. Statecraft**, the operator may also invoke `statecraft civ-state` directly to open the upstream CIV-STATE analysis bench. That command is for `Frame / Retrieve / Promote / Review`, not default lane drafting and not default book-writing.
