...
| Daily roster capture closeout | **`check streams`** for discovery + selection → transcript body (operator paste or bounded fetch) → **`source-intake`** |

## What is **not** deprecated

- **`check streams`** — daily watchlist discovery, clip filter, list-first approval (hand off to **source-intake**, not materialize)
...
- **`statecraft_youtube_discovery.json`** + **`channel-index.md`** — channel listing and routing SSOT
...
- **Known watch URL, need captions:** resolve metadata in the **`check streams`** thread or a one-off fetch step, then **`source-intake`** — do not close on materializer `--apply`.
