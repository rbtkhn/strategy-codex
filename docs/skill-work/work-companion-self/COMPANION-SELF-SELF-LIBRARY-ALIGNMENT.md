...
**Purpose:** Make **`_template/self-library.md`** in [companion-self](https://github.com/rbtkhn/companion-self) an **instance-agnostic governance package** (boundary + schema + **empty `entries:`**), aligned with:
...
- The companion-self template [`_template/self-library.md`](https://github.com/rbtkhn/companion-self/blob/main/_template/self-library.md) (governance + empty shelf).
...
On **`main`**, companion-selfâ€™s [`_template/self-library.md`](https://github.com/rbtkhn/companion-self/blob/main/_template/self-library.md) is a **large** file (LIB rows cloned from grace-marâ€™s corpus). That is **useful as a reference corpus** but it is **not** a neutral template for **every** new instance: it bakes in grace-mar-specific rows and obscures the **SELF-KNOWLEDGE vs SELF-LIBRARY** boundary for first-time operators.

**Alignment goal:** Template = **governance + empty shelf**; optional **example / mirror** docs can hold dense LIB lists **outside** `_template/self-library.md`.

---

## Proposed direction (companion-self PR)

1. **Replace** `_template/self-library.md` with a **short** file: rule-of-one-line boundary, pointer to IFP / boundary doc in-repo, **schema** pointer (companion-self may mirror or link [library-schema.md](https://github.com/rbtkhn/grace-mar/blob/main/docs/library-schema.md) from grace-mar or add `docs/library-schema.md` in template).
2. **Move** the existing long YAML / LIB list to something like **`docs/self-library-seed-example.md`** or **`_template/self-library.EXAMPLE-corpora.md`** (name TBD in PR) â€” clearly labeled **optional**, **not** copied into new instances by default.
...
## Proposed `_template/self-library.md` body (paste into companion-self PR)
...
**Upstream changes:** minimal `_template/self-library.md`; legacy LIB rows in `docs/self-library-example-corpus-grace-mar-derived.md`; `template-manifest.json` + `how-instances-consume-upgrades.md` updated.
...
| 4 | Periodic: re-diff grace-mar `self-library.md` vs [companion-self template](https://github.com/rbtkhn/companion-self/blob/main/_template/self-library.md) when auditing template drift. |
