# Job opportunity review â€” DRAFT (operator)

**Status:** Draft â€” not submitted. Do **not** treat as Voice or Record truth. When ready, paste a tightened block into [recursion-gate.md](../../../recursion-gate.md) **Candidates** only if the companion uses the gate for career decisions; otherwise keep this file in [reflection-proposals](../../../archive/queues/reflection-proposals/) or [recursion-gate-staging](../../../archive/grace-mar-instance/recursion-gate-staging/).

---

## Target

- **Role title:**
- **Company:**
- **Listing URL:**
- **Source board / how found:**

---

## Specification precision (intent)

- **One-sentence functional intent for this application or artifact package:**

---

## Success criteria

- **Functional correctness** (what must be true before you hit send):
- **Edge cases** (what could be misunderstood):
- **Failure modes to avoid** (silent success, spec drift, overstated claims):

---

## Context

- **Persistent references** (paths you will cite honestly):
- **Per-run only** (data for this posting only):

---

## Guardrails

- **What you will not claim** (skills not demonstrated):
- **Human / companion checkpoint** before submit:

---

## Failure pattern check (see checklist)

Review [failure-pattern-checklist.md](../../../archive/grace-mar-instance/recursion-gate-staging/failure-pattern-checklist.md) and note any mitigations:

- Context degradation / spec drift / sycophantic confirmation / tool misuse / cascade / silent failure / blast radius / reversibility / frequency / verifiability

---

## Verification

- **Scripts or docs reviewed:**
- **Token or cost note** (if relevant): `python3 scripts/token_economics.py --tokens â€¦ --config â€¦`

---

## Next action

- [ ] Save copy as `DRAFT-â€¦` under reflection-proposals or recursion-gate-staging
- [ ] Companion / operator sign-off before external submit

