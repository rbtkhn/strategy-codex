...
| **grace-mar** (this repo) | **Governed cognitive-architecture instance** â€” four Record surfaces (SELF, removed operator-books symlink, SKILLS, EVIDENCE), RECURSION-GATE, Voice, work territories; [state model](../../state-model.md) three layers **in doctrine**, not necessarily the same paths as Cici. |
...
| `target_surface` | string | yes | Recommended companion-self destination. One of: `museum knowledge section A` (knowledge), `museum knowledge section B` (curiosity), `museum knowledge section C` (personality), `archive/placeholders/evidence` (ACT/READ/WRITE/CREATE), `memory` (memory, non-Record), `reject`. |
...
|-----------|-----------------|-----------|
| Thought contains a factual claim about the companion's knowledge or learning | `museum knowledge section A` | Knowledge dimension |
| Thought describes a topic the companion showed interest in | `museum knowledge section B` | Curiosity dimension |
| Thought describes a behavioral pattern, preference, or personality trait | `museum knowledge section C` | Personality dimension |
...
**Export (CS â†’ OB1):**
`self.md Â§museum knowledge section A` â†’ `export_open_brain_bundle.py` â†’ `ob1-export/self.identity.md` + `self.identity.meta.json` â†’ OB1 recipe ingest â†’ OB1 thought
