...
| Durable museum knowledge section A knowledge surface | `archive/grace-mar-instance/museum-knowledge.md` |
...
| Capability index (THINK, WRITE, etc.) | **`self-skills.md`** â€” legacy `skills.md` is still read if present (see `scripts/repo_io.py` `resolve_surface_markdown_path`). **Skill split files (this repo):** **`continuity/skill-think.md`**, **`continuity/skill-write.md`**, **`continuity/skill-steward.md`** (resolved via `resolve_skill_split_path`; profile-local overrides when present). Template layouts may carry `self-skill-*.md`; do not create those here unless doctrine explicitly promotes a migration. See [id-taxonomy.md](id-taxonomy.md). **THINK operator doctrine:** [skill-think/README.md](skill-think/README.md). |
| Curated references, canon | `self-library.md` |
| Continuity buffer (continuity â€” short/medium/long; not Record) | **`memory.md`** â€” standard label **memory**. Legacy layouts may still have **`memory.md`**; readers resolve **memory first**, then **memory.md**, via `scripts/repo_io.py` `resolve_memory_path`. |
...
**`self-*` labels in prose:** Standard companion-self component names (**museum identity knowledge (archive)**, **self-identity**, **self-library**, …) and formal surfaces (**museum knowledge**, **removed operator-books symlink**) are defined in [id-taxonomy.md — Capitalization and format](id-taxonomy.md#capitalization-and-format). `archive/grace-mar-instance/museum-knowledge.md` is the canonical museum knowledge section A knowledge file; `self.md` now keeps the overview shell.
...
**Legacy (do not create new):** `SELF.md`, `EVIDENCE.md`, `ARCHIVE.md`, `PENDING-REVIEW.md`, `SKILLS.md` (uppercase), **`skills.md`** (old capability index name), and **`memory.md`** (old memory filename) are **not** canonical for new work â€” use **`self-skills.md`** and **`memory.md`**. The migration script renames `skills.md` â†’ `self-skills.md` and **`memory.md` â†’ `memory.md`** when the canonical file is absent.
