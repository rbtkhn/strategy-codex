...
| `runtime/artifacts/review-dashboard.md` | dashboard | review / gate | review_support | Fenced `CANDIDATE-*` in `recursion-gate.md` (parse) | `build_review_dashboard.py` | Pending / processed gate overview | [â€¦/recursion-gate.md](../archive/grace-mar-instance/recursion-gate.md) (authoritative) |
...
| `runtime/artifacts/repo-convergence-report.json` | machine_feed | operator / multi | derived_non_authoritative | Loop registry input watch paths | `run_repo_convergence.py` | Last convergence status per loop | [repo-convergence.md](repo-convergence.md), `repo-convergence-state.json` |
| `runtime/artifacts/repo-convergence-state.json` | machine_feed | operator / multi | derived_non_authoritative | Same as report (input hashes) | `run_repo_convergence.py` (`--write` success) | Shared dirty-loop baseline | [repo-convergence.md](repo-convergence.md) |
