...
| **Notebook script JSONL** ([STRATEGY-NOTEBOOK-TRACE-CONTRACT](../continuity/STRATEGY-NOTEBOOK-TRACE-CONTRACT.md)) | **Per-entrypoint** line audit (`strategy_page`, `compile_strategy_view`, …) — *which files a script read or wrote*. Complements session-scoped run state. |
