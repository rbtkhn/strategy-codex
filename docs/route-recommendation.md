...
- **Config:** [`platform/config/route_recommendation.json`](../platform/config/route_recommendation.json) â€” human-editable keywords and path blurbs only
- **Outputs:** Derived files under [`runtime/artifacts/route-recommendations/`](../runtime/artifacts/route-recommendations/README.md) (default; git ignores per-receipt blobs like other advisory buckets)
...
Writes `runtime/artifacts/route-recommendations/YYYY-MM-DD/<HHMMSS>-<slug>.md` unless `--no-write`; print only with `--stdout` (combine with `--no-write` to avoid cluttering disk).
...
- `platform/config/route_recommendation.json` â€” six coarse shapes + suggested next steps.
- `runtime/artifacts/route-recommendations/` â€” README + `.gitkeep`; `.gitignore` ignores generated `*.md` receipts.
- `tests/test_recommend_route.py` â€” classification, unclear fallback, render, forbidden path, lane bonus.
- `docs/route-recommendation.md` (this page) plus `runtime/artifacts/README.md` row and registry pointer.
