...
**Advisor module stays canonical in grace-mar** â€” her repo gets **what she needs to run**, not a full mirror of `singularity/work-cici/`.
...
| **Advisor docs** | Full `singularity/work-cici/` | **Usually omit** â€” duplicates INDEX, runbooks, BrewMind pack; risks path confusion unless she explicitly maintains advisor docs in-tree |

**Link expectation:** `SKILL.md` links many paths under `singularity/work-cici/` and `scripts/` â€” those resolve in **grace-mar**. On Cici, **Mode B** (journal template, guardrails, `validate-governed-state.py`) still works; deep links are for mentor context or grace-mar checkout. See [Porting](../../../.cursor/skills/skill-cici/SKILL.md#porting-this-skill-to-cici) (trim **Mode A** if confusing).
